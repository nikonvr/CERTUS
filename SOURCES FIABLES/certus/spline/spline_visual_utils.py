...
def snap_spline_visual_dict(r: dict) -> dict:
    ...
    for k in (
        ...
        if (val := out.get(k)) is not None:
            out[k] = np.asarray(val, dtype=np.float64).copy()
...
def live_monitor_nk_clipboard_tsv_2nm(lam: np.ndarray, n_: np.ndarray, k_: np.ndarray) -> str | None:
    ...
    m = min(lam.size, n_.size, k_.size)
    ...
    lines.extend(f"{l}\t{n:.10f}\t{k:.10e}" for l, n, k in zip(lam_i, n_i, k_i))
