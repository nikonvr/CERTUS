...
def _interp_n_L_linear_on_sigma(
    ...
    inv_t = np.argsort(order_t)

    sk_sorted = sk_target[order_t]

    n_sorted = interp_sorted(sk_sorted, sk_s, n_s, left=n_s[0], right=n_s[-1])

    L_sorted = interp_sorted(sk_sorted, sk_s, L_s, left=L_s[0], right=L_s[-1])
...
def _project_sio2_preset_to_sigma_knots(
    ...
    *,
    ...
    if d_nm_hint is not None:
        val_d = float(d_nm_hint)
        if np.isfinite(val_d) and val_d > 0.0:
            d_out = val_d
