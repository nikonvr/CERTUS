...
from certus.workers.certus_index_workers import _compute_RT_from_config, _spectrum_visibility_target_traces, _index_live_spectrum_visibility, Phase1Callback
...
class IndexOptimizationStrategy:
    ...
    def _run_subset_optim(worker, clues_slice, wls, n_sub, target_T, target_R, exclude_range) -> Any:
        ...
        obj_sub = TLUObjective(wls_sub, target_T_sub, target_R_sub, n_sub_sub, c.data_type, (c.thickness_min, c.thickness_max), use_normalized=c.use_normalized, weight_T=c.weight_T, weight_R=c.weight_R, exclude_range=exclude_range, is_frosted_glass=c.is_frosted_glass)
        res_sub = scipy.optimize.minimize(obj_sub, worker.best_params, method='L-BFGS-B', jac=obj_sub.gradient, bounds=[(lb, ub) for lb, ub in obj_sub.get_bounds()], options={'ftol': SMALL_EPSILON, 'gtol': SMALL_EPSILON, 'maxiter': 500})
        _bs = obj_sub.get_bounds()
        _lbs = _bs[:, 0]
        _ubs = _bs[:, 1]
        res_polish = scipy.optimize.minimize(obj_sub, res_sub.x, method='Nelder-Mead', options={'xatol': 1e-08, 'fatol': SMALL_EPSILON, 'maxiter': 300})
    ...
    def _prepare_run_inputs(worker, c: OptimizationConfig) -> tuple[np.ndarray, np.ndarray, np.ndarray | None, np.ndarray | None, tuple[float, float] | None, TLUObjective]:
        ...
        if _subn == 'Sapphire (Al2O3)':
            worker.logger.info('[FILE] Substrate Al2O3 n(lambda): Sellmeier equation (materials_v1.json, id=3) | k file: %s | k column in xlsx: %s', Path(_SAPPHIRE_DATA_FILE).resolve(), _SAPPHIRE_FILE_HAS_K_COLUMN)
        ...
        if len(wls) == 0:
            worker.logger.error('No spectral points in [%.1f, %.1f] nm found in the loaded data. If you are in IR only (lambda_min >= 2200), do not cut at 2200 nm; widen lambda_min towards UV or let the range cover < 2200 nm.', c.lambda_min, effective_lambda_max)
            raise ValueError('Wavelength window empty for optimization (check min/max lambda or exclusion range).')
        worker.logger.info(f'Wavelength range: {c.lambda_min:.1f} - {effective_lambda_max:.1f} nm' + (f' (TLU fit restricted from {c.lambda_max:.1f} nm)' if _lmf is not None and effective_lambda_max < c.lambda_max else ''))
        ...
        obj = TLUObjective(wls, target_T, target_R, n_sub, c.data_type, (c.thickness_min, c.thickness_max), use_normalized=c.use_normalized, weight_T=c.weight_T, weight_R=c.weight_R, exclude_range=exclude_range, is_frosted_glass=c.is_frosted_glass, has_absorbing_substrate=c.has_absorbing_substrate, k_sub_data=c.k_sub_data, substrate_thickness_nm=c.substrate_thickness_nm)
    ...
    def _run_phase5_final_optimization(worker, obj: TLUObjective, params_list: list[np.ndarray]) -> None:
        ...
        worker.progress_snapshot.emit(build_progress_snapshot(message='Final ultimate optimization...', display_ratio=0.92, progress_ratio=0.92, eta_seconds=None, confidence=0.25, state=StepState.RUNNING, module='INDEX', phase='OPT_FINAL'))
        ...
        try:
            ...
            res_final = scipy.optimize.minimize(obj, avg_params, method='L-BFGS-B', jac=obj.gradient, bounds=[(lb, ub) for lb, ub in obj.get_bounds()], options={'ftol': 1e-14, 'gtol': 1e-14, 'maxiter': 1000})
            res_polish = scipy.optimize.minimize(obj, res_final.x, method='Nelder-Mead', options={'xatol': 1e-09, 'fatol': 1e-14, 'maxiter': 500})
            ...
            if final_mse < worker.best_mse:
                ...
                worker.logger.info(f' Final optimization complete (RMSE unchanged: {calculate_index_rmse(worker.best_mse):.6f})')
        except NUMERICAL_FAULT_EXCEPTIONS as e:
            worker.logger.warning(f'Final optimization failed: {e}')

    @staticmethod

    def _run_phase1_global_search(worker, c: OptimizationConfig, obj: TLUObjective, wls: np.ndarray, target_T: np.ndarray | None, n_sub: np.ndarray) -> bool:
        ...
        worker.progress_snapshot.emit(build_progress_snapshot(message='Global Search...', display_ratio=0.05, progress_ratio=0.05, eta_seconds=None, confidence=0.25, state=StepState.RUNNING, module='INDEX', phase='OPT_GLOBAL'))
        ...
        worker._optimizer = PGlobalOptimizerINDEX(obj, obj.get_bounds(), n_workers=1, config=pg_conf, log_clues=[2, 5], stop_event=worker._stop_event)
        ...
        worker.logger.info('TLU warm start | d=%.1f nm  Eg=%.3f eV  A=%.2f  E0=%.3f  C=%.3f  Eu=%.3f  eps_inf=%.3f', smart_x0[0], smart_x0[1], smart_x0[2], smart_x0[3], smart_x0[4], smart_x0[5], smart_x0[6])
        try:
        ...
        try:
            ...
            if np.isfinite(_y_ws):
                ...
                worker.logger.info('TLU warm-start preview | RMSE = %.6f (traces avant 1er lot PGlobal)', float(np.sqrt(max(0.0, _y_ws))))
                worker._emit_live_best_snapshot()
        ...
        if best:
            ...
            worker.logger.info(f' PGLOBAL Pass 1 complete | Final RMSE: {rmse_best:.6f} | Total evals: {worker._optimizer.n_evals}')
        else:
        ...
        if best:
            ...
            worker.logger.info(f'Using callback-captured best params as fallback (RMSE={calculate_index_rmse(worker.best_mse):.6f}, d={worker.best_params[0]:.2f} nm)')
            worker._emit_live_best_snapshot()
        else:
    ...
    def _package_results(worker, thickness: float, tlu_params: TLUParameters, params_list: list | None=None) -> OptimizationResults:
        ...
        eps1 = epsilon1_TL_analytic(E_full, tlu_params.Eg, tlu_params.A, tlu_params.E0, tlu_params.C, tlu_params.eps_inf)
        ...
        df_data = {'lambda': l_full, 'n_calc': n_calc, 'k_calc': k_calc, 'delta_n_res': delta_n_res, 'delta_k_res': delta_k_res, 'alpha_cm-1': 4.0 * PI * k_calc / (l_full * 1e-07), 'T_calc (%)': T_calc * 100, 'T_norm_calc (%)': T_norm_calc * 100, 'R_calc (%)': R_calc * 100, 'R_norm_calc (%)': R_norm_calc * 100}
        if 'T' in c.target_data.columns:
        ...
        if np.isfinite(worker.best_mse) and float(worker.best_mse) < 1e+99:
            final_mse_canonical = float(worker.best_mse)
        else:
            final_mse_canonical = float(final_mse_recalc)
        stats = {'method': 'PGLOBAL + L-BFGS-B + DeepRefine', 'data_type': c.data_type.name, 'substrate_mode': c.substrate_mode.name, 'thickness_variation_pct': getattr(worker, 'thickness_variation', 0.0), 'mse_uniform_grid_recalc': float(final_mse_recalc)}
        return OptimizationResults(config=c, optimal_thickness=thickness, final_mse=final_mse_canonical, df_results=df, tlu_params=tlu_params, optimization_stats=stats, execution_time=getattr(worker, 'execution_time', 0.0))
