...
csp = pytest.importorskip("certus_spectral_preproc", reason="certus_spectral_preproc indisponible")
...
class TestDynamicSavgolBlend:
    def test_shape_preserved_2d(self) -> None:
        ...
        out = csp.dynamic_savgol_blend(x, y, 15, 2, 25)
...
class TestAutoTuneSavgol:
    def test_returns_odd_windows(self) -> None:
        ...
        w, p, hw = csp.auto_tune_savgol_params(x, y_mat, "Medium (Balanced)")
    ...
    def test_from_dataframe_matches_matrix(self) -> None:
        ...
        w1, p1, h1 = csp.auto_tune_savgol_params_from_dataframe(x, df, "Soft (High Fidelity)")
        w2, p2, h2 = csp.auto_tune_savgol_params(x, df.iloc[:, 1:].values.T, "Soft (High Fidelity)")
    ...
    def test_unknown_preset_uses_default_penalties_like_medium(self) -> None:
        ...
        y_mat = rng.random((2, 120))
        w_u, p_u, hw_u = csp.auto_tune_savgol_params(x, y_mat, "Preset Inconnu XYZ")
        w_m, p_m, hw_m = csp.auto_tune_savgol_params(x, y_mat, "Medium (Balanced)")
    ...
    def test_extreme_mode_limits_poly_to_2(self) -> None:
        ...
        w, p, hw = csp.auto_tune_savgol_params(x, y_mat, "Extreme (Aggressive)")
        assert p == 2
    ...
    def test_auto_tune_single_curve_row(self) -> None:
        ...
        w, p, hw = csp.auto_tune_savgol_params(x, y_mat, "Soft (High Fidelity)")
        assert w % 2 == 1 and hw % 2 == 1
...
class TestDynamicSavgolBlendEdge:
    def test_small_length_preserves_shape(self) -> None:
        ...
        out = csp.dynamic_savgol_blend(x, y, 7, 2, 15)
        assert out.shape == y.shape
