...
class OptimizationManager:
    ...
    def _handle_stopped_workflow_result(self, d: Dict) -> None:
        ...
        self.ui.log("Workflow stopped by user.", "WARNING")
        ...
        if d.get("ok", False) and d.get("ep") is not None:
            ...
            self.ui._apply_qw_values_to_front_table(qw)
            self.ui.ep_current = final_ep.copy()
            ...
            if optim_rmse_is_valid_for_log(rmse):
                ...
                self.ui._update_pareto_record(self.ui.ep_current, rmse)
            rmse_msg = optim_rmse_display_string(rmse)
            self.ui.log(f"Best result kept (RMSE: {rmse_msg}).", "SUCCESS")

        if get_export_config() and self.ui.last_result:
    ...
    def _finalize_if_post_optim_budget_exceeded(self) -> bool:
        ...
        if getattr(self.ui, "_post_optim_start_time", None) is None:
            self.ui._post_optim_start_time = time.time()

        _n_layers = self.ui.front_table.rowCount()
        _budget = optim_post_optim_time_budget_seconds(_n_layers)
        _elapsed = time.time() - self.ui._post_optim_start_time

        if not (
            _elapsed > _budget
            and (
                hasattr(self.ui, "_needle_cycle_step")
                or getattr(self.ui.orchestrator, "_healing_phase", None) is not None
                or getattr(self.ui.orchestrator, "_overshoot_active", False)
            )
        ):
            return False
        ...
        if getattr(self.ui, "_overshoot_active", False):
            self.ui.orchestrator._target_layer_count = getattr(self.ui.orchestrator, "_original_target_count", self.ui.orchestrator._target_layer_count)
            self.ui._overshoot_active = False

        if get_export_config():
            self.ui._export_pending = True
        self.ui._schedule_eval(True)
        self.ui.log("Optimization complete (time budget). Structure stable.", "SUCCESS")
        self.ui._set_busy(False)
    ...
    def _track_and_apply_post_optim_result(self, d: Dict) -> None:
        ...
        if rmse_before_cleanup < getattr(self.ui, "_workflow_best_rmse", float("inf")):
            self.ui._workflow_best_rmse = rmse_before_cleanup

        if rmse_before_cleanup is not None and np.isfinite(rmse_before_cleanup) and rmse_before_cleanup >= 0.0:
            prev_gate_rmse = getattr(self.ui, "_needle_recent_best_rmse", float("inf"))
            if rmse_before_cleanup < prev_gate_rmse - 1e-9:
                self.ui._needle_recent_best_rmse = rmse_before_cleanup
                self.ui._needle_no_improve_rounds = 0
            else:
                self.ui._needle_no_improve_rounds = min(getattr(self.ui, "_needle_no_improve_rounds", 0) + 1, 1000)
        else:
            self.ui._needle_no_improve_rounds = min(getattr(self.ui, "_needle_no_improve_rounds", 0) + 1, 1000)
        ...
        for i, layer in enumerate(stack):
            ...
            qw.append(val)
        self.ui._apply_qw_values_to_front_table(qw, debug_failures=True)
        self.ui._update_thickness_display()

        # Restore exact ep_current that was just overwritten by the rounded table read
        self.ui.ep_current = final_ep.copy()
        self.ui._use_exact_ep = True

        current_best_rmse = getattr(self.ui, "_workflow_best_rmse", float("inf"))
    ...
    def _run_post_optim_cleanup(self, d: Dict) -> int:
        ...
        if getattr(self.ui, "_post_prune", False):
            ...
            self.ui.log(f"Post-prune polish done. RMSE={d.get('rmse', '?')}", "INFO")
            return 0

        if not self.ui.orchestrator._is_in_needle_cycle():
    ...
    def _finalize_completed_optimization_workflow(self) -> None:
        ...
        if not getattr(self.ui, "_decimation_done", False) and self.ui.front_table.rowCount() > 4:
            self.ui._decimation_done = True
            
            # Record the absolute best BEFORE decimation AND BEFORE 5nm rule
            current_best_rmse = getattr(self.ui, "_workflow_best_rmse", float("inf"))
            if current_best_rmse < float("inf"):
                self.ui._update_pareto_record(self.ui.ep_current, current_best_rmse)
                
            self.ui.orchestrator.schedule_smart_pareto_decimation()
            return
        ...
        if get_export_config():
            ...
            if len(self.ui.pareto_history) > 1:
                self.ui.orchestrator.schedule_export_pareto_report()
        ...
        self.ui._schedule_eval(True)
        _final_best = getattr(self.ui, "_workflow_best_rmse", float("inf"))
        _final_n = self.ui.front_table.rowCount()
        self.ui.log(
            f"[FINAL] Optimization complete | {_final_n}L | workflow_best={_final_best:.6f}",
            "SUCCESS",
        )
    ...
    def _initialize_smart_decimation_session(self, n_start: int, n_min_target: int) -> None:
        ...
        self.ui._smart_deci_origin_ep = self.ui.ep_current.copy()
        self.ui._smart_deci_origin_rmse = getattr(self.ui, "_workflow_best_rmse", float("inf"))
        self.ui._smart_deci_origin_table = self._save_table_state()
        self.ui._smart_deci_origin_N = n_start

        self.ui._smart_decimation_start_N = n_start
        self.ui._smart_decimation_min_N = n_min_target
        self.ui._smart_decimation_best_rmse = self.ui._smart_deci_origin_rmse

    def _smart_decimation_remove_and_optimize(self) -> None:
        """One step of smart decimation: remove thinnest layer and re-optimize."""

        current_N = self.ui.front_table.rowCount()

        if self._should_stop_smart_decimation_step(current_N):
            return

        self.ui._smart_decimation_step += 1
        ...
        thinnest_mat = self.ui._safe_get_combo_text(thinnest_idx, 0)

        rmse_before = getattr(self.ui, "_workflow_best_rmse", float("inf"))

        self.ui.log(
            f"📉 Step {self.ui._smart_decimation_step}: N={current_N}->{current_N - 1}"
        ...
        self.ui.front_table.removeRow(thinnest_idx)
    ...
    def _apply_smart_decimation_post_removal_state(self) -> None:
        ...
        self.ui._smart_decimation_polish_pass = 1
    ...
    def _should_stop_smart_decimation_step(self, current_n: int) -> bool:
        ...
        if current_n <= self.ui._smart_decimation_min_N:
            self._finish_smart_decimation()
            return True

        if self.ui._smart_decimation_step >= 50:  # Safety limit
            self.ui.log("Smart decimation: safety limit reached", "WARNING")
            self._finish_smart_decimation()
            return True
    ...
    def _on_smart_decimation_optim_done(self, data) -> None:
        ...
        if not data or "rmse" not in data:
            self.ui.log("Smart decimation: optimization failed", "ERROR")
        ...
        ref_rmse = self.ui._smart_deci_origin_rmse
    ...
    def _apply_smart_decimation_optim_result(self, data: Dict[str, Any], rmse_after: float) -> None:
        ...
        if rmse_after < getattr(self.ui, "_workflow_best_rmse", float("inf")):
            self.ui._workflow_best_rmse = rmse_after
    ...
    def _record_smart_decimation_candidate(self, current_n: int, rmse_after: float) -> None:
        ...
        ep = self.ui.ep_current
        has_sub5nm = (ep is not None) and np.any(np.asarray(ep) < 5.0)

        # _update_pareto_record already enforces strictly-better updates.
        self.ui._update_pareto_record(self.ui.ep_current, rmse_after)
    ...
    def _log_smart_decimation_step_result(
        ...
        n_record = self.ui.pareto_history.get(current_n, {})
        ...
        if abs(rmse_after - best_rmse_for_n) < 1e-6:
            ...
            self.ui.log(f"   ✅ N={current_n}: {rmse_after:.6f}{flag}", "SUCCESS")
            return
    ...
    def _restore_smart_decimation_origin(self, start_n: int) -> None:
        ...
        origin_rmse = getattr(self.ui, "_smart_deci_origin_rmse", float("inf"))
        origin_table = getattr(self.ui, "_smart_deci_origin_table", None)

        if origin_ep is None or origin_table is None:
            return

        self._restore_table_state(origin_table)
        self.ui.ep_current = origin_ep.copy()
        self.ui._update_thickness_display()
        self.ui._workflow_best_rmse = origin_rmse
        self.ui._use_exact_ep = True
        self.ui.log(
    ...
    def _finish_smart_decimation(self) -> None:
        ...
        # Run finalization (which skips decimation loop because _decimation_done=True)
    ...
    def _decimation_remove_and_polish(self) -> None:
        ...
        if N <= 4 or ep is None or len(ep) != N:
            self.ui._finish_pareto_decimation()
        ...
        self.ui.log(
            f"▼ Decimation step {self.ui._decimation_step + 1}: removing layer #{thinnest_idx + 1} "
        ...
        new_N = self.ui.front_table.rowCount()

        self.ui.orchestrator._target_layer_count = new_N

        # Rebuild ep_current from remaining rows

        self.ui._update_thickness_display()

        self.ui._decimation_step += 1

        # Run a local polish to re-optimize

        self.ui._decimation_polishing = True

        self.ui.orchestrator.schedule_local_optim_keep_history()
    ...
    def _on_decimation_polish_done(self) -> None:
        ...
        current_rmse = getattr(self.ui, "_workflow_best_rmse", float("inf"))
        ...
        self.ui.log(
            f"▼ Decimation: {N} layers -> RMSE={current_rmse:.5f} (ref={self.ui._decimation_ref_rmse:.5f})",
            "INFO",
        )
        ...
        if current_rmse > self.ui._decimation_ref_rmse * 3.0 or N <= 4 or self.ui._decimation_step >= 20:
            self.ui._finish_pareto_decimation()
        ...
        self.ui.orchestrator.schedule_decimation_remove_and_polish()
    ...
    def _apply_5nm_minimum(self) -> None:
        ...
        self.ui.log(
            f"[5nm rule] Final design: {self.ui.orchestrator._target_layer_count} layers, "
            f"d_min = {float(np.min(self.ui.ep_current)):.2f}nm",
            "INFO",
        )

        # Reset _workflow_best_rmse because the topology changed and the old RMSE is no longer valid.
        self.ui._workflow_best_rmse = float("inf")
        # Pareto update is deferred to `_on_eval_finished` which will recalculate the true RMSE.
    ...
    def _save_table_state(self) -> list:
        ...
        for r in range(self.ui.front_table.rowCount()):
            ...
            qw = self.ui.front_table.cellWidget(r, 1).value() if self.ui.front_table.cellWidget(r, 1) else 1.0
            ...
            del_cw = self.ui.front_table.cellWidget(r, 4)
    ...
    def _restore_table_state(self, state: list) -> None:
        ...
        for item in state:
            self.ui._add_front_row(item["mat"], item["qw"], item["var"], item.get("del", False))
    ...
    def _revert_to_checkpoint(self) -> None:
        ...
        if not checkpoint:
            self.ui.log("No checkpoint to revert to.", "WARNING")
        ...
        self.ui._workflow_best_rmse = checkpoint["rmse"]
        ...
        self.ui._overshoot_done = True

        # Restore exact thicknesses: set QWOT from ep, then update display

        if checkpoint["ep"] is not None:
            self.ui._update_qwot_from_ep(checkpoint["ep"])

            self.ui._update_thickness_display()

            self.ui.ep_current = checkpoint["ep"].copy()
        ...
        if get_export_config():
            self.ui._export_pending = True  # Set BEFORE schedule_eval

        self.ui._schedule_eval(True)

        _cp_ep = checkpoint.get("ep")
        _ep_sum_str = f"{float(sum(_cp_ep)):.4f}" if _cp_ep is not None else "?"
        self.ui.log(
            f"[REVERT.done] Reverting complete | expecting eval \u2248 {checkpoint['rmse']:.6f} "
            f"| ep_sum={_ep_sum_str}",
            "INFO",
        )
        self.ui.log("Optimization complete (reverted to best). Structure stable.", "SUCCESS")
    ...
    def smart_cleanup(self, update_target: bool = True) -> int:
        ...
        update_target : bool, optional
            ...
            If True, updates self.ui.orchestrator._target_layer_count to match new count.
        ...
        current_rmse = getattr(self.ui, "_workflow_best_rmse", 1.0)

        multiplier = getattr(self.ui, "_cleanup_threshold_multiplier", 150.0)

        threshold = max(0.05, min(1.0, current_rmse * multiplier))  # Adaptive threshold

        rows_to_remove = []

        ep_current = self.ui.ep_current if self.ui.ep_current is not None else []

        if len(ep_current) == self.ui.front_table.rowCount():
            for r in range(len(ep_current) - 1, -1, -1):
                if ep_current[r] < threshold:
                    rows_to_remove.append(r)

        if rows_to_remove:
            self.ui.log(f"Smart cleanup: removing {len(rows_to_remove)} layers < {threshold:.2f} nm (adaptive)", "INFO")
        ...
        if merge_diff > 0:
            ...
            self.ui.log(f"Smart cleanup: merged {merge_diff} adjacent layers", "INFO")

        if changed:
            self.ui._update_layer_count()
            self.ui._update_thickness_display()
            self.ui._workflow_best_rmse = float("inf")  # Reset best RMSE for the new topology
            if update_target:
                self.ui.orchestrator._target_layer_count = self.ui.front_table.rowCount()
    ...
    def _prune_to_target(self, target_count: int) -> int:
        ...
        initial_count = self.ui.front_table.rowCount()
        ...
        def _evaluate_test_stack(ep_test, stack_test):
            try:
                ...
                back_stack = self.ui._get_back_stack()
                has_back_stack = len(back_stack) > 0
                ep_back = np.asarray(self.ui.ep_back_current) if self.ui.ep_back_current is not None else np.array([])
                
                n_layers_T, n_sub, has_back_calc, n_back_T, d_back = optim_prepare_stack_nk_back(
                    mats=mats,
                    stack=stack_test,
                    wls=wls,
                    stack_back=back_stack,
                    ep_back=ep_back,
                ...
                if not getattr(self.ui, "oblique_mode", False):
                    import scipy.optimize
                    var_idx = np.arange(len(ep_test), dtype=np.int32)
                    
                    def obj_func(x):
                        # compute_gradient_all_layers_analytic returns (cost, grad)
                        # but scipy.optimize expects float64.
                        c, g = compute_gradient_all_layers_analytic(x, n_layers_T, n_sub, wls, tgt_vals, tgt_weights, 0.01, has_back_calc, n_back_T, d_back, var_idx)
                        return c, g

                    # Bounds: minimum thickness 0.01, max 2000
                    bounds = [(0.01, 2000.0) for _ in range(len(ep_test))]
                    ...
                    return np.sqrt(res.fun) if res.fun < 1e20 else float('inf')
                else:
                    return float('inf') # fallback
            except Exception:
                return float('inf')

        while self.ui.front_table.rowCount() > target_count:
            current_count = self.ui.front_table.rowCount()
            ep = self.ui.ep_current if self.ui.ep_current is not None else np.array([])
            if len(ep) != current_count:
                self.ui._update_thickness_display()
                ep = self.ui.ep_current if self.ui.ep_current is not None else np.array([])
                if len(ep) != current_count:
                    self.ui.log("Prune: ep_current mismatch, stopping.", "WARNING")
                    break

            # Identify candidates (20% thinnest layers)
            k_candidates = max(1, int(len(ep) * 0.2))
            candidate_indices = np.argsort(ep)[:k_candidates]
            
            best_idx = candidate_indices[0]
            best_rmse = float('inf')
            
            # Fast test for each candidate
            if not getattr(self.ui, "oblique_mode", False) and len(ep) > 0:
                current_stack = self.ui._get_front_stack()
                if len(current_stack) == len(ep):
                    for idx in candidate_indices:
                        test_ep = np.delete(ep, idx)
                        test_stack = current_stack[:idx] + current_stack[idx+1:]
                        
                        # Simulate merge
                        merged_ep = []
                        merged_stack = []
                        for i in range(len(test_stack)):
                            if len(merged_stack) > 0 and merged_stack[-1].mat == test_stack[i].mat:
                                merged_ep[-1] += test_ep[i]
                            else:
                                merged_stack.append(test_stack[i])
                                merged_ep.append(test_ep[i])
                        
                        rmse = _evaluate_test_stack(np.array(merged_ep), merged_stack)
                        if rmse < best_rmse:
                            best_rmse = rmse
                            best_idx = idx

            thinnest_idx = int(best_idx)

            self.ui.log(f"  Prune: layer {thinnest_idx} ({ep[thinnest_idx]:.2f} nm)", "INFO")
            ...
            pre_merge = self.ui.front_table.rowCount()
            self.ui._merge_adjacent_layers()
            post_merge = self.ui.front_table.rowCount()
        ...
        final_count = self.ui.front_table.rowCount()
    ...
    def _needle_thresholds(self, rmse_ref: float) -> tuple:
        ...
        if rmse_ref is None or not np.isfinite(rmse_ref) or rmse_ref <= 0.0:
            return self.ui._needle_success_rel_threshold, self.ui._needle_success_abs_floor

        rel_thresh = 0.002 if rmse_ref < 0.01 else self.ui._needle_success_rel_threshold

        abs_thresh = max(self.ui._needle_success_abs_floor, rmse_ref * 5e-4)
    ...
    def _start_needle_process(self, ) -> None:
        return getattr(self.ui, 'orchestrator', self)._start_needle_process()

    def _on_needle_found(self, res) -> None:
        return getattr(self.ui, 'orchestrator', self)._on_needle_found(res)

    def _handle_needle_no_candidate(self, action, res) -> Any:
        return getattr(self.ui, 'orchestrator', self)._handle_needle_no_candidate(action, res)

    def _handle_needle_no_candidate_below_target(self, action, res, current_count) -> tuple:
        return getattr(self.ui, 'orchestrator', self)._handle_needle_no_candidate_below_target(action, res, current_count)

    def _abort_needle_after_failed_retries(self, ) -> None:
        return getattr(self.ui, 'orchestrator', self)._abort_needle_after_failed_retries()

    def _maybe_prune_needle_overshoot(self, current_count) -> bool:
        return getattr(self.ui, 'orchestrator', self)._maybe_prune_needle_overshoot(current_count)
