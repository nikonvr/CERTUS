...
def _build_live_dict(
    ...
    if t_is_ratio_val:
        ...
        t_th = _ratio_theoretical_from_nk(lam, n_l, k_l, n_sub_full, d_nm)

    else:

        t_th = calculate_transmission_array(lam, n_l, k_l, d_nm, n_sub_full)
    ...
    if cfg.r_exp is not None:
        ...
        if t_is_ratio_val:
            ...
            r_th = _reflectance_ratio_theoretical_from_nk(

                lam, n_l, k_l, n_sub_full, d_nm

            )

        else:

            r_th = _reflectance_absolute_backside_from_nk(

                lam, n_l, k_l, d_nm, n_sub_full

            )

    k_nodes = int(sk.size)

    n_nodes_phys = x_slice_n_to_physical_nodes(

        xv[1 : 1 + k_nodes], sk, cfg.n_mono_band_nm
...
def _polish_lbfgsb_chunked(
    ...
    *,
    ...
    class LBFGSBStop(Exception): pass
    ...
    def _cb(xk):
        ...
        _fv = float(obj(state["x"]))

        state["last_fun"] = _fv

        if _fv < float(_best_fun_seen[0]) - 1e-18:

            _best_fun_seen[0] = _fv

            _best_x_seen[0] = state["x"].copy()
        ...
        if now - last_ui_time[0] > 0.35:
            ...
            if progress_cb:
                ...
                progress_cb(
                    pct,
                    f"Polish L-BFGS-B: iterations={state['nit']} | RMSE={float(np.sqrt(max(float(state['last_fun']), 0.0))):.6f}",
                )
            ...
            if live_cb and sigma_knots is not None:
                ...
                try:
                    ...
                    current_cost = float(obj(xv))

                    live_cb(

                        _build_live_dict(

                            obj.cfg, sigma_knots, xv, current_cost, int(state["nit"])
                    ...
                    logging.getLogger("CERTUS").debug(
                        ...
                        "live_cb failed in _polish_lbfgsb_chunked", exc_info=True
    ...
    try:
        ...
        if spline_pwl_analytic_grad_supported(obj.cfg):

            _gt = obj.analytic_gradient(x_cur)
        ...
        logging.getLogger("CERTUS").debug(
            ...
            "Analytic gradient probe failed, falling back to numerical", exc_info=True
...
def _pack_spline_stage_result(
    ...
    *,
    ...
    _, L_nodes_f, k_floor_mod = enforce_k_floor_on_nodes(

        sk, L_nodes, k_floor=float(cfg.k_clip_lo), allow_insert=False
    ...
    if k_floor_mod:
        ...
        logging.getLogger("CERTUS").info(
            ...
            "k_floor enforce: L nodes adjusted (clamp/flat) in _pack_spline_stage_result"
    ...
    if cfg.t_is_ratio:
        ...
        t_sub_full = np.asarray(

            calculate_T_substrate_array(lam, n_sub_full), dtype=np.float64

        )

        t_th = _ratio_theoretical_from_nk(
    ...
    n_nodes_phys = x_slice_n_to_physical_nodes(
        ...
        x_best[1 : 1 + k_nodes], sk, cfg.n_mono_band_nm
...
def _run_free_knot_stage(
    ...
    *,
    ...
    n_fixed_f: np.ndarray | None = None

    if not optimize_n:

        n_fixed_f = np.interp(sig_f, skn0, nn0)

    reg_w = float(max(getattr(cfg, "lnk_spline_reg_weight", 1e-3), 0.0))

    min_dlam_ratio_req = float(
        max(getattr(cfg, "spline_min_delta_lambda_over_lambda_mean", 0.0) or 0.0, 0.0)
    )
    ...
    if optimize_n:
        ...
        z0 = np.concatenate(
            ...
            ([d0], _w2s(skn0), _w2s(skL0), np.clip(nn0, n_lo, n_hi), np.clip(LL0, L_lo, L_hi))
    ...
    def _obj(z: np.ndarray) -> float:
        ...
        return float(spline_objective_mse_on_masked_grid(
    ...
    def _run_lbfgsb_phase_with_progress(
        ...
        return _lbfgsb_phase_with_progress(
            ...
            z_start, options, phase_idx,

            ph_lo, ph_hi, ph_name, ph_count,

            bnds, stage_name, progress_cb, _obj_tracked,
    ...
    if optimize_n:
        ...
        x_f = np.concatenate(
            ...
            (np.asarray([float(d_f)], dtype=np.float64), xi_ff.ravel(), LL_rf.ravel())
    ...
    t_th = (
        ...
        _ratio_theoretical_from_nk(lam, n_lf, k_lf, cfg.n_sub, d_f)

        if cfg.t_is_ratio

        else calculate_transmission_array(lam, n_lf, k_lf, d_f, cfg.n_sub)
    ...
    if cfg.data_type in (DataType.REFLECTION, DataType.BOTH) and cfg.r_exp is not None:
        ...
        r_th = (
            ...
            _reflectance_ratio_theoretical_from_nk(lam, n_lf, k_lf, cfg.n_sub, d_f)

            if cfg.t_is_ratio

            else calculate_reflection_array(lam, n_lf, k_lf, d_f, cfg.n_sub)
...
def _run_split_free_knot_stage(
    ...
    *,
    ...
    lgr.info(
        ...
        (
            ...
            f"{float(trust_rho_val):.4g}"

            if trust_rho_val is not None

            else "n/a_full_bounds"
    ...
    _log_spline_pipeline_json(
        ...
        pglobal_random_seed=(
            int(getattr(pg_conf, "random_seed"))
            if getattr(pg_conf, "random_seed", None) is not None
            else None
        ),
            ...
            if now - _live_throttle[0] >= _LIVE_INTERVAL:
                ...
                try:
                    ...
                    logging.getLogger("CERTUS").debug(
                        ...
                        "live_cb failed in PGlobal callback", exc_info=True
    ...
    rmse_pg_returned = (
        ...
        float(np.sqrt(max(mse_pg_returned, 0.0))) if np.isfinite(mse_pg_returned) else float("nan")
    ...
    if (
        ...
        and np.isfinite(mse_pg_returned)

        and mse_pg_returned < mse_pg_best_obs - 1e-15
    ...
    x_start = clip_to_bounds(

        np.asarray(x_pg_best_obs, dtype=np.float64).copy(), bounds_full[:, 0], bounds_full[:, 1]
