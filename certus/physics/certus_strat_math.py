...
@njit(cache=True, fastmath=True, nogil=True, inline="always", error_model="numpy")
def _calc_T_added_layer(
    wl: float, n_layer: complex, d_nm: float, n_Sub: complex, m00: complex, m01: complex, m10: complex, m11: complex
) -> float:
...
def _seeded_noise_sample(seed_base: int, group_idx: int, run_idx: int, elem_idx: int, gaussian: bool) -> np.float64:
    s = (
        ...
        + np.uint64(13787848793156543929) * np.uint64(run_idx + 1)
        + np.uint64(10723151780598845931) * np.uint64(elem_idx + 1)
    )
    if not gaussian:
        x = s
        x ^= x >> np.uint64(12)
