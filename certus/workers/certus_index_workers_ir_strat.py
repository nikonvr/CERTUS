...
from certus.workers.certus_index_workers import _compute_RT_from_config, _spectrum_visibility_target_traces, _index_live_spectrum_visibility, IRPGlobalCallback, IRSplineCallback, IRStage2Callback
...
class IRGlobalModelStrategy:
    ...
    def _run_ir_stage0_to_stage2(worker, c: OptimizationConfig, l_full: np.ndarray, n_sub_full: np.ndarray, df_tlu: pd.DataFrame, n_tlu_ref: np.ndarray, obj: IRGlobalObjective, thickness: float) -> tuple | None:
        """Run warm start + PGlobal + final L-BFGS-B polish and return best sample + optimizer."""
        flat_bounds = np.array([[1.0, 16.0], [0.0, 20.0], [0.001, 1.0], [0.0, 20.0], [0.01, 20.0], [-40.0, 40.0], [-100.0, 80.0], [-40.0, 40.0], [-100.0, 80.0], [1e-10, 0.1], [0.5, 12.0], [0.005, 5.0], [1.0, 8.0]], dtype=np.float64)
        ...
        try:
            ...
            if params_sell is not None:
                if params_k8 is None:
                    _k_candidates = [np.array([0.5, -15.0, 0.1, -20.0, 1e-06, 5.0, 1.0, 2.0]), np.array([8.0, -20.0, 0.5, -25.0, 1e-05, 4.0, 0.5, 2.0]), np.array([3.0, -18.0, 0.2, -22.0, 0.0001, 3.5, 0.8, 2.0]), np.array([12.0, -22.0, 1.0, -28.0, 2e-05, 5.0, 1.5, 2.0]), np.array([0.1, -30.0, 0.1, -30.0, 0.0005, 4.5, 0.3, 2.0])]
                    _best_k_y = np.inf
                    _best_k_p = None
                    for _kp in _k_candidates:
                        ...
                        if _y_try < _best_k_y:
                            ...
                            _best_k_p = _kp
                    params_k8 = _best_k_p
                    worker.logger.info(f'  > Warm start: Sellmeier OK | k_8p best candidate RMSE = {np.sqrt(_best_k_y):.6f}')
                else:
                ...
                res_s0 = scipy.optimize.minimize(obj, x0_clamped, method='L-BFGS-B', bounds=list(zip(flat_bounds[:, 0], flat_bounds[:, 1])), options={'maxiter': 3000, 'ftol': 1e-15, 'gtol': 1e-10})
                if np.isfinite(res_s0.fun) and res_s0.fun < 100000000000.0:
                    ...
                    if np.isfinite(y_raw) and y_raw < 100000000000.0:
                        ...
                        y0_polished = y_raw
                        worker.logger.info(f'  > Stage 0 polish rejected, using raw warm start: RMSE = {np.sqrt(y0_polished):.6f}')
                    else:
        ...
        pg_conf = PGlobalConfig.for_dimension(13).with_overrides(max_feval=150000, max_time=480.0, max_active_clusters=100, n_samples_per_iter=3000, convergence_tol=1e-10)
        ...
        worker.logger.info(f'  > PGlobal: Sobol+Clustering | =0.008 | pop={pg_conf.n_samples_per_iter} | max_feval={pg_conf.max_feval}')
        worker.logger.info('-' * 65)
        optimizer = PGlobalOptimizerINDEX(obj, pg_bounds, n_workers=1, config=pg_conf, log_clues=log_clues, stop_event=worker._stop_event)
        ...
        pg_callback = IRPGlobalCallback(worker, optimizer, obj, c, l_full, thickness, n_sub_full, 'IR Global Search', True)
        ...
        try:
            ...
            res_s2 = scipy.optimize.minimize(obj, res_pg.x, method='L-BFGS-B', bounds=list(zip(flat_bounds[:, 0], flat_bounds[:, 1])), options={'maxiter': 5000, 'ftol': 1e-16, 'gtol': 1e-11}, callback=_stage2_cb)
            if np.isfinite(res_s2.fun) and res_s2.fun < res_pg.y:
                _improv = (1.0 - res_s2.fun / res_pg.y) * 100.0
                worker.logger.info(f'  > Stage 2 polish: {np.sqrt(res_pg.y):.6f} -> {np.sqrt(res_s2.fun):.6f}  ({_improv:.1f} % MSE reduction, {res_s2.nit} iters)')
    ...
    def _run_phase21_refinement(worker, c: OptimizationConfig, obj: IRGlobalObjective, target_T: np.ndarray | None, k_spline_knots_lambda_um: np.ndarray | None, k_spline_knots_values: np.ndarray | None, p_opt_final: np.ndarray, res_pg, l_full: np.ndarray, thickness: float, n_sub_full: np.ndarray) -> tuple[np.ndarray | None, np.ndarray | None, np.ndarray | None]:
        ...
        worker.progress_snapshot.emit(build_progress_snapshot(message='Phase 2.1 (90% T)', display_ratio=0.99, progress_ratio=0.99, eta_seconds=None, confidence=0.25, state=StepState.RUNNING, module='INDEX', phase='IR_PHASE_2_1'))
        try:
            ...
            rel = 0.05
            ps = p_opt_final[:5]
            bounds_sell_21 = [(max(1.0, ps[0] * (1 - rel)), min(16.0, ps[0] * (1 + rel))), (max(0.0, ps[1] * (1 - rel)), min(20.0, ps[1] * (1 + rel))), (max(0.001, ps[2] * (1 - rel)), min(1.0, ps[2] * (1 + rel))), (max(0.0, ps[3] * (1 - rel)), min(20.0, ps[3] * (1 + rel))), (max(0.01, ps[4] * (1 - rel)), min(20.0, ps[4] * (1 + rel)))]
            ...
            _cb_21 = IRSplineCallback(worker, obj, c, l_full, thickness, n_sub_full, 'Phase 2.1 (90% T)', knot_lam=k_spline_knots_lambda_um, n_k=n_k, lk_lo=log_k_lo, lk_hi=log_k_hi)
            r21 = scipy.optimize.minimize(phase23_T, x0_21, method='L-BFGS-B', jac=phase23_T.gradient, bounds=bounds_21, options={'maxiter': 1500, 'ftol': 1e-14, 'gtol': 1e-09}, callback=_cb_21)
    ...
    def _run_phase23_knot_reduction(worker, *, c: OptimizationConfig, obj: IRGlobalObjective, res_pg, l_full: np.ndarray, thickness: float, n_sub_full: np.ndarray, log_k_lo: float, log_k_hi: float, lam_min: float, lam_max: float, min_knot_dist_um: float, initial_best_n: int, initial_best_mse: float, initial_best_knot_lam: np.ndarray, initial_best_log_k: np.ndarray, initial_best_p_sell: np.ndarray, n_final: np.ndarray, k_final: np.ndarray, p_opt_final: np.ndarray) -> tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray, float]:
        ...
        while best_n > MIN_KNOTS:
            ...
            rel = 0.1
            ps0 = best_p_sell
            bounds_sell_try = [(max(1.0, ps0[0] * (1 - rel)), min(16.0, ps0[0] * (1 + rel))), (max(0.0, ps0[1] * (1 - rel)), min(20.0, ps0[1] * (1 + rel))), (max(0.001, ps0[2] * (1 - rel)), min(1.0, ps0[2] * (1 + rel))), (max(0.0, ps0[3] * (1 - rel)), min(20.0, ps0[3] * (1 + rel))), (max(0.01, ps0[4] * (1 - rel)), min(20.0, ps0[4] * (1 + rel)))]
            ...
            best_fun_red = np.inf
            while True:
                _cb_red_p1 = IRSplineCallback(worker, obj, c, l_full, thickness, n_sub_full, f'Knot reduction {best_n}->{n_k_try}', knot_lam=knot_lam_try, n_k=n_k_try, lk_lo=log_k_lo, lk_hi=log_k_hi, t0=t0_red)
                r1_try = scipy.optimize.minimize(phase23_try, best_x_red, method='L-BFGS-B', jac=phase23_try.gradient, bounds=bounds_p1_try, options={'maxiter': 3000, 'ftol': 1e-14, 'gtol': 1e-09}, callback=_cb_red_p1)
            ...
            if not np.isfinite(best_fun_red) or best_fun_red > RMSE_RATIO_MAX ** 2 * best_mse_red:
                worker.logger.info(f'  Knot reduction: {best_n} -> {n_k_try} rejected (loss > 10% RMSE), keeping {best_n} knots')
                break
            ...
            phase23_p2_try = Phase23Pass2SplineObjective(obj, n_k_try, lam_min, lam_max, obj.k_max_guard, min_knot_dist_um)
            _cb_red_p2 = IRSplineCallback(worker, obj, c, l_full, thickness, n_sub_full, f'Knot reduction {best_n}->{n_k_try}', phase23_obj=phase23_p2_try, n_k=n_k_try, lk_lo=log_k_lo, lk_hi=log_k_hi)
            try:
                r2_try = scipy.optimize.minimize(phase23_p2_try, x0_p2_try, method='L-BFGS-B', jac=phase23_p2_try.gradient, bounds=bounds_p2_try, options={'maxiter': 2000, 'ftol': 1e-14, 'gtol': 1e-09}, callback=_cb_red_p2)
                if np.isfinite(r2_try.fun) and r2_try.fun <= RMSE_RATIO_MAX ** 2 * best_mse_red:
                    ...
                    worker.logger.info(f'  Knot reduction: {best_n + 1} -> {best_n} OK (RMSE {np.sqrt(best_mse_red):.6f}, < 10% loss)')
                else:
                    worker.logger.info(f'  Knot reduction: {best_n} -> {n_k_try} rejected (loss > 10% RMSE), keeping {best_n} knots')
                    break
    ...
    def _run_phase23_spline_refinement(worker, c: OptimizationConfig, obj: IRGlobalObjective, res_pg, l_full: np.ndarray, thickness: float, n_sub_full: np.ndarray) -> tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray | None, np.ndarray | None]:
        ...
        try:
            ...
            bounds_sell = [(max(1.0, p_sell0[0] * (1 - rel)), min(16.0, p_sell0[0] * (1 + rel))), (max(0.0, p_sell0[1] * (1 - rel)), min(20.0, p_sell0[1] * (1 + rel))), (max(0.001, p_sell0[2] * (1 - rel)), min(1.0, p_sell0[2] * (1 + rel))), (max(0.0, p_sell0[3] * (1 - rel)), min(20.0, p_sell0[3] * (1 + rel))), (max(0.01, p_sell0[4] * (1 - rel)), min(20.0, p_sell0[4] * (1 + rel)))]
            ...
            worker.progress_snapshot.emit(build_progress_snapshot(message='Phase 2.3 spline k', display_ratio=0.98, progress_ratio=0.98, eta_seconds=None, confidence=0.25, state=StepState.RUNNING, module='INDEX', phase='IR_PHASE_2_3'))
            t0_p1 = time.time()
            best_x_23 = x0_23.copy()
            best_fun_23 = np.inf
            while True:
                _cb_p1 = IRSplineCallback(worker, obj, c, l_full, thickness, n_sub_full, 'Phase 2.3 spline k', knot_lam=knot_lam_um, n_k=num_knots, lk_lo=log_k_lo, lk_hi=log_k_hi, t0=t0_p1)
                r23 = scipy.optimize.minimize(phase23_obj, best_x_23, method='L-BFGS-B', jac=phase23_obj.gradient, bounds=bounds_23, options={'maxiter': 3000, 'ftol': 1e-14, 'gtol': 1e-09}, callback=_cb_p1)
            ...
            if np.isfinite(r23.fun) and r23.fun < 10000000000.0:
                ...
                worker.logger.info(f'  Phase 2.3 Pass 1 done: RMSE {np.sqrt(r23.fun):.6f} (before: {rmse_before_23:.6f}) [{elapsed_p1:.1f} s]')
                ...
                phase23_pass2 = Phase23Pass2SplineObjective(obj, num_knots, lam_min, lam_max, obj.k_max_guard, min_knot_dist_um)
                worker.logger.info('  Phase 2.3 Pass 2: knot positions + values...')
                worker.progress_snapshot.emit(build_progress_snapshot(message='Phase 2.3 Pass 2', display_ratio=0.99, progress_ratio=0.99, eta_seconds=None, confidence=0.25, state=StepState.RUNNING, module='INDEX', phase='IR_PHASE_2_3_P2'))
                _cb_p2 = IRSplineCallback(worker, obj, c, l_full, thickness, n_sub_full, 'Phase 2.3 Pass 2', phase23_obj=phase23_pass2, n_k=num_knots, lk_lo=log_k_lo, lk_hi=log_k_hi)
                try:
                    r23p2 = scipy.optimize.minimize(phase23_pass2, x0_pass2, method='L-BFGS-B', jac=phase23_pass2.gradient, bounds=bounds_23_pass2, options={'maxiter': 2000, 'ftol': 1e-14, 'gtol': 1e-09}, callback=_cb_p2)
                ...
                n_final, k_final, p_opt_final, best_knot_lam, best_log_k, _ = worker._run_phase23_knot_reduction(c=c, obj=obj, res_pg=res_pg, l_full=l_full, thickness=thickness, n_sub_full=n_sub_full, log_k_lo=log_k_lo, log_k_hi=log_k_hi, lam_min=lam_min, lam_max=lam_max, min_knot_dist_um=min_knot_dist_um, initial_best_n=num_knots, initial_best_mse=worker.best_mse, initial_best_knot_lam=k_spline_knots_lambda_um, initial_best_log_k=np.log(np.clip(k_spline_knots_values, 1e-09, obj.k_max_guard)), initial_best_p_sell=p_opt_final[:5], n_final=n_final, k_final=k_final, p_opt_final=p_opt_final)
    ...
    def _package_results(worker, n, k, thickness, l_full, p_opt, n_T=None, k_T=None, p_T=None, k_spline_knots_lambda_um=None, k_spline_knots_values=None) -> Any:
        c = worker.config
        ...
        df = pd.DataFrame({'lambda': l_full, 'n_calc': n, 'k_calc': k, 'T_calc (%)': Tc * 100, 'T_norm_calc (%)': T_norm * 100, 'R_calc (%)': Rc * 100, 'R_norm_calc (%)': R_norm * 100})
        ...
        if n_T is not None:
            ...
            df['delta_n'] = 0.0
            df['delta_k'] = 0.0
        results = OptimizationResults(config=c, optimal_thickness=thickness, final_mse=worker.best_mse, df_results=df, tlu_params=None, optimization_stats={'method': 'PGLOBAL Global Refinement (Sellmeier+k8p)', 'final_cost': worker.best_mse}, execution_time=0.0, sellmeier_params=p_opt[:5])
