...
from certus_qt_widgets import QThread
...
from certus_re_results_builder import REResultsBuilder as REResultsPayloadBuilder


from certus_physics import calc_rmse
...
class REPhase4Result:
    ...
    def from_legacy_dict(payload: dict[str, Any]) -> "REPhase4Result":
        ...
        return REPhase4Result(
            ...
            re_sub_cauchy_a0=(
                float(payload["re_sub_cauchy_a0"])
                if payload.get("re_sub_cauchy_a0") is not None
                else None
            ),
            re_sub_cauchy_a1=(
                float(payload["re_sub_cauchy_a1"])
                if payload.get("re_sub_cauchy_a1") is not None
                else None
            ),
            re_sub_cauchy_a2=(
                float(payload["re_sub_cauchy_a2"])
                if payload.get("re_sub_cauchy_a2") is not None
                else None
            ),
        )
...
def _re_backside_bundle_fixed(ep_local, n_layers_sel, n_sub_sel, wls_sel, var_idx, angle, is_s_pol):
...
def _re_eval_angle_physics_for(
    ...
    if pl == "avg":
        ...
        if not inc_back:
            ...
            yRs, dRs, yTs, dTs = _re_backside_bundle_fixed(

                ep_use, n_lay_s, n_sub_s, wls_s, var_idx, angle, True

            )

            yRp, dRp, yTp, dTp = _re_backside_bundle_fixed(

                ep_use, n_lay_s, n_sub_s, wls_s, var_idx, angle, False
        ...
        if not inc_back:
            ...
            rR, rdR, rT, rdT = _re_backside_bundle_fixed(

                ep_use, n_lay_s, n_sub_s, wls_s, var_idx, angle, is_s_pol
...
class REWorker(QThread):
    ...
    def __init__(self, cfg: dict[str, Any] | REWorkerRequest):
    ...
    def _execute_phase1(self) -> list[dict]:
        ...
        for run_idx, (label, wt_wls, x0_run) in enumerate(c.runs):
            ...
            def _fun_res(x: np.ndarray):

                _eval_both(x)

                return _cache["res"]

            def _jac_res(x: np.ndarray):
            ...
            try:
                ...
                x_use = (
                    ...
                    if _cache["x"] is not None

                    else np.asarray(x0_run, dtype=np.float64).ravel()
    ...
    def _execute_phase1_p4_scan(self) -> None:
        ...
        if (
            ...
            _p4_fd_ap_s2 = float(
                ...
                self.cfg.get("re_p4_ap_fd_step_deg", RE_P4_AP_FD_STEP_DEG)
            ...
            _p4_tol_s2 = float(
                ...
                self.cfg.get(
                    ...
                    "re_phase4_trf_tol_factor", RE_PHASE4_TRF_TOL_FACTOR
            ...
            def _eval_both_s2(xv_full: np.ndarray, *, emit_interval: float = 4.0):
                ...
                for _k in range(_nap_s2):
                    ...
                    L._re_state["re_aperture_knots"][:] = xv_p[

                        i_ap_s2 : i_ap_s2 + _nap_s2
            ...
            def _fun_s2(xv_full: np.ndarray):

                _eval_both_s2(xv_full)

                return _c_s2["res"]

            def _jac_s2(xv_full: np.ndarray):
            ...
            try:
                ...
                nfev_s2 = int(_c_s2.get("i", 0) or 0)
            ...
            rmse_s2_sp = float(
                ...
                np.sqrt(max(L._report_mse_spectral(ep_s2_f, L._correc_nom), 0.0))
            ...
            _kn_s2 = np.asarray(
                ...
                L._re_state["re_p4_beam_knots_lam_nm"], dtype=float
            ...
            L.rmse_final_milestone[0] = float(

                _top_s2_dto.rmse_combined if _top_s2_dto is not None else float("nan")
            ...
            _pairs_s2_done = ", ".join(
                ...
                f"(lambda={float(lk):.0f}nm->{float(ak):.2f})"

                for lk, ak in zip(_kn_s2, ap_s2_f, strict=False)
        ...
        elif L._re_use_staged_order and not any(

            float(meta["angle"]) >= 10.0 for meta in L.oblique_config_meta

        ):

            logging.info(

                "RE staged order  step 2/3 skipped (all angles < 10 for P4 averaging)."
            ...
            logging.info(
                ...
                "RE staged order  step 2/3 skipped (no available phase-1 candidate)."
    ...
    def _execute_phase2_splines(self) -> None:
        ...
        _p2fd_lam = float(
            ...
            self.cfg.get("re_phase2_lam2_fd_step", RE_PHASE2_LAM2_FD_STEP)
        ...
        _fd_par = bool(
            ...
            self.cfg.get("re_phase2_fd_parallel", RE_PHASE2_FD_PARALLEL)
        ...
        _fd_cap = _mw_cfg if _mw_cfg > 0 else min(8, _cpu)

        _nk = int(RE_SPLINE_N_KNOTS)

        _fd_nw = (

            max(1, min(_fd_cap, 2 * _nk + 1)) if _fd_par else 1
        ...
        if results and not self._stop:
            ...
            _prep_msg = (
                ...
                f"RE step 3/3: full joint optimization preparing "

                f"(K={_nk} spline knots, nlambda={len(wls)})..."
            ...
            _top_k = min(
                ...
                int(
                    ...
                    self.cfg.get(
                        ...
                        "re_phase2_top_k", RE_GUI_DEFAULT_RE_PHASE2_TOP_K
            ...
            _merge_rtol = float(
                ...
                self.cfg.get(
                    ...
                    "re_phase2_top_k_merge_rel_tol", RE_PHASE2_TOP_K_MERGE_REL_TOL
            ...
            if _top_k > 1 and len(results) >= 2 and _merge_rtol >= 0.0:
                ...
                if abs(c0 - c1) <= _merge_rtol * max(abs(c0), 1e-12):
                    ...
                    logging.info(
                        ...
                        "RE phase 2: top_k reduced to 1 (candidates #1/#2 nearly identical, "
            ...
            b_lam = RE_SPLINE_NODE2_BOUNDS_NM if (_act_h or _act_l) else (RE_SPLINE_NODE2_DEFAULT_NM - 1e-10, RE_SPLINE_NODE2_DEFAULT_NM + 1e-10)

            bounds_spline = [(float(-_env_knot[i]), float(_env_knot[i])) for i in range(_nk)]
            ...
            _n_tab_sub = np.real(
                ...
                np.asarray(n_sub_nominal, dtype=np.complex128)

            ).astype(np.float64).ravel()

            _Phi_sub = re_substrate_cauchy_phi_matrix(wls, lambda_ref)

            _theta0_sub = re_substrate_cauchy_initial_theta(

                _n_tab_sub, wls, lambda_ref

            )

            _use_sub_c3 = bool(

                self.cfg.get("re_phase2b_substrate_cauchy", True)

            ) and (_theta0_sub is not None)
            ...
            if _use_sub_c3:
                ...
                logging.warning(
                    ...
                    "RE phase 2b  Cauchy substrate requested but feasible set empty; "

                    "phase 2b with no substrate DOF."
                ...
                logging.info(
                    ...
                    "RE phase 2b: substrate - fixed tabulated n(lambda) (Cauchy / corridor disabled)."
            ...
            _hl_w = float(
                ...
                self.cfg.get(
                    ...
                    "re_hl_delta_re_reg_sqrt_w", RE_HL_DELTA_RE_REG_SQRT_W
            ...
            _dzre_l = float(
                ...
                self.cfg.get(
                    ...
                    "re_hl_delta_re_deadzone_abs", RE_RE_DEADZONE_DELTA_RE_ABS
            ...
            _dzqw_l = float(
                ...
                self.cfg.get("re_qwot_deadzone_abs", RE_RE_DEADZONE_QWOT_ABS)
            ...
            logging.info(
                ...
                self.cfg.get(
                    ...
                    "re_spline_tikhonov_scale", RE_GUI_DEFAULT_RE_SPLINE_TIKHONOV
            ...
            def _eval_both_p2(xv: np.ndarray, emit_interval: float = 3.0):
                ...
                if _re_state.get("is_phase4"):

                    _kap = np.asarray(

                        _re_state["re_aperture_knots"], dtype=np.float64
                ...
                cur_lam2 = float(xv[i_lam])

                if _c["tk_w_c"] is None or abs(cur_lam2 - (_c["tk_lam2"] or 0.0)) > 1.0:

                    _c["tk_w_c"] = re_compute_tikhonov_weights(re_knots_wavelengths(cur_lam2), wls)
                ...
                ep_x = np.asarray(xv[:n_layers_count], dtype=np.float64, copy=False)

                dh4 = xv[i0:i0 + _nk].copy()

                dl4 = xv[i0 + _nk:i_lam].copy()

                lam2 = float(xv[i_lam])

                th4 = (

                    np.asarray(xv[i_cu : i_cu + 3], dtype=np.float64).ravel()
                ...
                b_mat_c = re_compute_spline_basis_matrix(re_knots_wavelengths(lam2), wls)

                env_c = re_envelope_max_delta_n(wls, scale=re_env_s)
                ...
                mse, _, r_c, j_ep = _mse_grad_accumulate_ep(

                    ep_x, wt_spectral, True, cor_spl, return_residuals=True

                )

                if _c["i"] == 1:

                    _n_fd = (_n_joint_fd) if _fd_1s else (_n_joint_fd * 2)

                    logging.info(

                        "RE phase 2b  1st TRF eval: %d thick. analytic, %d FD residual blocks "
                        ...
                        "forward" if _fd_1s else "centered",

                        _fd_nw,

                    )

                J_var = np.zeros((len(r_c), _n_joint_fd), dtype=np.float64)
                ...
                def _p2_fd_j_res(j: int) -> tuple[int, np.ndarray]:
                    ...
                    if _use_sub_c3:
                        ...
                        if j < 2 * _nk:
                            ...
                            dh_p = xv64[i0 : i0 + _nk].copy()

                            dl_p = xv64[i0 + _nk : i_lam].copy()
                            ...
                            if j < _nk:
                                ...
                                dl_p[j - _nk] += _p2fd_spl

                            cor_p = _cor_sub3(dh_p, dl_p, float(xv64[i_lam]), th4)

                            h = _p2fd_spl

                        elif j == 2 * _nk:

                            lam_p = float(xv64[i_lam]) + _p2fd_lam
                        ...
                        cor_p = ("spline_cached", dh_p, dl_p, float(xv64[i_lam]), b_mat_c, env_c, tk_w_c)
                    ...
                    r_p = _mse_grad_accumulate_ep(

                        ep_x, wt_spectral, False, cor_p, return_residuals=True
                    ...
                    if _use_sub_c3:
                        ...
                        if j < 2 * _nk:
                            ...
                            dh_m = xv64[i0 : i0 + _nk].copy()

                            dl_m = xv64[i0 + _nk : i_lam].copy()
                            ...
                            if j < _nk:
                                ...
                                dl_m[j - _nk] -= _p2fd_spl

                            cor_m = _cor_sub3(dh_m, dl_m, float(xv64[i_lam]), th4)

                        elif j == 2 * _nk:

                            lam_m = float(xv64[i_lam]) - _p2fd_lam
                        ...
                        cor_m = ("spline_cached", dh_m, dl_m, float(xv64[i_lam]), b_mat_c, env_c, tk_w_c)
                    ...
                    r_m = _mse_grad_accumulate_ep(

                        ep_x, wt_spectral, False, cor_m, return_residuals=True

                    )[2]

                    return j, (r_p - r_m) / (2.0 * h)

                _nw_j = min(_fd_nw, _n_joint_fd)
                ...
                for j in range(_n_joint_fd):

                    if j < _nk and not _act_h:

                        continue

                    if _nk <= j < 2 * _nk and not _act_l:

                        continue

                    if j == 2 * _nk and not (_act_h or _act_l):
                ...
                if _use_sub_c3:
                    ...
                    _c["jac"] = np.hstack([j_ep, J_var[:, :n_sp]])
                ...
                if _c["i"] == 1 or (now - _c["last_emit"]) >= emit_interval:
                    ...
                    rq2 = _compute_qwot_rmse(ep_x, cor_spl)

                    rmse_cur = _rmse_combined(rs2, rq2)

                    if _p2_trf_log_tag[0] == "phase 4 finale":

                        _ap_k = np.asarray(

                            _re_state["re_aperture_knots"], dtype=float

                        ).ravel()[: int(RE_P4_BEAM_N_KNOTS)]

                        _lam_k = np.asarray(

                            _re_state.get("re_p4_beam_knots_lam_nm", []), dtype=float

                        ).ravel()[: _ap_k.size]

                        _pairs = ", ".join(

                            f"(lambda={lk:.0f}nm->{ak:.2f})"

                            for lk, ak in zip(_lam_k, _ap_k, strict=False)
                    ...
                    _intra_2b = min(
                        ...
                        float(_c["i"]) / float(max(_maxiter_p2b, 1)),

                    )

                    _emit_re_prog(

                        _pct_p2b(pl, _p2_ki_slot[0], _intra_2b),
                    ...
                    _emit_re_spectrum_live(
                        ...
                        ep_x, _c["i"], correc=cor_spl, last_mse=_c["mse"], force=False

                    )

            def _fun_res_p2(xv: np.ndarray, emit_interval: float = 3.0):

                _eval_both_p2(xv, emit_interval=emit_interval)

                return _cb2_ref[0]["res"]

            def _jac_res_p2(xv: np.ndarray, emit_interval: float = 3.0):

                _eval_both_p2(xv, emit_interval=emit_interval)

                return _cb2_ref[0]["jac"]
            ...
            for _ki in range(_top_k):
                ...
                rmse_p2_x0_sp = float(
                    ...
                    np.sqrt(max(_report_mse_spectral(ep_p1, _correc_nom), 0.0))

                )

                rmse_p2_x0 = _rmse_combined(

                    rmse_p2_x0_sp, _compute_qwot_rmse(ep_p1, _correc_nom)
                ...
                if _prefit_max > 0 and not self._stop:
                    ...
                    def _eval_both_p2a(x_sp: np.ndarray):
                        ...
                        cur_lam2 = float(x_sp[2 * _nk])

                        if _cb2a["tk_w_c"] is None or abs(cur_lam2 - (_cb2a["tk_lam2"] or 0.0)) > 1.0:

                            _cb2a["tk_w_c"] = re_compute_tikhonov_weights(re_knots_wavelengths(cur_lam2), wls)
                        ...
                        dh = np.asarray(x_sp[:_nk], dtype=np.float64, copy=False).reshape(_nk)

                        dl = np.asarray(x_sp[_nk: 2 * _nk], dtype=np.float64, copy=False).reshape(_nk)

                        lam_p = float(x_sp[2 * _nk])

                        # Cache the B_matrix and envelope for the central point and the 10 FD perturbations (dh, dl)

                        b_mat_c = re_compute_spline_basis_matrix(re_knots_wavelengths(lam_p), wls)

                        env_c = re_envelope_max_delta_n(wls, scale=re_env_s)

                        cor_c = ("spline_cached", dh, dl, lam_p, b_mat_c, env_c, tk_w_c)

                        mse, _, r_c, _ = _mse_grad_accumulate_ep(

                            ep_p1, wt_spectral, False, cor_c, return_residuals=True

                        )

                        if _cb2a["i"] == 1:

                            _n_fd = n_sp if _fd_1s else (n_sp * 2)
                        ...
                        def _pf_fd_j_res(j: int) -> tuple[int, np.ndarray]:
                            ...
                            cor_p = ("spline_cached", dhp, dlp, lamx, b_mat_c, env_c, tk_w_c) if j < 2 * _nk else ("spline", dhp, dlp, lamx, tk_w_c)

                            r_p = _mse_grad_accumulate_ep(

                                ep_p1, wt_spectral, False, cor_p, return_residuals=True
                            ...
                            cor_m = ("spline_cached", dhm, dlm, lamm, b_mat_c, env_c, tk_w_c) if j < 2 * _nk else ("spline", dhm, dlm, lamm, tk_w_c)

                            r_m = _mse_grad_accumulate_ep(

                                ep_p1, wt_spectral, False, cor_m, return_residuals=True

                            )[2]

                            return j, (r_p - r_m) / (2.0 * hs)

                        _nw_sp = min(_fd_nw, n_sp)
                        ...
                        if _nw_sp <= 1:
                            ...
                            _f_pf = [_ex_pf.submit(_pf_fd_j_res, j) for j in range(n_sp)]
                        ...
                        if _cb2a["i"] == 1 or (now - _cb2a["last_emit"]) >= 3.0:
                            ...
                            _intra_2a = min(
                                ...
                                float(_cb2a["i"]) / float(max(_prefit_max, 1)),

                            )

                            _emit_re_prog(

                                _pct_p2a(pl, _ki, _intra_2a),

                                f"RE phase 2a  iter {_cb2a['i']} TRF spline prefit...",

                            )

                    def _fun_res_p2a(x_sp: np.ndarray):

                        _eval_both_p2a(x_sp)

                        return _cb2a["res"]

                    def _jac_res_p2a(x_sp: np.ndarray):
                    ...
                    rmse_sp_pf = float(
                        ...
                        np.sqrt(max(_report_mse_spectral(ep_p1, cor_pf), 0.0))

                    )

                    rmse_after_prefit = _rmse_combined(

                        rmse_sp_pf, _compute_qwot_rmse(ep_p1, cor_pf)
                    ...
                    _emit_re_prog(
                        ...
                        f"RE phase 2a done  RMSE {rmse_after_prefit:.6f}, "

                        f"{nfev_phase2_prefit} evals",
                    ...
                    logging.info(
                        ...
                        "RE phase 2a  skipped (re_phase2_spline_prefit_maxiter=0), "

                        "using zero splines for 2b."
                ...
                dl_end = np.asarray(
                    ...
                    x_end[i0 + _nk:i_lam], dtype=np.float64
            ...
            if p2_candidates:
                ...
                _cor_best_p2 = p2_result_to_correc_tuple(

                    results[0], _use_sub_c3
    ...
    def _execute_phase3_shakes(self) -> None:
        ...
        n_shakes = int(
            ...
            self.cfg.get(
                ...
                "re_phase3_shake_rounds", RE_GUI_DEFAULT_RE_PHASE3_SHAKES
        ...
        if n_shakes > 0 and not self._stop and results:
            ...
            _rng3 = np.random.default_rng(

                (_sk_i % (2**32)) if _sk_i >= 0 else None
            ...
            x_best = np.concatenate([
                ...
                ([best_res_dto.re_sub_cauchy_a0, best_res_dto.re_sub_cauchy_a1, best_res_dto.re_sub_cauchy_a2] if _use_sub_c3 and best_res_dto.re_sub_cauchy_a0 is not None else [])
            ...
            for _si in range(n_shakes):
                ...
                try:
                    ...
                    res_sh = least_squares(
                        ...
                        _fun_res_p2, x_shake, method="trf", bounds=bounds_p2_trf,

                        jac=_jac_res_p2, x_scale="jac",

                        ftol=RE_LBFGSB_FTOL * 10, gtol=RE_LBFGSB_GTOL * 10,
                ...
                if _use_sub_c3:
                    ...
                    cor_sh = (
                        ...
                        "spline_sub3", dh_sh, dl_sh, lam_sh,

                        float(th_sh[0]), float(th_sh[1]), float(th_sh[2])
                ...
                if rmse_comb_sh < rmse_best - 1e-8:
                    ...
                    _shake_best = float(
                        ...
                        _cb2_ref[0].get("best_rmse_combined", rmse_comb_sh)
    ...
    def _execute_phase4_beam(self) -> None:
        ...
        if results and not self._stop:
            ...
            if _has_high_angle and len(_x0_p4) > 1:
                ...
                _lo_ap, _hi_ap = RE_P4_BEAM_AP_BOUNDS_DEG

                _apb_cfg = self.cfg.get("re_phase4_ap_bounds_deg")

                if _apb_cfg is not None:

                    _vb = np.asarray(_apb_cfg, dtype=np.float64).ravel()
                    ...
                    if _vb.size >= 2:
                        ...
                        if 0.0 < _c0 < _c1 < 90.0:
                            ...
                            _lo_ap, _hi_ap = _c0, _c1

                _p4_trf_nfev = int(

                    self.cfg.get("re_phase4_trf_max_nfev", RE_PHASE4_TRF_MAX_NFEV)
                ...
                _p4_hi_ang = sorted(

                    {
                        ...
                        for m in oblique_config_meta

                        if float(m["angle"]) >= 10.0
                ...
                _p4_cfg_knm = self.cfg.get("re_p4_beam_ap_knots_nm")

                _p4_ft_eff = float(RE_LBFGSB_FTOL) * _p4_tol

                _p4_gt_eff = float(RE_LBFGSB_GTOL) * _p4_tol

                _kn_sorted_cfg = np.sort(

                    np.asarray(_kn_log, dtype=np.float64).ravel().copy()
                ...
                _p4_band_thr = np.array(
                    ...
                    [
                        ...
                        0.5 * (_kn_sorted_cfg[i] + _kn_sorted_cfg[i + 1])

                        for i in range(_nap - 1)
                ...
                _p4_band_thr_s = np.array2string(

                    _p4_band_thr, precision=2, separator=","
                ...
                best_rmse_ap = float(
                    ...
                    np.sqrt(max(_report_mse_spectral(ep_p4, _cor_base), 0.0))
                ...
                if _p4_scan_trace:
                    ...
                    logging.debug(
                        ...
                        "RE phase 4 scan detail | "

                        + " | ".join(

                            f"ap={a:.2f}||r||2={c:.6g}"

                            for a, c in _p4_scan_trace
                ...
                def _insert_p4_result(
                    ...
                    *,
                    ...
                    b_mc = re_compute_spline_basis_matrix(

                        re_knots_wavelengths(lam_v), wls

                    )

                    env_c = re_envelope_max_delta_n(wls, scale=re_env_s)

                    tk_mc = re_compute_tikhonov_weights(
                    ...
                    rmse_p4_sp = float(
                        ...
                        np.sqrt(max(_report_mse_spectral(ep_f, cor_p4), 0.0))
                    ...
                    _kn4 = np.asarray(
                        ...
                        _re_state["re_p4_beam_knots_lam_nm"], dtype=float
                ...
                if _p4_trf_nfev > 0 and not self._stop:
                    ...
                    def _fun_res_p4(xv):
                        ...
                        return _fun_res_p2(
                            ...
                            xv[:i_ap0], emit_interval=8.0

                        )

                    def _jac_res_p4(xv_full: np.ndarray) -> np.ndarray:

                        _re_state["re_aperture_knots"][:] = xv_full[
                        ...
                        r0 = np.asarray(
                            ...
                            _cb2_ref[0]["res"], dtype=np.float64

                        ).copy()

                        j0 = np.asarray(

                            _cb2_ref[0]["jac"], dtype=np.float64
                        ...
                        for _k in range(_nap):
                            ...
                            _re_state["re_aperture_knots"][:] = xv_p[
                    ...
                    try:
                        ...
                        th_p4 = (
                            ...
                            np.asarray(x_p4[i_cu : i_cu + 3], dtype=np.float64).ravel()
                        ...
                        _ap_i = np.asarray(
                            ...
                            x0_p4[i_ap0 : i_ap0 + _nap], dtype=np.float64

                        ).ravel()

                        _ap_f = np.asarray(

                            x_p4[i_ap0 : i_ap0 + _nap], dtype=np.float64
                        ...
                        _opt = float(
                            ...
                            getattr(res_p4, "optimality", float("nan"))
                        ...
                        logging.info(
                            ...
                            _re_p4_ap_band_intervals_str(
                                ...
                                _kn_log, _ap_i, _wmin_obj, _wmax_obj

                            ),

                            _re_p4_ap_band_intervals_str(

                                _kn_log, _ap_f, _wmin_obj, _wmax_obj
                        ...
                        if _p4_best_seen_rmse is not None and results:
                            ...
                            logging.info(
                                ...
                                "RE phase 4 TRF best tracker | RMSE_best_seen=%.6f | "

                                "RMSE_selected_result=%.6f",
                ...
                logging.info(
                    ...
                    "RE phase 4 skipped | reason=all_angles_below_10deg | "

                    "beam average not applied for low incidence"
        ...
        if results:
            _a_gui = float(
                ...
                self.cfg.get(
                    ...
                    "re_qwot_penalty_weight", RE_GUI_DEFAULT_RE_QWOT_ALPHA
            ...
            logging.info(
                ...
                "RE  final summary (progress readout / e.g. reverse_sample.xlsx) "
    ...
    def _build_re_run_context(self, _re_t0: float):
        ...
        wls, wls_min, wls_max = re_objective_wls_grid(

            self.cfg, oblique_tgts, float_dtype=float_dtype
        ...
        for meta in oblique_config_meta:
            ...
            pos_list = [
                ...
                for b in meta.get("buckets", [])

                if b.get("local_positions", np.array([])).size > 0
            ...
            for bucket in meta.get("buckets", []):
                ...
                if pos.size == 0:
                    ...
                    bucket["idx_union"] = np.searchsorted(pos_all, pos).astype(
        ...
        def _mse_grad_accumulate_ep(
            ...
            n_lay_m, n_sub_m = _re_apply_correc(
                ...
                is_H=is_H, is_L=is_L, wls=wls,

                lambda_ref=lambda_ref, correc=correc,

                re_env_s=re_env_s, env_cache=_re_env_on_wls,
            ...
            grad_raw = np.zeros(n_layers_count, dtype=np.float64)
            ...
            def _accum_from_stats(y_vals, dy_vals, stats, spectral_weights_local):

                nonlocal total_err, total_w, grad_raw, res_list, jac_list
            ...
            for meta in oblique_config_meta:
                ...
                if _re_state["is_phase4"] and angle >= 10.0:

                    _p4_prof = _re_state.get("p4_prof")

                    _t_p4_phy = time.perf_counter() if _p4_prof is not None else None

                    knots_lam = np.asarray(
                    ...
                    if _p4_prof is not None:
                        ...
                        _p4_prof["n_wls_union_max"] = max(

                            int(_p4_prof.get("n_wls_union_max", 0)), nloc
                    ...
                    for m in masks:
                        ...
                        if _p4_prof is not None:
                            ...
                            _p4_prof["band_mask_steps"] = int(

                                _p4_prof.get("band_mask_steps", 0)
                        ...
                        if h <= 0.0:
                            ...
                            yR1, dR1, yT1, dT1 = _re_eval_angle_physics_for(
                                ...
                                sub, angle - h, pl, inc_back, ep_use, n_layers_all, n_sub_all, wls_all, pos_all, var_idx
                    ...
                    yR_all, dR_all, yT_all, dT_all = _re_eval_angle_physics_for(
                        ...
                        None, angle, pl, inc_back, ep_use, n_layers_all, n_sub_all, wls_all, pos_all, var_idx
                ...
                for bucket in meta["buckets"]:
                    ...
                    _accum_from_stats(
                        ...
                        yR_all[idx], dR_all[idx, :], bucket["R"], spectral_weights_local

                    )

                    _accum_from_stats(

                        yT_all[idx], dT_all[idx, :], bucket["T"], spectral_weights_local

                    )

            if return_residuals and correc and correc[0] in (
                ...
                if dh.size >= 3:
                    ...
                    elif correc[0] == "spline_sub3" and len(correc) >= 8 and isinstance(
                    ...
                    if alpha > 0.0:
                        ...
                        if want_grad:
                            ...
                            jac_list.append(np.zeros((len(diff2_h), n_layers_count), dtype=np.float64))
                        ...
                        if want_grad:
                            ...
                            jac_list.append(np.zeros((len(diff2_l), n_layers_count), dtype=np.float64))
            ...
            if return_residuals:
                ...
                if _w_hl > 0.0 and correc and correc[0] in RE_SPLINE_CORREC_KINDS:
                    ...
                    _env_k = np.maximum(
                        ...
                        re_envelope_max_delta_n(_kn, scale=re_env_s),
                    ...
                    if _dH_k.size == _env_k.size and _dL_k.size == _env_k.size:
                        ...
                        if want_grad:
                            ...
                            jac_list.append(
                                ...
                                np.zeros((len(_rh), n_layers_count), dtype=np.float64)

                            )

                            jac_list.append(

                                np.zeros((len(_rl), n_layers_count), dtype=np.float64)
            ...
            if return_residuals:
                ...
                _alpha_qwot = float(_alpha_slot[0])
                ...
                if _alpha_qwot > 0.0:
                    ...
                    _qw_c = (
                        ...
                        if (
                            ...
                            and correc[0] in RE_SPLINE_CORREC_KINDS
                    ...
                    _kq_wl0 = 4.0 / max(float(lambda_ref), 1e-9)

                    _dz_qw = float(

                        self.cfg.get(

                            "re_qwot_deadzone_abs", RE_RE_DEADZONE_QWOT_ABS
            ...
            grad = (2.0 / denom) * grad_raw if want_grad else np.zeros(n_layers_count)
            ...
            if return_residuals:
                ...
                j_out = np.vstack(jac_list) * f2 if jac_list else np.empty((0, n_layers_count), dtype=np.float64)
        ...
        def _rmse_combined(rmse_sp: float, rmse_qwot: float) -> float:
            ...
            return _re_rmse_combined_spectral_qwot(

                float(rmse_sp), float(rmse_qwot), float(_alpha_slot[0])
        ...
        _ap_gui = float(
            ...
            self.cfg.get("re_beam_aperture_deg", RE_GUI_DEFAULT_BEAM_APERTURE_DEG)
        ...
        _re_state = {
            ...
            "re_aperture_knots": np.full(

                int(RE_P4_BEAM_N_KNOTS), _ap_gui, dtype=np.float64

            ),

            "re_p4_beam_knots_lam_nm": np.asarray(

                _p4_beam_knots_lam, dtype=np.float64
        ...
        def _emit_re_spectrum_live(
            ...
            *,
            ...
            if (
                ...
                and _re_live_emit["t"] > 0.0

                and (now_te - _re_live_emit["t"]) < 0.45
            ...
            try:
                ...
                n_lm, n_sm = _re_apply_correc(
                    ...
                    n_lay_disp, n_sub_disp,

                    is_H=is_H, is_L=is_L, wls=wls_display,

                    lambda_ref=lambda_ref, correc=correc,

                    re_env_s=re_env_s, env_cache=_re_env_on_wls_disp,
                ...
                if last_mse is not None and np.isfinite(last_mse):
                    ...
                    _sp_mse = float(
                        ...
                        _mse_grad_accumulate_ep(
                            ...
                            ep_use, wt_spectral, False, correc
                ...
                for meta in oblique_config_meta:
                    ...
                    R_o, T_o = _re_calc_spectrum_for_config(
                        ...
                        beam_aperture_knots_lam_nm=_re_state[

                            "re_p4_beam_knots_lam_nm"
            ...
            except (ValueError, TypeError, RuntimeError, AttributeError, KeyError, IndexError, FileNotFoundError) as _emit_e:
        ...
        n_starts = int(
            ...
            self.cfg.get(
                ...
                "re_phase1_multistarts", RE_GUI_DEFAULT_RE_PHASE1_RESTARTS

            )

        )

        runs = re_phase1_trf_runs_multistart(

            ep0, wt_spectral, lb_ep, ub_ep, n_starts, lhs_seed=42
        ...
        rmse_initial_sp = float(
            ...
            np.sqrt(max(_report_mse_spectral(ep0_u, _correc_nom), 0.0))

        )

        rmse_initial_q = _compute_qwot_rmse(ep0_u, _correc_nom)

        _a_p1, _a_p2a, _a_p2b, _a_p3 = resolve_re_qwot_alphas(

            self.cfg, rmse_initial_sp, rmse_initial_q
        ...
        if _de_mx > 0 and not self._stop:
            ...
            except (ValueError, TypeError, RuntimeError, AttributeError, KeyError, IndexError, FileNotFoundError) as _e_de:
        ...
        _re_top_k_cfg = max(
            ...
            1, int(self.cfg.get("re_phase2_top_k", RE_GUI_DEFAULT_RE_PHASE2_TOP_K))

        )

        _re_n_sh_cfg = max(0, int(self.cfg.get("re_phase3_shake_rounds", 4))) if _any_spl_act else 0

        _re_prefit_max_cfg = int(
        ...
        _maxiter_p1 = max(10, int(self.cfg.get("re_phase1_maxiter", 100)))

        _maxiter_p2b = max(

            10, int(self.cfg.get("re_phase2b_maxiter", RE_PHASE2B_MAXITER))
    ...
    def _finalize_from_context(self, fin) -> None:
        ...
        """Adaptateur de finalisation basé sur le contexte construit en amont."""

        _alpha_rank_ref = float(

            self.cfg.get("re_ranking_alpha_ref", RE_RANKING_ALPHA_REF)
    ...
    def run(self):
        ...
        try:
            ...
            logging.error(f"REWorker error: {e}")

            tb = traceback.format_exc()

            self.signals.error.emit(tb)

            # Without ``finished``, the UI stays on "Computing..." / RE progress bar stuck.

            self.signals.finished.emit(
                REResultsPayloadBuilder.build_error_payload(self.cfg.get("ep0"))
            )
