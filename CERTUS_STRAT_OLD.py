...
# NOISE DISTRIBUTION - Gaussian-only model for Monte-Carlo simulations


# -----------------------------------------------------------------------------


# gaussian: Realistic sensor noise distribution (production mode)


# DESIGN DECISION (2026):


# STRAT runs in gaussian-only mode by product choice.


# We keep the legacy `distribution` argument for backward compatibility with


# older configs/tests, but runtime behavior is intentionally and always gaussian.
...
def _build_layer_importance_map(
    ...
    for i in range(num_layers):
        ...
        dyn_values = [
            ...
            float(c.get("dynamics", 0.0)) for c in layer_items if np.isfinite(c.get("dynamics", 0.0))
...
def generate_noise_array(
    ...
    shape : tuple
        ...
        Shape of the output array (e.g., (num_runs,) or (num_runs, num_layers))

    scale : float

        Scale factor applied to the normalized noise (e.g., noise_pct / 100.0)

    distribution : str

        Legacy parameter kept for backward compatibility.

        Noise is always generated with gaussian distribution.
    ...
    # Gaussian: clip to +/-3sigma for numerical stability.
...
def smart_get_refractive_index(mat_id, wl, db_instance=None):
...
def smart_get_refractive_clues_vectorized(mat_id, wls, db_instance=None):
...
class PlotCache:
    ...
    def __init__(self, _max_size_mb=200):

        self.cache = {}

        self.max_size_items = 20  # Keep last 20 plots

    def get_hash(self, data_obj):
...
def _worker_init(stats_queue, live_queue=None):
...
def get_wl_index(wl: float, all_wls: np.ndarray) -> int:

    idx = np.searchsorted(all_wls, wl)

    if idx < len(all_wls) and np.abs(all_wls[idx] - wl) < 1e-6:

        return idx

    idx_rounded = np.searchsorted(all_wls, round(wl, WL_DECIMALS))

    if idx_rounded < len(all_wls) and np.abs(all_wls[idx_rounded] - wl) < 1e-3:

        return idx_rounded

    raise ValueError(f"Wavelength {wl:.1f} nm not in precomputed cache")
...
def calculate_RT_normal_real(
    ...
    Uses the Macleod Transfer Matrix Method (TMM) with convention:

        - Complex refractive index: n̂ = n - ik  (k > 0 for absorption)

        - Matrix factor: +1j in off-diagonal elements

        - Pre-multiply Air->substrate accumulation order

        - Incoherent backside correction at substrate/air interface

    All arrays are promoted to float64/complex128 before entering the Numba

    vectorized kernel ``calculate_RT_vectorized_real_HL`` to avoid mixed-precision

    recompilation.

    Args:

        wavelengths: 1-D array of wavelengths in nm.

        nH_id:       Material identifier for the high-index layer.

        nL_id:       Material identifier for the low-index layer.

        nSub_id:     Material identifier for the substrate.

        p_thick:     Physical thicknesses in nm (layer 0 = first deposited).

        db_instance: Optional ``MaterialDatabase`` override.

    Returns:

        2-D array (num_wl, 2) - column 0 = R, column 1 = T.
...
def calculate_dynamics_ULTIMATE(
    ...
    The dynamics metric is the **peak-to-peak** range of T(d) over the current

    layer growth: DeltaT = T_max - T_min, where T(d) is computed from d=0 to

    *nominal_thickness* in 1 nm steps. A large DeltaT means the monitoring signal

    is sensitive at lambda_k, which improves cut-off precision. This definition is

    used consistently in ``compute_dynamics_kernel`` and for ranking/filtering.

    Pipeline (all Numba-accelerated, parallel over wavelengths):

        1. ``prepare_dynamics_data_kernel`` - extracts per-lambda refractive index

           and the pre-computed cumulative matrix M_before from the cache.

        2. ``compute_dynamics_kernel`` - sweeps thickness in 1 nm steps,

           computing T at each step via pre-multiply Air->Sub TMM; returns

           dynamics = T_max - T_min, plus t_init and t_final.

    Args:

        scan_wl_range:        Candidate wavelengths to evaluate (nm).

        i_layer:              Current layer index (0-based).

        nominal_thickness:    Target thickness for this layer (nm).

        clues_at_wl:        {lambda: {"H": n̂_H, "L": n̂_L, "substrate": n̂_s}}.

        nominal_matrix_cache: Pre-computed cumulative TMM matrices (from

                              ``precompute_matrix_cache_kernel``).

        all_wls:              Full wavelength grid (superset of scan range).

    Returns:

        List of dicts with keys: ``wl``, ``dynamics`` (peak-to-peak DeltaT),

        ``t_init``, ``t_final``.

    """

    with PERF_MONITOR.measure("calculate_dynamics_ULTIMATE"):
        ...
        # [OPTIMIZATION 2026] Linear preparation for Numba kernel

        # Avoids repeated dictionary lookups in the inner loop

        # Extract clues for all SCAN wavelengths at once

        all_clues = [clues_at_wl[float(wl)] for wl in wls_array]

        n_H_arr = np.array([idx["H"] for idx in all_clues], dtype=np.complex128)

        n_L_arr = np.array([idx["L"] for idx in all_clues], dtype=np.complex128)

        n_Sub_arr = np.array(

            [idx.get("substrate", 1.0) for idx in all_clues], dtype=np.complex128

        )

        # Parallel Data Prep via Numba Kernel
        ...
        # [DIAGNOSTIC LOGGING 2026]

        logger = logging.getLogger("ThinFilm")

        for i, wl in enumerate(wls_array):

            if abs(wl - 455.0) < 0.5:

                logger.info(

                    f"   [DIAG-455-L{i_layer+1}] Tinit={t_inits[i]*100:.2f}%, Tfinal={t_finals[i]*100:.2f}%, Tmin={t_mins[i]*100:.2f}%, Dyn={dyn_vals[i]*100:.2f}%"
        ...
        return [
            ...
            for wl, dyn, ti, tf, tm in zip(

                scan_wl_range, dyn_vals, t_inits, t_finals, t_mins
...
def calculate_nominal_properties(
    ...
    Parses the stack string (e.g. ``"1,1,1,1"``) into quarter-wave

    multipliers, converts them to physical thicknesses using the reference

    wavelength lambda₀ and real part of the refractive clues at lambda₀, then

    evaluates R(lambda) and T(lambda) over the user-specified spectral range.

    The physical thickness of layer *i* with multiplier *m* is::

        d_i = m · lambda₀ / (4 · Re(n_i(lambda₀)))

    Returns:

        (result_dict, multipliers)

        - result_dict contains: wavelengths, R/T spectra, thicknesses, n at lambda₀.

        - multipliers: list of quarter-wave multipliers from the stack string."""

    with PERF_MONITOR.measure("calculate_nominal_properties"):
        ...
        if abs(np.real(nH_at_l0) - 1.0) < 1e-4 and "air" not in str(nH_id).lower():
            ...
            logger.warning(
                ...
                f"⚠️ High Index Material ({nH_id}) appears to be AIR (n=1.0). Missing file?"
        ...
        if abs(np.real(nL_at_l0) - 1.0) < 1e-4 and "air" not in str(nL_id).lower():
            ...
            logger.warning(
                ...
                f"⚠️ Low Index Material ({nL_id}) appears to be AIR (n=1.0). Missing file?"
...
def precompute_clues_and_matrices(
    ...
    if estimated_mb > MAX_CACHE_MB:
        ...
        logger.warning(
            ...
            f"⚠️ Cache limit reached. Scan step adjusted: {req_scan_step:.2f} nm -> {final_step:.2f} nm"
    ...
    all_wls = np.sort(
        ...
        np.unique(np.concatenate([scan_wl_range, wl_range_full, np.array([l0], dtype=np.float64)]))
    ...
    logger.info(
        ...
        f"  Cache allocation: {num_layers * num_wls * 2 * 4 * 8 / (1024**2):.1f} MB ({num_wls} wavelengths)"
    ...
    for wl in all_wls:
        ...
        try:
            ...
            # Correction: Set both keys to avoid any COMMON/STRAT conflict

            clues_at_wl[float(wl)] = {"H": n_h, "L": n_l, "substrate": n_sub}
            ...
            except (ValueError, TypeError, RuntimeError, AttributeError, KeyError, IndexError, FileNotFoundError) as e2:

                # [FIX 2026] Never fall back to n=1.0 (air) - gives physically wrong T=1.0 for bare substrate.

                # Use the raw numeric constants from params if available, else realistic defaults.

                _nH_fb  = float(params.get("nH_r",  2.1))  if "nH_r"  in params else 2.1

                _nL_fb  = float(params.get("nL_r",  1.46)) if "nL_r"  in params else 1.46

                _nSub_fb = float(params.get("nSub_custom", 1.52)) if "nSub_custom" in params else 1.52

                logger.warning(

                    f"⚠️ Also failed at l0 for {wl}nm ({e2}). "

                    f"Using numeric fallback H={_nH_fb}, L={_nL_fb}, Sub={_nSub_fb}. "

                    f"CHECK that your material DB covers the monitoring wavelengths!"

                )

                clues_at_wl[float(wl)] = {"H": _nH_fb, "L": _nL_fb, "substrate": _nSub_fb}

    # =========================================================================

    # BACKSIDE APPROXIMATION VALIDITY CHECK (Opus 4.6)

    # The inconsistent backside formula requires effectively real clues.

    # Warn if materials have significant absorption.

    # =========================================================================

    _backside_warned = False

    # [FIX 2026] Worker IPC compat
    ...
    for wl in all_wls:
        ...
        if not (H_ok and L_ok and Sub_ok) and not _backside_warned:
            ...
            _backside_warned = True  # Warn only once to avoid log spam

            break  # One warning is enough

    # [OPTIMIZATION 2026] Parallel matrix cache via Numba kernel

    # Now supports complex clues for accurate SEEL/Phase B even with absorbing layers.

    all_wls_f64 = all_wls.astype(np.float64)

    n_H_cache = np.array([idx_dict[float(wl)]["H"] for wl in all_wls], dtype=np.complex128)

    n_L_cache = np.array([idx_dict[float(wl)]["L"] for wl in all_wls], dtype=np.complex128)

    p_thick_f64 = np.array(p_thick_nominal, dtype=np.float64)

    nominal_matrix_cache = precompute_matrix_cache_kernel(

        all_wls_f64, n_H_cache, n_L_cache, p_thick_f64, num_layers

    )

    # [DIAGNOSIS] Check clues loaded for 455nm

    for i, wl in enumerate(all_wls):

        if abs(wl - 455.0) < 0.1:

            logger.info(f"   [IDX-DIAG] 455nm: H={n_H_cache[i]:.4f}, L={n_L_cache[i]:.4f}")
...
def _select_candidates_phase_a(
    ...
    Workflow:
        ...
        1. Compute transmittance dynamics (peak-to-peak DeltaT = T_max - T_min)

           for every scan wavelength via ``calculate_dynamics_ULTIMATE``.

        2. Keep only candidates with DeltaT >= *dynamics_threshold* (default 2.5 %).

        3. Drop candidates with *T_min* (min T over full layer growth) < *min_transmission_floor*

           (default 10 %). **Strict rule:** any lambda for which T drops below this floor during the

           layer is excluded (no monitoring below 10 %).

        4. Remove candidates near transmittance extrema (flat T -> poor cut-off

           precision) using ``check_extrema_proximity_batch``.

        5. Verify spectral resolution: the stack must resolve Deltalambda >= 1 nm at

           the candidate wavelength to avoid aliasing.

        6. Always include lambda₀ as a fallback candidate.

    Args:

        scan_wl_range:        Wavelengths to test (nm, float64).

        i_layer:              Layer index (0-based).

        p_thick_nominal:      Nominal thicknesses for all layers.

        clues_at_wl:        Complex index dictionary.

        nominal_matrix_cache: Cumulative TMM cache.

        all_wls:              Full wavelength grid.

        params:               Parameter dictionary.

        l0:                   Reference wavelength lambda₀.

    Returns:

        (filtered_candidates, full_dynamics_map)
    ...
    for d in dynamics:
        ...
        if abs(d["wl"] - 455.0) < 1.0:

            params["logger"].info(

                f"   [DIAG] Dyn 455nm (Layer {i_layer+1}): {d['dynamics']:.4f} (Seuil: {threshold}) | Tmin: {d['t_min']*100:.1f}% | Tinit: {d['t_init']*100:.1f}% | Tfinal: {d['t_final']*100:.1f}%"
    ...
    n_dyn = len(pre_candidates_dyn)

    n_after_tmin = len(pre_candidates)

    # Use reality-feedback stack for resolution when provided (align with dynamics).
    ...
    # --- Vectorized Extrema Check (Optimized 2026 - Complex128) ---
    ...
    is_nominal_mask = np.zeros(n_check, dtype=np.bool_)
    ...
    for i, wl in enumerate(wls_arr):

        idx_data = idx_dict[wl]

        n_curr_arr[i] = idx_data["H" if is_h_layer else "L"]

        n_sub_arr[i] = idx_data["substrate"]

        if i_layer > 0:

            n_prev_arr[i] = idx_data["L" if is_h_layer else "H"]

        if abs(wl - l0) < 0.5:

            is_nominal_mask[i] = True

    if i_layer > 0:

        # Vectorized Matrix Lookup

        clues = np.searchsorted(all_wls, wls_arr)

        clues = np.clip(clues, 0, len(all_wls) - 1)

        for i, idx in enumerate(clues):

            target_wl = wls_arr[i]

            if idx > 0 and abs(all_wls[idx - 1] - target_wl) < abs(all_wls[idx] - target_wl):

                clues[i] = idx - 1

        cache_arr = (

            nominal_matrix_cache.get_array()

            if hasattr(nominal_matrix_cache, "get_array")

            else nominal_matrix_cache

        )

        M_befores = cache_arr[i_layer - 1, clues]
    ...
    exclusion_width = l0 / exclusion_ratio

    # Build wl_changed_arr: True for each candidate whose lambda differs from the

    # previous layer's selected wavelength (triggers wider start margin).
    ...
    # [FIX 2026] Pass complex clues for coherence with M_befores (built from complex cache)
    ...
    for i, cand in enumerate(check_list):

        if is_nominal_mask[i]:

            is_safe_extrema = True

        else:

            is_safe_extrema = extrema_results[i]

        if not is_safe_extrema:

            continue

        wl_val = wls_arr[i]

        # Resolution/Curvature Check

        half_res = res_limit_nm / 2.0

        wls_res_check = [max(0.1, wl_val - half_res), wl_val, wl_val + half_res]

        try:

            RT_check = calculate_RT_normal_real(
                ...
                current_stack_for_res,

                db_instance=params.get("materials_db"),
            ...
            if len(T_vals) == 3:
                ...
                if not all(np.isfinite(t) for t in T_vals):

                    rejected_resolution_count += 1

                    continue

                avg_neighbors = (T_vals[0] + T_vals[2]) / 2.0

                curvature = abs(avg_neighbors - T_vals[1])

                if curvature > resolution_curvature_tolerance:
        ...
        cand_out["ext_prev_start"] = float(dist_ps)

        cand_out["ext_next_start"] = float(dist_ns)

        cand_out["ext_prev_end"] = float(dist_pe)

        cand_out["ext_next_end"] = float(dist_ne)

        valid_candidates_data.append(cand_out)

    # [SURVIVAL LOG]

    # Count effective extrema-safe candidates exactly as processed in the loop

    # (nominal wavelength is always accepted by design).
    ...
    if n_check > 0:
        ...
        n_safe = int(
            ...
            np.sum(np.asarray(extrema_results, dtype=np.bool_) | np.asarray(is_nominal_mask, dtype=np.bool_))

        )

    n_res_ok = int(len(valid_candidates_data))

    n_res_base = n_safe if n_safe > 0 else 0

    params["logger"].info(

        f"   [SURVIVAL L{i_layer+1}] Dyn-OK: {n_dyn} | Tfloor-OK: {n_after_tmin} | Safe-Extrema: {n_safe}/{n_check} | Resolution-OK: {n_res_ok}/{n_res_base}"

    )

    if valid_candidates_data:

        final_list = valid_candidates_data

    else:

        strict_floor = bool(params.get("strict_min_transmission_floor", True))

        if strict_floor and min_t_floor > 0.0:

            if pre_candidates_dyn:

                best_tmin_item = max(pre_candidates_dyn, key=lambda x: float(x.get("t_min", -1.0)))

                best_wl = float(best_tmin_item.get("wl", l0))

                best_tmin = float(best_tmin_item.get("t_min", 0.0))

            else:

                best_tmin_item = max(dynamics, key=lambda x: float(x.get("t_min", -1.0))) if dynamics else {}
            ...
            if n_after_tmin == 0:
                ...
                raise RuntimeError(
                    ...
                    f"Layer {i_layer + 1}: no wavelength satisfies strict floor T_min >= {min_t_floor*100:.1f}% "

                    f"after dynamics filter (dyn-OK={n_dyn}). Best T_min among dyn-OK: lambda={best_wl:.1f} nm, "

                    f"T_min={best_tmin*100:.1f}%."

                )

            # Keep strict T-floor, but avoid hard failure when other filters over-prune.

            # Fallback on Tfloor-OK candidates only (already dynamics-filtered and T_min-safe).

            params["logger"].warning(

                f"Layer {i_layer + 1}: strict floor passed (Tfloor-OK={n_after_tmin}) but "

                f"extrema/resolution removed all candidates. Using Tfloor-OK fallback."

            )

            final_list = check_list if check_list else pre_candidates

            if not final_list:

                raise RuntimeError(

                    f"Layer {i_layer + 1}: internal fallback failed (no Tfloor-OK candidate available)."

                )

            return final_list, full_dyn_map

        # Legacy fallback (non-strict mode only)

        l0_dyn = full_dyn_map.get(float(l0), 0.0)

        final_list = [{"wl": l0, "dynamics": l0_dyn}]
...
def _validate_candidates_phase_a(
    ...
    For each candidate lambda_k, launches *num_runs* parallel growth simulations

    using ``validate_wavelengths_batch`` (Numba kernel).  Each run applies

    a different random noise realisation drawn from

    ε ~ U(-tol, +tol) × noise_pct.

    The cost metric is the P95 of |d_simulated - d_nominal|

    to keep marginal cases in the decision metric.

    After scoring, the best wavelength is selected and

    ``update_run_states_kernel`` propagates the simulated thickness

    errors to the run states for the next layer.

    Args:

        candidates:    Pre-filtered candidates from Step 1.

        i_layer:       Layer index (0-based).

        num_runs:      Number of Monte Carlo realisations.

        p_thick_nominal: Nominal thicknesses.

        clues_at_wl: Complex index dictionary.

        params:        Parameter dictionary.

        run_states:    Per-run cumulative simulation state.

    Returns:

        (results_thickness, p_thick_sim_updates)
    ...
    # [FIX 2026] Use complex clues for coherence with Phase B (simulate_stack_robustness_batch)
    ...
    n_H_arr = np.array([idx_dict[w]["H"] for w in candidate_wls], dtype=np.complex128)

    n_L_arr = np.array([idx_dict[w]["L"] for w in candidate_wls], dtype=np.complex128)

    n_Sub_arr = np.array([idx_dict[w]["substrate"] for w in candidate_wls], dtype=np.complex128)
    ...
    if layer_noise_array is not None:
        ...
        # Fallback for anciens tests (gaussian-only policy)
    ...
    for idx, wl in enumerate(candidate_wls):
        ...
        # [PERSIST DYNAMICS] Carry over dynamics from candidates for Phase B logging

        results_thickness.append(

            {
                ...
                "dynamics": candidates[idx].get("dynamics", 0.0),

                "ext_prev_start": float(candidates[idx].get("ext_prev_start", SYM_MISSING_DISTANCE)),

                "ext_next_start": float(candidates[idx].get("ext_next_start", SYM_MISSING_DISTANCE)),

                "ext_prev_end": float(candidates[idx].get("ext_prev_end", SYM_MISSING_DISTANCE)),

                "ext_next_end": float(candidates[idx].get("ext_next_end", SYM_MISSING_DISTANCE)),
    ...
    # Generate Sim Data for Next Layer

    best_wl = float(results_thickness[0]["wl"])

    best_idx_data = idx_dict[best_wl]

    # Keep identical run-wise noise between validation and state propagation

    # to preserve Phase A Monte Carlo coherence.

    final_noise = noise_values
    ...
    **Phase B - Block Grouping & Robustness**
    ...
    try:
        ...
        scan_wl_range = arange_inclusive(
            ...
            params["scan_wl_min"], params["scan_wl_max"], params["scan_wl_step"]
        ...
        )  # wl/cost/std_dev are real floats

        full_dynamics_grid: dict[int, dict[float, float]] = (

            {}

        )  # [NEW 2026] For real-time monitoring
        ...
        # [OPTIMIZATION] Monte-Carlo Pre-allocation

        try:

            noise_tol = (

                float(params.get("reality_sim_params", {}).get("trigger_tolerance", 0.1)) / 100.0
        ...
        for i_layer in range(num_layers):
            ...
            # 1. Update Matrix Cache with REAL (Average) thicknesses from previous layers

            # This ensures Dynamics are calculated on the actual shifted mirror, not the theoretical nominal.
            ...
            if i_layer > 0:
                ...
                for j in range(i_layer):
                    ...
                    # Average simulated thickness for layer j

                    avg_t = np.mean([run["p_thick_sim"][j] for run in run_states])

                    current_avg_stack.append(avg_t)

            # Recalculate M_before for all scan wavelengths based on reality

            n_H_scan = np.array(

                [idx_dict_opt[float(wl)]["H"] for wl in scan_wl_range],
            ...
            n_L_scan = np.array(
                ...
                [idx_dict_opt[float(wl)]["L"] for wl in scan_wl_range],
            ...
            np.array(
                ...
                [idx_dict_opt[float(wl)]["substrate"] for wl in scan_wl_range],

                dtype=np.complex128,

            )

            # We use a temporary cache for this layer's dynamics

            # prepare_dynamics_data_kernel will use these matrices
            ...
            # 2. Select Candidates using the Updated Visual Reality

            # Pass previous layer's best wavelength so the start check can be asymmetric
                ...
                if not nucleation_active:
                    ...
                    if 0.0 < attenuation_factor < 1.0:
                        ...
                        logger.info(
                            ...
                            f"   -> Layer 1: Costs attenuated by factor {attenuation_factor}."
        ...
        raw_results_sq: dict[int, list[dict[str, float]]] = {}  # cost values are real floats
        ...
        for i in range(num_layers):
            ...
            for item in t_data:
                ...
                item["cost"] = item["cost"] / mean_thickness
        ...
        result_phase_a["sym_bonus_map"] = (
            ...
            _build_symmetry_bonus_map(raw_results_thickness, num_layers, sym_window_ot)
        ...
        result_phase_a["sym_layer_importance"] = (
            ...
            _build_layer_importance_map(raw_results_thickness, num_layers)
        ...
        for n_blk in blocks_range:
            ...
            strats = mine_strategies_for_block_count(
                ...
                sym_same_wl_bonus=float(

                    params.get("sym_same_wl_bonus", SYM_DEFAULT_SAME_WL_BONUS)

                ),

                sym_continuity_weight=float(

                    params.get("sym_continuity_weight", SYM_DEFAULT_CONTINUITY_WEIGHT)
            ...
            if tmin_violations:
                ...
                sample = ", ".join(
                    ...
                    [
                        ...
                        f"L{int(v['layer'])}@{v['wl']:.1f}nm:{v['t_min']*100:.2f}%"

                        for v in tmin_violations[:5]
...
def _convert_solution_to_strategy(sol, num_layers, n_blocks, origin_tag, s_id):
...
def mine_strategies_for_block_count(
    ...
    *top_k* strategies ranked by total cost.
    ...
    def apply_nucleation_constraint(cost_map_in):
    ...
    logging.getLogger("ThinFilm").info(
        ...
        f"Mining Mining: n_blocks={n_blocks}, CostMapThick Size={len(cost_map_thick)}, CostMapSq Size={len(cost_map_sq)}"

    )

    def run_mining(cost_map, origin_name, offset_id, apply_sym_post=False):

        logger = logging.getLogger("ThinFilm")

        logger.info(

            f"[DEBUG MINING] {origin_name}: Starting DP with {len(cost_map)} layers, n_blocks={n_blocks}"
        ...
        if solutions:
            ...
            for rank, sol in enumerate(solutions):
                ...
                is_valid, reason = _validate_strategy_blocks_contract(

                    strat, num_layers, expected_n_blocks=n_blocks
                ...
                if not is_valid:
                    ...
                    logger.warning(
                        ...
                        f"[DEBUG MINING] Dropped invalid strategy {s_id} ({origin_name}): {reason}"
...
def _get_best_noise_results(

    final_results: dict[str, Any], logger: logging.Logger
...
def _build_layer_wavelengths_from_strategy(

    strategy: dict[str, Any], num_layers: int, l0: float
...
def run_final_simulation_block(
    ...
    if noise_levels is None:
        ...
        if params.get("thickness_tolerance_nm") is not None:
            ...
            noise_levels = [base_tol * f for f in noise_factors]  # Absolute nm values

        else:

            # Fallback: try to get from reality_sim_params, else use trigger_tolerance directly
            ...
            try:
                ...
                base_noise = float(
                    ...
                    params.get("trigger_tolerance", 0.5)

                )  # Fallback to param or default

            noise_factors = params.get("robustness_noise_factors", [0.5, 1.0, 2.0])

            noise_levels = [base_noise * f for f in noise_factors]  # Percentages
    ...
    _, T_nom = calculate_RT_vectorized_real_HL(

        wl_arr, nH_arr, nL_arr, nSub_arr, p_thick_nom_arr

    )

    params_safe = {

        k: v for k, v in params.items() if k not in ["logger", "materials_db", "gui_parent"]
    ...
    # --- SEQUENTIAL MODE (Safe for Frozen or Single Core) ---
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
            ...
            for f in concurrent.futures.as_completed(futures):
                ...
                try:
                    ...
                    traceback.print_exc()  # Force print to stderr
    ...
    strategies_results = filtered_results
    ...
    # Keep unique order and enforce configured seed budget (important in fast mode).
    ...
    def _apply_consensus_ranking_inplace(results_in: list[dict[str, Any]], stage_tag: str) -> None:
        ...
        if not (consensus_enabled and len(consensus_seeds) > 1 and results_in):
            ...
            for seed in consensus_seeds:
                ...
                try:
                    ...
                    logger.warning(
                        ...
                        f"[ROBUSTNESS] Consensus re-score failed for strategy {sid} seed={seed}: {e}"
    ...
    _apply_consensus_ranking_inplace(strategies_results, "pre-elite")
    ...
    sym_tie_epsilon_rel = max(

        float(params.get("sym_tie_epsilon_rel", SYM_DEFAULT_TIE_EPS_REL)), 0.0
        ...
        while i < len(strategies_results):
            ...
            tie_group.sort(
                ...
                key=lambda item: (
                    ...
                    # Prefer any SYM family (SYM, SMART_MERGE_SYM, ...)

                    0

                    if "SYM"

                    in str(item.get("strategy", {}).get("origin", "")).upper()
    ...
    if enable_family_diversity and len(strategies_results) > 1:
        ...
        strategies_results = _apply_family_diversity(

            strategies_results, diversity_top_k, diversity_max_per_family
    ...
    # Optional ELITE refinement (multi-round):

    # local neighborhood search around top-ranked solutions, kept only if

    # nominal-noise RMSE P95 is better than current rank #10 nominal baseline.
    ...
    if strategies_results and noise_levels:
        ...
        for elite_round in range(1, elite_rounds + 1):
            ...
            nominal_threshold = _extract_rmse_p95_for_noise(

                strategies_results[top10_idx], nominal_noise_level
            ...
            existing_signatures = {
                ...
                _blocks_signature(item.get("strategy", {}).get("blocks", []))
            ...
            # Stage-1 ELITE pruning: quick nominal pass, keep only best M for full MC.
            ...
            logger.info(
                ...
                f"[ELITE] Round {elite_round}: quick_pass={len(quick_pass)}, "

                f"full_eval={len(full_eval_candidates)}."
            ...
            for _quick_nominal, e_idx, elite_strat in full_eval_candidates:
                ...
                try:
                    ...
                    min_res, bad_layer = _calculate_strategy_spectral_resolution(
                        ...
                        full_res["strategy"], p_thick_nominal, params
            ...
            logger.info(
                ...
                f"[ELITE] Round {elite_round}: added {len(elite_added)} strategy(ies) above threshold."

            )

            strategies_results.extend(elite_added)

            # Re-apply consensus so ELITE and base designs follow identical ranking criteria.

            _apply_consensus_ranking_inplace(strategies_results, f"post-elite-r{elite_round}")
                ...
                while i < len(strategies_results):
                    ...
                    tie_group.sort(
                        ...
                        key=lambda item: (
                            ...
                            if "SYM" in str(item.get("strategy", {}).get("origin", "")).upper()
    ...
    # Keep full MC payload (rmse_all, thicknesses_all) only for top-K strategies to reduce memory.
    ...
    return {
        ...
        "results_per_noise": (
            ...
            strategies_results[0]["results_per_noise"] if strategies_results else []

        ),

        "optimal_blocks": (

            strategies_results[0]["strategy"]["blocks"] if strategies_results else []
...
def _extract_local_extrema_points(

    d_vals: np.ndarray, t_vals: np.ndarray, eps: float = 1e-10
...
def calculate_sensitivity_matrix(
    ...
    with PERF_MONITOR.measure("calculate_sensitivity_matrix"):
        ...
        local_db = params.get("materials_db") or APP_CONTEXT.get("materials_db")
        ...
        valid_idxs = [i for i, s in enumerate(sigma_steps) if s > 0]

        valid_sigmas = [sigma_steps[i] for i in valid_idxs]
        ...
        p_bulk = np.zeros(
            ...
            (len(valid_sigmas), runs_per_step, len(p_thick_nominal)), dtype=np.float64
        ...
        for idx, sigma in enumerate(valid_sigmas):
            ...
            noise = rng.normal(0.0, sigma, (runs_per_step, len(p_thick_nominal)))

            p_bulk[idx] = np.maximum(0.0, p_thick_arr + noise)

        p_flat = p_bulk.reshape(total_runs, -1)

        _, batch_T_flat = calculate_RT_batch_kernel(

            wavelengths, nH_arr, nL_arr, nSub_arr, p_flat
        ...
        for i, sigma in enumerate(sigma_steps):
            ...
            sensitivity_grid[i, :] = mean_delta

            closest_target = min(target_sigmas, key=lambda x: abs(x - sigma))

            if abs(closest_target - sigma) < 0.05:

                envelopes[closest_target] = {
...
def calculate_seel_analysis(
    ...
    with PERF_MONITOR.measure("calculate_Statistical Equivalent Error per Layer_analysis"):
        ...
        np.array(nominal_results["T_spectral_nominal"], dtype=np.float64)

        # Use exact same IDs as Nominal calculationation (Double Precision)
        ...
        # ── FULL NUMBA VECTORIZATION WITHOUT MULTIPROCESSING ──

        total_runs = len(target_sigmas) * batches_per_sigma * runs_per_batch

        _emit_stat("SP", total_runs)  # Report total runs to UI
        ...
        for i, sigma in enumerate(target_sigmas):
            ...
            for b in range(batches_per_sigma):
                ...
                noise = rng.normal(0.0, sigma, (runs_per_batch, len(p_thick_nominal)))
        ...
        batch_T_bulk = batch_T_flat.reshape(

            (len(target_sigmas), batches_per_sigma, runs_per_batch, len(wavelengths))
        ...
        # Per-sigma P95/P99 for alignment with Phase B scoring
        ...
        # Calculate averages per sigma for the fit (correct approach)
        ...
        for sigma in target_sigmas:
            ...
            global_avg = np.mean(batch_means)

            dispersion = np.std(batch_means)

            logger.info(

                f"   -> SEEL={sigma}nm : RMSE~={global_avg:.5f} (Dispersion +/-{dispersion:.5f})"

            )

            # Store averages for fitting

            sigma_averages.append(global_avg)
        ...
        try:
            ...
            if np.any(valid_idx):
                ...
                # Linear fit passing through origin: y = k * x

                fit_k = np.sum(x * y) / np.sum(x * x)

                fit_alpha = 1.0

                logger.info(f"   -> SEEL Fit: sigma = {fit_k:.2e} * RMSE")
            ...
            logger.warning(f"SEEL Fit failed: {e}")
    ...
    with PERF_MONITOR.measure("generate_excel_report"):
        ...
        with pd.ExcelWriter(output, engine=engine) as writer:
            ...
            if robustness_data:
                ...
                pd.DataFrame(robustness_data).to_excel(

                    writer, sheet_name="Robustness Best Strategy", index=False
...
class StrategiesTableWindow(QMainWindow):
    ...
    def __init__(
        ...
        if Path(svg_path).exists() and QSvgWidget:
            ...
            lbl_fallback.setStyleSheet(

                f"color: {CertusTheme.PRIMARY}; font-weight: bold; font-size: 14px;"
        ...
        self.origin_summary_label.setStyleSheet(
            ...
            "font-size: 12px; color: #4a5568; padding: 2px 10px 8px 10px;"
        ...
        export_btn.setToolTip(
            ...
            "Export the full strategies table to a CSV file (all rows and columns)."
    ...
    def _calculate_worst_layers(self, strategy_result, top_k=10):
    ...
    def update_data(self, strategies_results: list[dict[str, Any]]):
        ...
        try:
            ...
            if origin_counts:
                ...
                self.origin_summary_label.setText(

                    f"Origins ({len(strategies_results)}): {summary}"
            ...
            for row, result in enumerate(strategies_results):
                ...
                self.table.setItem(
                    ...
                    NumericTableWidgetItem(
                        ...
                        str(th_rank) if th_rank is not None else "N/A (non calcule)"
                ...
                self.table.setItem(
                    ...
                    NumericTableWidgetItem(
                        ...
                        str(sp_rank) if sp_rank is not None else "N/A (non calcule)"
                ...
                self.table.setItem(
                    ...
                    NumericTableWidgetItem(str(result["num_unique_wavelengths"])),
                ...
                if sym_score is None:
                    ...
                    sym_score = _compute_strategy_symmetry_score_percent(
                        ...
                        strat.get("theoretical_layer_profile", []),
                ...
                sym_item.setToolTip(
                    ...
                    "Layer-by-layer symmetry score (0-100). "

                    "100 = perfect symmetry across the entire stack."
                ...
                for col_idx, noise_idx in enumerate([0, 1, 2]):
                    ...
                    if noise_idx < len(noise_results):
                        ...
                        rmse_val = noise_results[noise_idx].get(

                            "rmse_p95", noise_results[noise_idx]["rmse_mean"]
                        ...
                        if seel_val is not None:
                            ...
                            self.table.setItem(
                                ...
                                NumericTableWidgetItem(f"R:{rmse_val:.5f}"),
    ...
    def on_cell_clicked(self, row: int, col: int):
        ...
        if strategy_result:
            ...
            if extrema_distances or theo_profile:
                ...
                dlg.finished.connect(lambda *_: self._details_dialogs.remove(dlg) if dlg in self._details_dialogs else None)
    ...
    def export_csv(self, strategies_results, max_blocks):
            ...
            try:
                ...
                for rank, result in enumerate(strategies_results, 1):
                        ...
                        try:
                            ...
                            if th_data:
                                ...
                                diffs = np.abs(
                                    ...
                                    mat_sim[:, :limit_l] - self.p_thick_nominal[:limit_l]
                                ...
                                if seel_data and "fit_alpha" in seel_data:
                                    ...
                                    seel_val = seel_data["fit_k"] * (

                                        rmse_val ** seel_data["fit_alpha"]
                            ...
                            # Skip if calculationation fails
                    ...
                    for idx, noise_res in enumerate(noise_results):
                        ...
                        row[f"Noise_{idx}_RMSE_P95"] = (

                            f"{noise_res.get('rmse_p95', noise_res['rmse_mean']):.6f}"
...
def validate_material_coverage(params: dict[str, Any]):

    logger = params.get("logger")

    db = APP_CONTEXT.get("materials_db")

    if db is None or not db.data:
    ...
    used_materials = {params["nH_id"], params["nL_id"], params["nSub_id"]}
    ...
    for mat_name in files_to_check:
        ...
        valid_min = mat_data.get("min_wl_valid", 0.0)

        valid_max = mat_data.get("max_wl_valid", 99999.0)
...
def _estimate_fusion_cost_fast(b1, b2, cost_map_sq):
...
def _find_best_wl_fast(start_layer, end_layer, cost_map_sq):
        ...
        for fusion in best_fusions:
            ...
            for wl in sorted(candidate_wls):
                ...
                if sig not in seen_signatures:
                    ...
                    new_id = int(parent_strat["strategy_id"]) + 200000 + (i * 100) + int(wl)
                    ...
                    derived_strategies.append(

                        {
                            ...
                            "origin_details": (
                                ...
                                f"{merge_origin} from {parent_origin} "

                                f"(parent ID {parent_strat['strategy_id']})"

                            ),

                            "avg_rmse_nominal": 0.0,

                            "num_unique_wavelengths": len(

                                set(b["wavelength"] for b in new_blocks_struct)
...
def find_robust_nucleation_wavelength_adaptive(
    ...
    candidates_to_test = np.array(

        [wl for wl in valid_candidates if wl in idx_dict], dtype=np.float64
    ...
    def score_block(item):

        return (item["size"] ** 1.5) / (item["rmse"] + 0.1)
...
def _parallel_block_worker(args):
    ...
    try:
        ...
        logger.debug(
            ...
            f"[W{n_blk}] raw_results_sq keys: {list(pre_calc_data['raw_results_sq'].keys())[:5]}..."
        ...
        sample_layer = (
            ...
            list(pre_calc_data["raw_results_thickness"].keys())[0]

            if pre_calc_data["raw_results_thickness"]
        ...
        logger.debug(
            ...
            f"[W{n_blk}] mine_strategies_for_block_count returned {len(strategies_dp)} strategies"
        ...
        if strategies_dp:
            ...
            logger.info(
                ...
                f"   [Block {n_blk}] Running screening on {len(strategies_dp)} strategies..."
            ...
            if "all_strategies_results" in res_dp:
                ...
                logger.info(
                    ...
                    f"   [Block {n_blk}] Screening complete. Results count: {len(results_list)}"
        ...
        if inherited_strategies:
            ...
            for s in inherited_strategies:
                ...
                ok, _ = _validate_strategy_blocks_contract(
                    ...
                    s, pre_calc_data["num_layers"], expected_n_blocks=n_blk
        ...
        logger.error(
            ...
            f"Critical error in parallel worker for block {n_blk}: {traceback.format_exc()}"
...
class WorkerThread(QThread):
    ...
    def __init__(
        ...
        self.opti_results = opti_results
    ...
    def run(self):
        ...
        try:
            ...
            if self.step in [0, 2, 3, 23, 33]:

                validate_material_coverage(self.params)
    ...
    def _run_step_0(self):
        ...
        - Nominal property calculationation for the design
        ...
        nominal_results, multipliers = calculate_nominal_properties(self.params)

        self.params["logger"].info("   -> Running Sensitivity Matrix Calculation...")

        sensitivity_data = calculate_sensitivity_matrix(self.params, nominal_results)

        seel_data = calculate_seel_analysis(self.params, nominal_results)
        ...
        self.signals.finished.emit({"nominal_results": nominal_results, "seel_data": seel_data})

    def _run_step_0_auto(self):
        ...
        try:
            ...
            nH_curve = get_refractive_clues_vectorized(

                self.params["nH_id"], wl_clues, db_instance=local_db

            )

            nL_curve = get_refractive_clues_vectorized(

                self.params["nL_id"], wl_clues, db_instance=local_db
    ...
    def _run_step_2(self):
        ...
        - Performance metrics calculationation and analysis
        ...
        opti_results = optimize_block_strategy_hybrid(

            self.params, self.signals.progress, self.signals.plot
        ...
        self.signals.finished.emit({"opti_results": opti_results})
    ...
    def _run_step_3(self):
        ...
        if self.params.get("export_excel", True):
            ...
            excel_data = generate_excel_report(
                ...
                nominal_results, self.opti_results, final_results, self.params
        ...
        if self.params.get("show_plots", True):
            ...
            nH_arr = get_refractive_clues_vectorized(
                ...
                self.params["nH_id"], wavelengths, db_instance=local_db

            ).astype(np.complex128)

            nL_arr = get_refractive_clues_vectorized(

                self.params["nL_id"], wavelengths, db_instance=local_db
        ...
        self.signals.finished.emit({"final_results": final_results})

    def _run_step_23_full(self):
        ...
        Notes:
            ...
            - Handles deep exploration mode with extensive search
        ...
        stats_thread = threading.Thread(
            ...
            target=self._stats_consumer_loop, args=(stats_queue,), daemon=True
        ...
        try:
            ...
            nominal_res, _ = calculate_nominal_properties(self.params)

            p_thick_nom = nominal_res["physical_thicknesses_nominal"]

            self.params["logger"].info("🚀 PHASE 0: Initializing Smart Nucleation Analysis...")

            clues_cache, _, _ = precompute_clues_and_matrices(

                self.params, p_thick_nom, self.params["logger"]

            )

            max_scan_size = min(12, max(2, len(p_thick_nom) // 2))

            mc_runs_nucl = int(self.params.get("nucleation_mc_runs", 40))

            # PHASE 0: Smart Nucleation Analysis (Re-enabled after Matrix Fix)

            nucleation_wl, nucleation_size = (

                (float(self.params["l0"]), 0)
            ...
            # Pass the true nominal thicknesses to Phase A

            self.params["p_thick_nominal"] = p_thick_nom

            pre_calc_data = optimize_block_strategy_hybrid(

                self.params, self.signals.progress, None, phase_a_only=True
            ...
            if self.params.get("show_plots", True) and "raw_results_thickness" in pre_calc_data:

                self.signals.plot.emit(pre_calc_data["raw_results_thickness"], "pyqtgraph_heatmap")

            gc.collect()

            num_layers = pre_calc_data["num_layers"]

            blocks_range = _compute_blocks_range_for_params(

                num_layers, self.params, dense=True
            ...
            self.params["logger"].info(
                ...
                f"🔄 PHASE B: Deep Exploration ({len(blocks_range)} steps) - HYBRID ENGINE..."
            ...
            with SharedIndicesManager(

                pre_calc_data["clues_at_wl"]

            ) as shm_manager, SharedArrayManager(

                pre_calc_data["nominal_matrix_cache"]
                ...
                def monitor_live_feed():
                    ...
                    LIVE_REFRESH_INTERVAL = 2.0  # Refresh the display of the best result every 2 s
                    ...
                    while True:
                        ...
                        try:
                            ...
                            # Periodic refresh of the best result (every 2 s)
                ...
                try:
                        ...
                        for i, n_blk in enumerate(blocks_range):
                            ...
                            if stop_check():
                                ...
                                self.params["logger"].warning(
                                    ...
                                    f"🛑 Stopping Optimization at {n_blk} blocks..."
                            ...
                            try:
                                ...
                                if (
                                    ...
                                    "error" in result_batch

                                    and result_batch.get("strategies_results") == []

                                ):

                                    self.params["logger"].error(

                                        f"    [Block {n_blk}] ❌ Error: {result_batch['error']}"
                                    ...
                                    strategies_this_step_raw = result_batch.get(
                                    ...
                                    # [DYNAMICS TRACKING] Calculate min dynamics across ALL layers using full grid
                                            ...
                                            for blk in s_res["strategy"].get("blocks", []):
                                                ...
                                                block_layers = int(
                                                    ...
                                                    blk.get(
                                                        ...
                                                        int(blk.get("end", 0)) - int(blk.get("start", 0)),
                                                ...
                                                wl_per_layer.extend(
                                                    ...
                                                    [blk["wavelength"]] * block_layers

                                                )

                                            for li, wl in enumerate(wl_per_layer):

                                                # Ignore layer 1 for Min Dyn reporting:

                                                # start of first layer is often near an extremum by construction.
                                    ...
                                    if strategies_this_step:
                                        ...
                                        inherited_strategies = derive_strategies_exhaustive(
                                            ...
                                            max_fusions_per_parent=int(

                                                self.params.get("max_fusions_per_parent", 5)
                                ...
                                self.params["logger"].error(
                                    ...
                                    f"   [Block {n_blk}] ❌ Process pool broken: {e}"
                    ...
                    # Guaranteed cleanup even if an error occurs
                    ...
                    if executor is not None:
                        ...
                        try:
                            ...
                            self.params["logger"].warning(f"Error shutting down executor: {e}")
                ...
                if self.params.get("show_plots", True):
                    ...
                    try:
                        ...
                        self.params["logger"].warning(f"Plotting error: {e}")

                    best_noise_results = _get_best_noise_results(best_res, self.params["logger"])
                    ...
                    if best_noise_results:
                        ...
                        nH_arr = get_refractive_clues_vectorized(
                            ...
                            self.params["nH_id"], wl_arr, db_instance=local_db

                        ).astype(np.complex128)

                        nL_arr = get_refractive_clues_vectorized(

                            self.params["nL_id"], wl_arr, db_instance=local_db

                        ).astype(np.complex128)

                        nSub_arr = get_refractive_clues_vectorized(

                            self.params["nSub_id"], wl_arr, db_instance=local_db
                        ...
                        _, T_clean_batch = calculate_RT_batch_kernel(
                            ...
                            np.array(pre_calc_data["p_thick_nominal"], dtype=np.float64).reshape(
                ...
                if self.params.get("export_excel", True):
                    ...
                    excel_data = generate_excel_report(
                        ...
                        nominal_res, sim_context, final_complete_structure, self.params

                    )

                    # Build metadata for HTML report generation

                    best_rmse = 0.0

                    if (

                        "all_strategies_results" in final_complete_structure

                        and final_complete_structure["all_strategies_results"]

                    ):

                        best_rmse = final_complete_structure["all_strategies_results"][0].get(
                    ...
                    metadata = {
                        ...
                        "strategies_count": len(
                            ...
                            final_complete_structure.get("all_strategies_results", [])
    ...
    def _stats_consumer_loop(self, stats_queue):
    ...
    def _run_step_external_strategies(self):
        ...
        self.params["logger"].info(
            ...
            f"🚀 Simulating {len(loaded_strategies)} external strategies ({num_runs} runs)..."
        ...
        if self.params.get("export_excel", True):
            ...
            excel_data = generate_excel_report(
                ...
                nominal_results, sim_context, final_results, self.params
...
class CertusScientificPlot(pg.PlotWidget):
    ...
    def __init__(self, parent=None, title="", y_label="", x_label=""):
        ...
        self.proxy = pg.SignalProxy(
            ...
            self.scene().sigMouseMoved, rateLimit=60, slot=self.on_mouse_move
    ...
    def add_curve_for_tracking(self, curve_item, name):
    ...
    def get_toolbar(self, parent_widget):
        ...
        act_reset.triggered.connect(lambda: self.plotItem.autoRange())
...
class UniversalPlotWindow(QMainWindow):
    ...
    def __init__(self, parent, data_obj, plot_type):
    ...
    def _init_stack_plot(self, data):
    ...
    def _init_sensitivity_plot(self, data):
    ...
    def _init_seel_plot(self, data):
        ...
        def generate_custom_log_ticks(min_val, max_val):
    ...
    def _init_strategies_plot(self, data):
        ...
        if strategies:
            ...
            for idx, res in enumerate(strategies):
                ...
                scatter = pg.ScatterPlotItem(
                    ...
                    x=x_vals, y=y_vals, size=8, brush=pg.mkBrush(col), pen=pg.mkPen("k")
    ...
    def _init_robustness_plot(self, data):
        ...
        plot = CertusScientificPlot(
            ...
            self, "Monte Carlo Distribution", "Transmission", "Wavelength (nm)"
...
class InteractiveHeatmapWindow(QWidget):  # <--- Changement ici: QWidget au lieu de QMainWindow
    ...
    def __init__(self, parent, raw_data_thickness):
        ...
        if path_x:
            ...
            self.plot_widget.plot(
                ...
                step_x, step_y, pen=pg.mkPen("c", width=3), name="Optimal Strategy"
...
class TransmissionVsThicknessWindow(QMainWindow):
    ...
    def __init__(self, parent, strategy_result, opti_results, params):
        ...
        self.plot_widget.plotItem.addLegend(offset=(30, 30)).setBrush(
    ...
    def _draw_complete_graph(self, strategy_result, opti_results, params):
            ...
            for l in range(block["start"], block["end"]):
                ...
                if l < len(ext_dists):
                    ...
                    txt_ext = pg.TextItem(
                        ...
                        color=(max(color_s[0], color_e[0]),

                               min(color_s[1], color_e[1]),

                               min(color_s[2], color_e[2])),
    ...
    def _extract_layer_errors(self, strategy_result, p_thick_nominal):
...
class StrategySpectralPerformanceWindow(QMainWindow):
    ...
    def _calculate_and_plot(self, strategy_result, opti_results, params):
        ...
        try:
            ...
            nH_arr = get_refractive_clues_vectorized(nH_id, wls, db_instance).astype(

                np.complex128

            )

            nL_arr = get_refractive_clues_vectorized(nL_id, wls, db_instance).astype(

                np.complex128

            )

            nSub_arr = get_refractive_clues_vectorized(nSub_id, wls, db_instance).astype(
            ...
            for p_sim in thicknesses_all:
                ...
                if len(p_sim) == len(p_thick_nominal):
                    ...
                    _, T_val_batch = calculate_RT_batch_kernel(

                        wls, nH_arr, nL_arr, nSub_arr, p_arr
...
class JsonViewerWindow(QMainWindow):
    ...
    def __init__(self, parent, title: str, data: Any):
...
def compute_probe_offset_nm_from_ratio(params: dict[str, Any]) -> float:
    ...
    # NEW LOGIC: If absolute thickness tolerance is defined, use it directly
    ...
    try:
        ...
        n_avg_at_l0 = np.real((nH_at_l0 + nL_at_l0) / 2.0)  # Extract real part for complex clues
...
class InteractiveIndicesWindow(QMainWindow):
    ...
    def __init__(self, parent, data_dict):
        ...
        header.setStyleSheet(
            ...
            f"background: {CertusTheme.BACKGROUND}; border-bottom: 1px solid {CertusTheme.BORDER};"
        ...
        self.plot_widget = CertusScientificPlot(
            ...
            self, title="", x_label="Wavelength (nm)", y_label="Refractive Index"
...
class InteractiveSpectrumWindow(QMainWindow):
    ...
    def __init__(self, parent, data_dict, sigma=None):
        ...
        def add_legend_item(color, text):
            ...
            lbl_txt.setStyleSheet(
                ...
                f"color: {CertusTheme.TEXT_SUB}; font-size: 11px; font-weight: bold;"
        ...
        self.plot_widget = CertusScientificPlot(
            ...
            self, title="", x_label="Wavelength (nm)", y_label="Transmission"
    ...
    def _plot_data(self):
        ...
        curve_nom = self.plot_widget.plot(

            self.wavelengths, self.T_nominal, pen=pg.mkPen(color="#d62728", width=3)
    ...
    def _add_corridor(self, mean, std, factor, color_tuple):
...
class PopOutWindow(QMainWindow):
    ...
    def __init__(self, widget_to_host, parent=None, title="Detached Window"):
        ...
        self.setStyleSheet(
            ...
            f"QMainWindow {{ background-color: {CertusTheme.SURFACE}; }} QWidget {{ font-size: 10pt; }}"
...
class LiveMonitorWindow(QMainWindow):
    ...
    def update_monitor(self, x, y, bounds, info_text, strategy_blocks):
                ...
                if start_layer < len(bounds) and end_layer < len(bounds):
                    ...
                    if i < len(strategy_blocks) - 1:
                        ...
                        sep_line = pg.InfiniteLine(
                            ...
                            pos=x_end, angle=90, pen=pg.mkPen(color="#ef4444", width=2)
...
class WelcomeGuideWidget(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        for i, (icon, text) in enumerate(points):
            ...
            lbl_txt.setStyleSheet(
                ...
                f"color: {CertusTheme.TEXT_MAIN}; font-size: 13px; background: transparent;"
        ...
        try:
            ...
            mat_count = (
                ...
                len(APP_CONTEXT.get("materials_db").data) if APP_CONTEXT.get("materials_db") else 0
        ...
        sys_layout.addLayout(

            self._create_status_row("⚡", "HPC Active", f"<b>{cpu_count} Threads</b>")

        )

        sys_layout.addLayout(

            self._create_status_row("📚", "Database", f"<b>{mat_count} Materials</b>")
        ...
        cap_layout.addLayout(

            self._create_status_row("✓", "Hybrid Exploration", "DP + hybridization")
    ...
    def _create_step_card(self, number, title, desc, accent_color):
    ...
    def _create_status_row(self, icon, label, value):
...
class PlotRenderWorker(QObject):
    ...
    def __init__(self, fig, width, height, plot_hash):
    ...
    def run(self):
        ...
        try:
            ...
            img_bytes = self.fig.to_image(
                ...
                format="png", scale=2.0, width=self.width, height=self.height
...
class CertusStratApp(CertusBaseApp):
    ...
    def _init_global_shortcuts(self):
        ...
        run_opti.activated.connect(

            lambda: self.run_workflow(2) if self.run_step2_btn.isEnabled() else None

        )

        run_opti_alt = QShortcut(QKeySequence("Ctrl+R"), self)

        run_opti_alt.activated.connect(
    ...
    def _apply_theme(self):
        ...
        if (
            ...
            hasattr(self, "live_monitor_window")

            and self.live_monitor_window

            and self.live_monitor_window.isVisible()
        ...
        if (
            ...
            hasattr(self, "results_window")

            and self.results_window

            and self.results_window.isVisible()
    ...
    def on_stats_update(self, counter_type: str, increment: int):
    ...
    def _warmup_numba(self):
        ...
        try:
            ...
            _ = calculate_RT_vectorized_real_HL(

                dummy_wl, dummy_n, dummy_n, dummy_n, dummy_thick
    ...
    def tile_auxiliary_windows(self):

        import math

        top_margin, gap = 150, 5

        screen_geo = QApplication.primaryScreen().availableGeometry()

        avail_w, avail_h = screen_geo.width(), screen_geo.height() - top_margin

        start_y, start_x = screen_geo.y() + top_margin + gap, screen_geo.x() + gap
        ...
        visible_wins = [w for w in all_wins if w.isVisible()]
        ...
        wide_wins = [w for w in visible_wins if w not in narrow_wins]

        if not wide_wins and not narrow_wins:

            return

        x_split = int(avail_w * 0.75) if narrow_wins else avail_w

        if not wide_wins and narrow_wins:

            x_split = 0

        if wide_wins:

            cols = 1 if len(wide_wins) < 4 else 2

            rows = math.ceil(len(wide_wins) / cols)

            w_item, h_item = (x_split - gap - (cols - 1) * gap) // cols, (

                avail_h - (rows + 1) * gap

            ) // rows

            for i, win in enumerate(wide_wins):

                win.setGeometry(

                    start_x + (i % cols) * (w_item + gap),

                    start_y + (i // cols) * (h_item + gap),
        ...
        if narrow_wins:
            ...
            rows = len(narrow_wins)

            w_item, h_item = (avail_w - x_split) - gap, (avail_h - (rows + 1) * gap) // rows

            for i, win in enumerate(narrow_wins):

                win.setGeometry(

                    start_x + x_split if wide_wins else start_x,

                    start_y + i * (h_item + gap),
        ...
        self.status_label.setText(f"Smart Tiled: {len(wide_wins)} Main + {len(narrow_wins)} Side.")

    def close_all_auxiliary_windows(self):
    ...
    def _build_gui(self):
        ...
        workflow_hint = QLabel(
            ...
            "1 Build stack  2 Configure optimization  3 Run workflow  4 Inspect strategies"
        ...
        if getattr(self, "_log_panel", None):
            ...
            self._log_panel.copied.connect(

                lambda: self.status_label.setText(CERTUS_UI_STRINGS["logs_copied"])
    ...
    def _create_design_tab(self):
        ...
        materials_container = QGroupBox("Material Refractive Indices")
        ...
        gb_sub = QGroupBox("substrate_Base Wavelength")

        gb_sub_layout = QHBoxLayout(gb_sub)
        ...
        self.widgets["substrate_choice"].currentTextChanged.connect(

            self._on_substrate_choice_changed
        ...
        gb_lam = QGroupBox("Reference")

        gb_lam_layout = QHBoxLayout(gb_lam)
        ...
        self.stack_group = QGroupBox("Stack Control & Workflow")

        cockpit_layout = QHBoxLayout(self.stack_group)
        ...
        def set_std_icon(btn, pixmap_enum):
        ...
        self.detach_btn.setToolTip(
            ...
            "Detach the Stack Definition table into its own floating window for easier editing."
        ...
        self.run_step0_btn.clicked.connect(lambda: self.run_workflow(0))
        ...
        self.run_step2_btn.clicked.connect(lambda: self.run_workflow(2))
        ...
        self.run_step3_btn.clicked.connect(lambda: self.run_workflow(3))
        ...
        self.run_full_btn.clicked.connect(lambda: self.run_workflow(23))
        ...
        _hdrs = self.widgets["stack_table"].horizontalHeader()
        ...
        self.widgets["stack_table"].setSizePolicy(
    ...
    def request_stop_optimization(self):
    ...
    def _create_optimization_tab(self):
        ...
        scan_group = QGroupBox("Spectral Scanning Range")
        ...
        filter_group = QGroupBox("Candidate Filtering")
        ...
        loop_group = QGroupBox("Iteration Parameters (Nb Layers / X)")
        ...
        self.widgets["noise_distribution"].setToolTip(

            "Gaussian-only policy enabled for STRAT."
        ...
        report_group = QGroupBox("Reports & Data")
        ...
        self.btn_open_reports.clicked.connect(

            lambda: open_file_explorer(get_resource_path("reports"))
    ...
    def detach_stack_window(self):
        ...
        self._floating_stack_window = PopOutWindow(

            self.stack_group, self, "Stack Definition Manager"
    ...
    def reattach_stack_window(self):
    ...
    def _create_advanced_tab(self):
        ...
        robust_group = QGroupBox("Robustness Test (Step 3)")

        robust_layout = QVBoxLayout(robust_group)
        ...
        engine_group = QGroupBox("Deep Search Engine & Hybridization")
        ...
        limits_group = QGroupBox("Physical Limits & Search Depth")
        ...
        sym_group = QGroupBox("SYM Strategy Controls")
    ...
    def _create_why_certus_tab(self):
    ...
    def _create_material_group(self, parent_layout, title, prefix, label, _is_compact=False):
    ...
    def _on_material_mode_changed(self, prefix, label):
    ...
    def _on_substrate_choice_changed(self, text):
    ...
    def _create_line_edits(self, layout, items, columns=2):
    ...
    def _on_stack_table_changed(self, row, col):
        ...
        if "stack_multipliers" in config:
            ...
            try:
                ...
                stack_mults = [
                    ...
                    float(m.strip()) for m in str(config["stack_string"]).split(",") if m.strip()
    ...
    def load_configuration(self, filename=None):
        ...
        try:
            ...
            self._loaded_config = dict(config) if isinstance(config, dict) else {}
            ...
            if os.environ.get("QT_QPA_PLATFORM", "").lower() != "offscreen":
                ...
                summary = build_summary_plain_text(
                    ...
                    "CERTUS STRAT - Config Summary", summary_lines
    ...
    def _get_float_safe(self, widget_name, default=0.0):
    ...
    def collect_params(self) -> dict[str, Any]:
        ...
        if self.widgets["h_type_custom"].isChecked():
            ...
            if not txt:
                ...
                self.logger.warning(
                    ...
                    "⚠️ No H-material file selected. Reverting to Custom value (2.3)."
        ...
        if self.widgets["l_type_custom"].isChecked():
            ...
            if not txt:
                ...
                self.logger.warning(
                    ...
                    "⚠️ No L-material file selected. Reverting to Custom value (1.45)."
        ...
        multipliers = [
            ...
            float(table.item(r, 2).text()) for r in range(table.rowCount()) if table.item(r, 2)
        ...
        try:
            ...
            noise_str = (
                ...
                self.widgets["robustness_noise_factors"]
        ...
        params_out = {
            ...
            "sym_same_wl_bonus": self._get_float_safe(

                "sym_same_wl_bonus", SYM_DEFAULT_SAME_WL_BONUS

            ),

            "sym_extrema_window": self._get_float_safe(

                "sym_extrema_window", SYM_DEFAULT_EXTREMA_WINDOW_OT

            ),

            "sym_continuity_weight": self._get_float_safe(

                "sym_continuity_weight", SYM_DEFAULT_CONTINUITY_WEIGHT
            ...
            "sym_tie_epsilon_rel": self._get_float_safe(

                "sym_tie_epsilon_rel", SYM_DEFAULT_TIE_EPS_REL
            ...
            "consensus_seed_list": str(

                getattr(self, "_loaded_config", {}).get("consensus_seed_list", "")
    ...
    def run_workflow(self, step: int):
    ...
    def on_workflow_error(self, exc_info):
    ...
    def on_plot_ready(self, fig: Any, fig_type: str):
    ...
    def on_excel_ready(self, excel_data: io.BytesIO, metadata: Dict):
        ...
        try:
            ...
            try:
                ...
                req = IndexFitRequest(
                    ...
                    source_paths=[str(getattr(self, "_last_config_file", "") or "")],
                    seed=params_for_manifest.get("seed", params_for_manifest.get("random_seed")),
                    app_id="CERTUS_STRAT",
            ...
            if manifest_dict:
                try:
                    ...
                    self.logger.debug("STRAT manifest Excel sheet injection skipped")
                try:
                    ...
                    self.logger.debug("STRAT manifest JSON write skipped")
        ...
        try:
            try:
                ...
                if params_for_status.get("seed") is None and params_for_status.get("random_seed") is None:
                    self.set_validation_status("WARNING_UNSEEDED_STOCHASTIC")
                    self.add_validation_warning(
                        "STRAT auto-export run uses stochastic stages without explicit seed."
                    )
                else:
                    self.set_validation_status("OK")
            except Exception:
            ...
            if hasattr(self, "final_results") and isinstance(self.final_results, dict):
                ...
                strats = list(self.final_results.get("all_strategies_results", []) or [])

            if not strats and "strategies_results" in self.opti_results:

                strats = list(self.opti_results.get("strategies_results", []) or [])
            ...
            try:
                ...
                t_exec = (
                    ...
                    f"{self.opti_results.get('execution_time', 0):.2f}"
                ...
                if strats and isinstance(strats[0], dict) and isinstance(strats[0].get("strategy"), dict):
                    ...
                    # Convert blocks to simple list format
                ...
                if OPENPYXL_AVAILABLE:
                    ...
                    with pd.ExcelWriter(excel_path, engine="openpyxl") as writer:
                        ...
                        if manifest_dict:
                            pd.DataFrame(
                                [{"Key": str(k), "Value": str(v)} for k, v in manifest_dict.items()]
                    ...
                    self.logger.info(
                        ...
                        f"✅ Simple Excel saved (openpyxl missing): {Path(excel_path).name}"
            ...
            try:
                ...
                # Insert Methodology and Details
                ...
                # --- SECTION 3: TOP STRATEGIES TABLE ---
                ...
                if strats:
                    ...
                    for s in top_strategies:
                        ...
                        # Format blocks like "12.5-45.2 (32nm)"

                        blocks_fmt = ", ".join(

                            [f"{b['start']:.0f}-{b['end']:.0f}" for b in strat.get("blocks", [])[:3]]
                ...
                # Grab plots from current view context if available, or generate them
    ...
    def on_show_strategies_table(self, strategies_results: list[dict[str, Any]]):
        ...
        try:
            ...
            include_secondary_rmse_stats = bool(

                self.collect_params().get("include_secondary_rmse_stats", False)
        ...
        if self.strategies_table_window and self.strategies_table_window.isVisible():
            ...
            self.strategies_table_window.p_thick_nominal = np.array(

                current_p_thick, dtype=np.float64

            )

            self.strategies_table_window.include_secondary_rmse_stats = (

                include_secondary_rmse_stats
            ...
            self.strategies_table_window.strategy_selected.connect(

                self.on_strategy_visualization_requested

            )

            self.strategies_table_window.show()

    def on_strategy_visualization_requested(self, row_idx: int, strategy_result: dict[str, Any]):
        ...
        try:
            ...
            try:
                ...
                trans_win = TransmissionVsThicknessWindow(

                    self, strategy_result, self.opti_results, params
            ...
            try:
                ...
                spec_win = StrategySpectralPerformanceWindow(
            ...
            while True:
                ...
                if fig_type == "main_spectral_interactive":
                    ...
                    try:
                        ...
                        win = InteractiveSpectrumWindow(

                            self, data_obj, sigma=data_obj.get("sigma", None)
    ...
    def update_main_plot(self, fig: Any, force_render: bool = False):
        ...
        try:
            ...
            worker.finished.connect(lambda b, h: self._on_render_complete(b, h))
    ...
    def _on_render_complete(self, png_bytes, plot_hash):
    ...
    def update_stack_plot(self, fig):
    ...
    def on_live_growth_update(self, live_data):

        try:

            if self.live_monitor_window is None:

                self.live_monitor_window = LiveMonitorWindow(

                    None

                )  # No parent to avoid sub-window rendering
...
if __name__ == "__main__":
    ...
    setup_logging(log_file="certus_strat.log")
    ...
    if Path(clues_file).exists():
        ...
        try:
            ...
            for _ in range(20):  # ~1s total, processEvents every 50 ms
                app.processEvents()
                QThread.msleep(50)
