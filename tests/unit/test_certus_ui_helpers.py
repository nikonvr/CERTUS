...
from certus_ui import StatsCounter, confirm_and_stop, format_count_kmg
...
class TestConfirmAndStop:
    def test_returns_false_when_user_cancels(self):
        with patch("certus_ui.confirm_stop_with_timeout", return_value=False) as mock_dlg, \
             patch("certus_ui.stop_worker_and_thread") as mock_stop:
    ...
    def test_returns_true_when_confirmed_and_thread_stopped(self):
        with patch("certus_ui.confirm_stop_with_timeout", return_value=True) as mock_dlg, \
             patch("certus_ui.stop_worker_and_thread", return_value=True) as mock_stop:
    ...
    def test_returns_false_when_thread_does_not_stop(self):
        with patch("certus_ui.confirm_stop_with_timeout", return_value=True), \
             patch("certus_ui.stop_worker_and_thread", return_value=False):
