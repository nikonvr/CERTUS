...
def _build_profile_corridor_config(
    ...
    return ProfileCorridorConfig(
        ...
        adaptive_rmse_abs_noise_factor=float(
            getattr(cfg, "corridor_profile_d_adaptive_rmse_noise_factor", 3.0) or 3.0
        ),
    ...
    try:
        ...
        if bool(getattr(cfg, "gui_use_nl_alpha_for_corridors", False)):
            ...
            if alpha_nl is not None and np.isfinite(float(alpha_nl)):
                ...
                cfg_eff = cfg.replace(t_exp=t_exp_eff, r_exp=r_exp_eff)
        ...
        _raw_corr = str(getattr(cfg_eff, "corridor_profile_d_mode", "abs_delta_adaptive") or "abs_delta_adaptive").strip().lower()
        ...
        cfg_prof = cfg_eff.replace(nk_profile_interp="smooth")
