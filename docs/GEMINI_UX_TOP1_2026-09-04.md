...
|---|---|---|
| 3.1 | 🟠 **PREMIER INCRÉMENT le 2026-09-06 — l'échelle existe, le gros du routage reste.** ⚠️ **Et pas comme écrit** : `certus_ux.py` déclarait **déjà** une classe `Typography` avec la même échelle, dans la même unité, dont les **huit tailles ne servaient nulle part** en production. `FONT_SIZE_BASE = 10` et `Typography.BODY = 10` étaient donc le même fait écrit deux fois ; en ajouter une troisième aurait aggravé. Les noms demandés existent, comme **vues** sur l'échelle en place. | 23 tailles distinctes, px et pt mélangés. **162 déclarations `font-size:` mesurées le 2026-09-06** hors fichiers de palette (le « 187 » comptait aussi `setPointSize` et `QFont`, que le cliquet n'observe pas).<br>📏 **Routé : 5**, les seules valeurs en pt tombant **exactement** sur un pas. Preuve que c'est un refactor pur — les feuilles sortent **identiques au caractère**, SHA-256 comparée avant/après. Cliquet resserré **173 → 157**, ce qui supprime aussi 11 de mou qui traînaient.<br>🔴 **Les 157 restantes ne sont PAS un oubli.** La majorité est en **px** — `11px` revient **56 fois**, c'est la taille la plus répandue de la suite. Convertir px en pt change le rendu des onze fenêtres : à 96 dpi 10 pt valent 13,3 px, donc un libellé à 11 px est plus **petit** que la base tout en paraissant plus grand dans la source. **Choisir l'unité se fait les yeux sur un écran** — c'est une décision de 👤, pas un refactor. Les valeurs pt hors échelle (9,5 · 11 · 13 · 15) sont laissées telles quelles : les arrondir serait un changement de rendu déguisé. Verrou : `tests/ui/test_ux_type_scale.py` |
| 3.2 | **Police unique et installée** : une seule famille, une seule taille de base pour les 11 fenêtres ; refuser au démarrage une famille absente. 🔴 **NON FAITE, malgré les apparences** — le harnais annonce `Segoe UI @10 pt` partout parce qu'il **épingle** cette police depuis le correctif 0.1. Remesuré sans épinglage : rien n'a changé. Et les deux tests censés le garder sont inexploitables — l'un dépend de l'ordre d'exécution, l'autre teste la machine. Détail au §0bis | DESIGN et RE demandent « Open Sans », absente ; base 9 pt vs 10 pt — **inchangé au 2026-09-04 soir**<br><br>🔴 **ET IL Y A PIRE, TROUVÉ LE 2026-09-06 : L'APPLICATION TÉLÉCHARGE UNE POLICE À CHAQUE LANCEMENT, DEPUIS UNE URL MORTE.** `CertusTheme.apply_to_app()` — le point d'entrée du thème, appelé au démarrage de chaque fenêtre — appelle **inconditionnellement** `load_inter_font()`, qui tente deux `urllib.request.urlopen` vers `github.com` avec `timeout=2`.<br>📏 Mesuré : le dossier de cache `certus_fonts` sous `%APPDATA%` **existe et est VIDE** (créé le 2026-09-04, jamais rempli), et les deux URL rendent **HTTP 404** — elles sont mortes. Le téléchargement échoue donc **toujours**, rien n'est mis en cache, et **il est retenté à chaque lancement**.<br>🔑 **Le coût est supporté par la machine la plus susceptible d'être hors ligne** : un poste de bâti en salle. Sans route réseau, c'est **jusqu'à 2 s par fichier**, deux fichiers, à chaque ouverture de fenêtre. Ici le DNS résout et l'erreur revient en 0,11 s, ce qui masque le problème sur un poste connecté.<br>✅ **Le correctif est celui que l'étape 3.2 demande déjà** : exiger une famille **installée** et refuser au démarrage si elle manque. Retirer le téléchargement au passage — un logiciel scientifique de production ne doit pas dépendre d'un dépôt tiers pour s'afficher.<br><br>🟢 **LE TÉLÉCHARGEMENT EST RETIRÉ, le 2026-09-06.** La chaîne de repli est désormais **locale et explicite** : Inter installée → Inter · sinon les TTF déposés à la main dans le dossier de cache → Inter · sinon Segoe UI. Le second cas est gardé exprès, pour qu'un opérateur sur un poste hors ligne puisse encore installer la famille en copiant deux fichiers.<br>⚠️ **Le reste de 3.2 n'est PAS fait** : « une seule famille, une seule taille de base pour les 11 fenêtres » et le refus au démarrage restent ouverts.<br>🔑 **Le garde-fou COMPTE les appels au lieu d'en lever un**, et c'est la seule forme qui marche : une première version levait depuis le piège, or le code d'avant enveloppait le téléchargement dans un `except Exception: pass` — il aurait **avalé** l'exception et le test serait passé sur le code fautif. Un test incapable d'échouer. Vérifié échouant sur `HEAD` : **2 appels réseau, 2 URL**. Verrou : `tests/ui/test_ux_no_startup_network.py` |
| 3.3 | ✅ **FAITE le 2026-09-06 — mais PAS comme écrit ici.** L'étape disait « porter `BORDER` à ≥ 3:1 ». 🔴 **Mesuré : `BORDER` a 122 usages, et il sert de FOND — pas de bordure — aux boutons DÉSACTIVÉS, aux séparateurs et aux poignées d'ascenseur.** Le relever d'un bloc aurait rendu un contrôle désactivé plus présent qu'un contrôle actif, et sur-appliqué la règle : **WCAG exempte explicitement les éléments désactivés**. Un second jeton **`BORDER_STRONG`** a donc été posé, pour les contrôles dont le contour EST l'affordance (champs de saisie, bouton plat) ; le décoratif reste sur `BORDER`. | départ 1,35 (clair) / 1,48 (sombre) · **`BORDER_STRONG` : 3,58 sur SURFACE et 3,18 sur BACKGROUND en clair, 3,19 et 3,21 en sombre** — les deux fonds sont vérifiés, un champ pouvant se poser sur l'un comme sur l'autre. `#7792b1` avait été écarté : 3,22 sur SURFACE mais **2,86 sur BACKGROUND**. Verrou : `tests/ui/test_ux_border_contrast.py` |
| 3.4 | ✅ **FAITE le 2026-09-06.** Les 4 paires de badge ont désormais des valeurs sombres, et les **deux** jetons réellement morts sont retirés. | 📏 **Remesuré 2026-09-06.** Les 8 jetons identiques sont confirmés — ce sont les paires badge `SUCCESS/WARNING/DANGER/INFO` × `_BG`/`_TEXT`. ⚠️ **Ce n'est PAS un échec d'accessibilité** : le texte tient 5,30 à 6,49:1 sur son propre fond dans les deux modes. Le défaut est visuel — en sombre le fond pastel est à **14,5 à 16,2:1** contre la SURFACE, il troue l'écran. 🔴 **« 6 jetons morts » est FAUX : DEUX le sont** (`DARK_BORDER`, `DARK_TEXT_SUB`, 0 usage) ; les quatre autres construisent la **palette Qt** en mode sombre. 🔴 **Et il y a une divergence que l'étape ne mentionne pas** : `SURFACE` vaut `#111827` en QSS mais `DARK_SURFACE` vaut `#1e293b` dans la palette, alors que `BACKGROUND` et `TEXT_MAIN` s'accordent avec leur jumeau. L'asymétrie suggère un oubli, **mais ne la corrige pas sans regarder l'écran** : `QPalette.Base` plus clair que la fenêtre est aussi un choix d'élévation courant.<br><br>📏 **Résultat.** Fond de puce contre SURFACE sombre : **de 14,5–16,2:1 à 1,45–1,72:1** — elle se lit comme une puce au lieu de trouer l'écran. Texte sur son fond : **5,91 à 6,00:1**, donc AA tenu. Teinte conservée. 🔑 **Les deux grandeurs vont en sens opposés** — il fallait faire *baisser* fond/surface tout en gardant texte/fond — et une seule des deux est une exigence WCAG. Traiter cela comme « un échec de contraste » aurait mené au mauvais correctif. Verrou : `tests/ui/test_ux_dark_palette.py`, qui vérifie aussi qu'un aller-retour `dark → light` **restaure** le clair, et que le ménage des jetons morts **n'a pas débordé** sur les quatre vivants |
...
| 3.6 | ✅ **FAITE le 2026-09-06.** Anneau de focus sur `QPushButton` et sur les quatre variantes pleines. 🔑 **La couleur diffère par famille, et c'est une contrainte mesurée, pas un goût** : un anneau `{primary}` disparaît sur un fond PRIMARY (1:1), et un anneau `{text_main}` tombe à **1,56:1** sur SUCCESS en sombre — parce qu'en mode sombre le texte est clair et les remplissages aussi. Retenu : `{primary}` sur le bouton par défaut (5,00 clair / 6,98 sombre contre SURFACE), `{surface}` en creux sur les variantes (4,83 à 9,29 contre leur remplissage). ⚠️ `QToolButton` est **hors périmètre** : la feuille ne lui pose aucune marge, donc rien à compenser. | 0 règle au départ · **la marge est compensée** pour que prendre le focus ne déplace RIEN : 8+1=9 → 7+2=9, 8+0=8 → 6+2=8, 10+0=10 → 8+2=10. Verrou : `tests/ui/test_ux_focus_ring.py` |
| 3.7 | ✅ **FAITE le 2026-09-06.** 🔴 **LA CAUSE ÉTAIT AUTRE QUE CE QUI EST ÉCRIT, ET ELLE ÉTAIT PIRE.** L'étape vise `certus_theme.py` (14 px, pas de coche). Or `certus_ux.py`, appliqué **par-dessus**, déclare déjà 16 px **et** une coche SVG **et** un point radio SVG. Le défaut devrait donc être réparé — il ne l'est pas. 📏 **Mesuré au pixel le 2026-09-06** : une case cochée rend **`#0f62fe` uni sur 284 px**, sans un seul pixel de coche. 🔑 **Cause isolée par expérience : Qt QSS ne gère PAS les data-URL.** Le même PNG rend 36 px (bruit) en `data:` et **76 px en FICHIER**. La coche et le point radio **n'ont donc jamais fonctionné**. ⚠️ Ce n'est **pas** l'instabilité QtSvg — le processus survit ; c'est le mécanisme d'URL. **Correctif appliqué : les glyphes sont générés au `QPainter` — aucun SVG sur ce chemin — et référencés par CHEMIN** (`certus/utils/certus_qss_icons.py`). Écrits une fois dans le dossier temporaire, selon la convention déjà en place pour les polices et le cache numba. Chemin en `/`, car QSS lit la contre-oblique comme une échappement. Si l'écriture échoue, ou si aucune `QApplication` n'existe, la propriété `image` est **omise** et non posée vide — une `image: url("")` masquerait le glyphe natif. 🔑 La couleur du glyphe est `PRIMARY_TEXT` et non un blanc en dur : en sombre le remplissage est bleu clair, un blanc aurait donné **2,54:1** ; `PRIMARY_TEXT` donne 5,00 en clair et 7,02 en sombre. Verrou : `tests/ui/test_ux_checkbox_glyph.py`, qui **peint le widget et compte les pixels** au lieu de relire la feuille — c'est précisément une feuille d'apparence correcte qui a masqué le défaut. | échec WCAG 1.4.1 **confirmé en effet** : l'état coché n'est signalé que par la couleur |
| 3.8 | ✅ **FAITE le 2026-09-06.** La règle qui vidait l'image de `QComboBox::down-arrow` est retirée : Qt redessine sa flèche native dans les 22 px déjà réservés par `drop-down`. 🔑 **Le `chevron-down` du dépôt n'a PAS été utilisé** — QSS veut un chemin d'image, et `certus_icons.py` garde `is_svg_icon_rendering_disabled()`, **vrai sur cette machine**, parce que `QSvgRenderer` peut faire **aborter le processus** sur Windows + CPython 3.14. Échanger un défaut certain contre un défaut intermittent aurait été pire. | 6 combos dans DESIGN seul · Verrou : `tests/ui/test_ux_combo_affordance.py`, qui **dépouille les commentaires** avant de lire la feuille — le commentaire expliquant la règle supprimée la contenait en QSS littéral et a trompé la première vérification |
| 3.9 | ✅ **FAITE le 2026-09-06 — première moitié seulement, et c'est un choix.** ⚠️ **L'étape se contredit** : « un jeton par module » et « la couleur suit la catégorie » ne tiennent pas sur le même canal — six modules sont `core_workflow`, ils deviendraient identiques. Trancher avant d'agir ; la catégorie se dit mieux par la mise en page que par la couleur. 📏 **Mesuré 2026-09-06 sur les 10 tuiles** : 4 couleurs pour 10 modules · RE porte l'émeraude de STRAT · FIELD le violet de DESIGN · SUBSTRATE le bleu d'INDEX · 🔴 **et SMOOTHER porte `#15803d`, qui est le jeton sémantique `SUCCESS`** — pas une couleur de marque du tout. 🔑 **Contrainte d'architecture à connaître** : `certus/core/certus_hub_config.py` **duplique les hex** au lieu de lire `CertusTheme`, et ce n'est pas une négligence — `core` n'a pas le droit d'importer `certus.ui`. La source unique doit donc descendre dans la couche basse, pas monter.<br><br>📏 **Fait.** Quatre jetons neufs, choisis par mesure — ≥ 3,4:1 pour l'icône blanche et distance RGB maximale aux couleurs en place : RE `#d66b1f` · FIELD `#1995ae` · SMOOTHER `#e548b1` · SUBSTRATE `#469e1a`. Le jeton sémantique détourné est supprimé. La palette est définie **une fois**, dans `certus/core/certus_hub_config.py`, et `CertusTheme.BRAND_*` la **lit** au lieu de la redéclarer.<br>🔑 **Deux partages sont CONSERVÉS, délibérément** : `INDEX`/`INDEX SPLINE` et les deux `METAL` sont des outils **frères**, une couleur de famille se lit comme une parenté. Une première version du garde-fou exigeait « toutes distinctes » — elle aurait forcé six couleurs au lieu de quatre en cassant deux parentés voulues. L'étape n'en nomme d'ailleurs que quatre, ce qui confirme la lecture par familles.<br>🔴 **Non fait, et signalé : STRAT est à 2,54:1**, sous le seuil de 3:1. C'est une couleur d'identité établie ; la changer est une décision de 👤, pas une correction de routine. Elle est consignée dans le garde-fou pour ne pas disparaître de la vue.<br>⚠️ **La seconde moitié — « la couleur suit `category` » — n'est PAS implantée**, et c'est délibéré : elle rendrait six modules identiques. Le groupement se dit par la mise en page. Verrou : `tests/ui/test_ux_brand_colors.py` |
| 3.10 | **Les hexadécimaux et les `setStyleSheet`** : campagne de réduction, fichier par fichier, cliquet à chaque passe | annoncé 314 / 314 · **remesuré le 2026-09-06 : 409 hex sur 45 fichiers, 315 `setStyleSheet`**. 🔴 Le compte d'hex avait dérivé de **+30 %** — recompte, ne cite ni l'un ni l'autre. ⚠️ **Et 85 des 409 sont dans `certus_theme.py` lui-même, où ils sont LÉGITIMES** : c'est la définition de la palette. La dette réelle est donc d'environ **324**, concentrée ensuite sur `certus_index_spline_rendering.py` (28), `certus_manual_sigma_knot_dialog.py` (20) et `certus_field_events_mixin.py` (18).<br><br>🔴 **NE TRAITE PAS CETTE ÉTAPE COMME UN REFACTOR MÉCANIQUE — mesuré le 2026-09-07, et c'est ce qui m'a fait arrêter avant d'y toucher.**<br>**1. Convertir une couleur en jeton est un changement de COMPORTEMENT, pas une réécriture.** Contrairement à une taille de police, la valeur d'un jeton **change avec le thème** : `#ffffff` → `PRIMARY_TEXT` devient `#0f172a` en mode sombre. Router un littéral, c'est affirmer « cet élément doit suivre le thème » — souvent juste, jamais gratuit. L'étape 3.1 a pu se prouver identique au caractère ; **celle-ci ne le peut pas**.<br>**2. Qu'un hexadécimal VAILLE un jeton ne dit pas qu'il EST ce jeton.** 📏 **84 des 298 littéraux tombent exactement sur une valeur de jeton** — mais `#1e293b` sert **trois rôles opposés** : texte d'un rapport HTML dans `certus_field_events_mixin.py`, texte de mode **clair** dans `certus_plot.py:49` (`"#e2e8f0" if dark_mode else "#1e293b"`), et couleur de base **sombre** dans `certus_ui_widgets_utils.py:691`. Le router vers `DARK_SURFACE` parce que la valeur coïncide serait absurde deux fois sur trois.<br>**3. Une part de ces littéraux est dans du HTML EXPORTÉ**, qui doit rester **indépendant du thème de l'application** : un rapport enregistré ne doit pas changer de couleur parce que l'opérateur a basculé en mode sombre. ⚠️ Mon classement automatique n'en identifie que **20 avec certitude** et laisse 92 % en indéterminé — **le chiffre n'est donc pas la proportion**, seulement la preuve que le cas existe.<br>🔑 **La bonne forme reste celle qu'annonce l'étape** : fichier par fichier, avec jugement à chaque site, cliquet à chaque passe. Pas de passe globale |
