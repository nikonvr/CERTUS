...
class CorridorLiveStreamer:
    ...
    run_id: str = "corridor_run_0"
    stage: str = "live"

    def reset(self, *, run_id: str | None = None, stage: str | None = None) -> None:
        with self.live_stream_lock:
            self.live_d_vals.clear()
            self.live_rmse_vals.clear()
            self.live_n_curves.clear()
            self.live_k_curves.clear()
            self.live_chi2_vals.clear()
    ...
    def emit_profile(self, current_d_nm=None) -> None:
        if not callable(self.live_cb):
            log.info("%s live emit skipped: no callback attached.", _LOG_PREFIX)
            return
        if not self.live_d_vals or len(self.live_rmse_vals) != len(self.live_d_vals):
            log.info(
                "%s live emit skipped: inconsistent buffer sizes | d=%d rmse=%d n=%d k=%d chi2=%d",
                _LOG_PREFIX,
                int(len(self.live_d_vals)),
                int(len(self.live_rmse_vals)),
                int(len(self.live_n_curves)),
                int(len(self.live_k_curves)),
                int(len(self.live_chi2_vals)),
            )
            return
        ...
        if not np.any(m):
            log.info("%s live emit skipped: no finite d/rmse pairs.", _LOG_PREFIX)
            return
        d_a = d_a[m]
        r_a = r_a[m]
        ...
        payload = {
            ...
            "profile_d_run_id": str(self.run_id),
            "profile_d_stage": str(self.stage),
        ...
        if len(self.live_n_curves) == len(self.live_d_vals):
            payload["profile_d_n_curves"] = np.asarray([self.live_n_curves[i] for i in idx], dtype=np.float64)[order]
        if len(self.live_k_curves) == len(self.live_d_vals):
            payload["profile_d_k_curves"] = np.asarray([self.live_k_curves[i] for i in idx], dtype=np.float64)[order]
        if len(self.live_chi2_vals) == len(self.live_d_vals):
            payload["profile_d_chi2_values"] = np.asarray([self.live_chi2_vals[i] for i in idx], dtype=np.float64)[order]
        log.info(
            "%s live emit payload ready | stage=%s run=%s points=%d d[min,max]=[%.6f, %.6f] rmse[min,max]=[%.6e, %.6e] current_d=%.6f",
            _LOG_PREFIX,
            str(self.stage),
            str(self.run_id),
            int(order.size),
            ...
            float(r_a[order][0]),
            float(r_a[order][-1]),
            float(payload["profile_d_manual_grid_current_d_nm"]),
        )
        if log.isEnabledFor(logging.DEBUG):
            for _i, (_d, _r) in enumerate(zip(d_a[order], r_a[order], strict=False)):
                log.debug("%s live emit point[%d] | d=%.6f nm | rmse=%.6e", _LOG_PREFIX, int(_i), float(_d), float(_r))
        try:
            self.live_cb(payload)
            log.debug("%s live emit dispatched successfully.", _LOG_PREFIX)
...
def _clone_corridor_context(ctx: CorridorProfileContext) -> CorridorProfileContext:
    """Return a shallow copy of the corridor context for the final packaging step."""
...
def _validate_corridor_reference_envelope(
    ...
    """Apply nominal-envelope fixes and emit consistency warnings."""

    if n_stack.ndim == 2 and k_stack.ndim == 2 and n_stack.shape == k_stack.shape:
        crossing_mask = np.nanmin(n_stack, axis=0) < np.nanmax(k_stack, axis=0)
        if np.any(crossing_mask):
            log.warning(
                ...
                _LOG_PREFIX,
                int(np.sum(crossing_mask)),
                100.0 * float(np.mean(crossing_mask)),
            )
...
def _clone_corridor_context(ctx: CorridorProfileContext) -> CorridorProfileContext:
    """Return a shallow copy of the corridor context for packaging."""
...
def _log_corridor_start_config(
    ...
    if use_abs_delta:
        ...
        # live stream reset is handled by the caller-context before profiling starts.
...
class CorridorContextBuilder:
    def __init__(
        ...
        initial_run_id = f"corridor_{int(time.time_ns())}"
        self._run_id = initial_run_id
        self._run_stage = "run_0"
    ...
    def _reset_live_stream(self, *, run_id: str, stage: str) -> None:
        self._run_id = str(run_id)
        self._run_stage = str(stage)
        self.live_streamer.reset(run_id=self._run_id, stage=self._run_stage)
...
def compute_profiled_corridors_by_d(
    ...
    *,
    ...
    if len(ctx.d_vals) > 0:
        _rmse_dbg = np.asarray(ctx.rmse_vals, dtype=np.float64)
        _d_dbg = np.asarray(ctx.d_vals, dtype=np.float64)
        _n_dbg = min(len(_d_dbg), len(_rmse_dbg), len(ctx.n_curves), len(ctx.k_curves))
        log.info(
            "%s Corridor RMSE(d) debug: collected %d points (showing one by one).",
            _LOG_PREFIX,
            int(_n_dbg),
        )
        for _i_dbg in range(_n_dbg):
            try:
                _n0 = np.asarray(ctx.n_curves[_i_dbg], dtype=np.float64).ravel()
                _k0 = np.asarray(ctx.k_curves[_i_dbg], dtype=np.float64).ravel()
                _chi2_dbg = float(ctx.chi2_vals[_i_dbg]) if _i_dbg < len(ctx.chi2_vals) and np.isfinite(float(ctx.chi2_vals[_i_dbg])) else float("nan")
                log.info(
                    "%s Corridor point[%d] | d=%.6f nm | rmse=%.6e | chi2=%.6e | n0=%.6e | k0=%.6e | npts=%d",
                    _LOG_PREFIX,
                    _i_dbg,
                    float(_d_dbg[_i_dbg]),
                    float(_rmse_dbg[_i_dbg]),
                    _chi2_dbg,
                    float(_n0[0]) if _n0.size else float("nan"),
                    float(_k0[0]) if _k0.size else float("nan"),
                    int(_n0.size) if _n0.size else int(_k0.size),
                )
            except Exception:
                log.exception("%s Corridor point[%d] debug logging failed.", _LOG_PREFIX, _i_dbg)
    ...
    ctx = _clone_corridor_context(ctx)
