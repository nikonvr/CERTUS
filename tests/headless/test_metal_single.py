...
def test_metal_single_headless():
    ...
    try:
        ...
        metal_app = CertusMetalSingleApp()
        print("CertusMetalSingleApp created")
        
        # User constraint: Substrate MUST be SiO2
        # Ensure combo box matches the required choice before doing anything else
        metal_app.combo_substrate.setCurrentText("SiO2")
        print("setCurrentText SiO2 done")
        
        example_path = Path("example/example_metal_single/JSON-metal-example.json").resolve()
        # METAL SINGLE uses load_config, which triggers the loading of Target_Titane_Simu_20nm.xlsx internally
        ...
        if worker:
            # Some workers use finished, some use signals.finished
            # Let's handle both
