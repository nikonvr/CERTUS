...
| 1.3 | **`score > 0` → `score >= 0`** | `certus_strat_service.py:1157-1175` | Une stratégie de score exactement 0 — le meilleur possible — est **sautée**. Et `extract_best_rmse` **lève** si le score tombe dans `[0, 1e-7)`. |
