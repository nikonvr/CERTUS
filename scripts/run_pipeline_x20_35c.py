"""Exécution du pipeline nominal complet sur le passe-bande 35 couches avec un rayon d'action multiplié par 20 (x20).
...
def main():
    print("=" * 80)
    print("EXÉCUTION DU PIPELINE NOMINAL AVEC RAYON D'ACTION ÉLARGI x20 (35 COUCHES)")
    print("=" * 80, flush=True)
    ...
    def patched_collect(*args, **kwargs):
        ...
        p["robustness_num_runs"] = 300
        p["n_screen_runs"] = 10
        # Paramètres x20 désactivés (retour au nominal) pour tester la nouvelle Phase A
        p["dp_top_k"] = 40
    ...
    print("\n" + "=" * 80)
    print(f"PIPELINE x20 TERMINÉ EN {elapsed:.1f} s")
    print("=" * 80, flush=True)
    ...
    if res and isinstance(res, dict):
        ...
        print("\nTOP 10 DES STRATÉGIES SUR LE 35 COUCHES (RAYON ÉLARGI x20) :")
        for idx, s in enumerate(strats[:10]):
            ...
            print(f"  #{idx+1} | ID: {s_id} | Blocs: {n_b} | Crash: {cr*100:5.1f}% | Score: {sc:.6f} | RMSE P95: {r_p95:.4f}")
