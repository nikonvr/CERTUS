...
### 🔴 La suite immédiate, et l'ordre est imposé par ce qu'on vient de mesurer

1. **Réparer la cohérence Phase A / Phase B — §17-20, 23, 24, 25.** Six paramètres de modèle
   n'atteignent pas la propagation d'état, la Phase A n'a pas l'enveloppe du corridor, et un
   repli silencieux réinstalle un bug corrigé. **Tant que ce n'est pas fait, les cinq runs
   D1 et les quatre bras de A12 mesurent deux étages qui ne modélisent pas la même machine.**
2. **Refaire D1 seulement** — un `D0.ref` frais comme dénominateur sur le code recompilé, plus
   D1.1…D1.5. Six runs, ~2 h. Le reste de la campagne est à corridor 0 et **n'est pas touché** :
   le bloc fautif est derrière `if index_corridor > 0.0:`.
3. **Implanter le tri de §14** — SEEL quantifié à 0,1 nm, départage au rendement. §17-26
   démontre que le tri continu actuel départage du bruit ; ce n'est plus une option.
4. **Un run `dp_yield_weight` à corridor 0,005**, là où il y a des plantages — c'est la seule
   expérience qui tranche §17-14, et elle coûte un run.
