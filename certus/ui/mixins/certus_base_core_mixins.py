...
from certus.ui.certus_ui_utils import install_standard_shortcuts, show_toast, update_global_plot_config
...
class CertusZoomMixin:
    ...
    def _apply_ui_zoom(self, factor: float) -> None:
        ...
        self.setMinimumSize(int(getattr(self, "MIN_WIDTH", 800) * factor), int(getattr(self, "MIN_HEIGHT", 600) * factor))
        self._store_ui_zoom()
        try:
...
class CertusCommandPaletteMixin:
    ...
    def _default_commands(self) -> list[Any]:
        ...
        if hasattr(self, "save_config") and callable(getattr(self, "save_config")):
            actions.append(
                CommandAction(
                    id="file.save_config", title="Save configuration…", subtitle="Export current settings to a JSON file",
                    shortcut="Ctrl+S", category="File", icon_name="save", keywords=("export", "json", "write"),
        ...
        if hasattr(self, "load_config") and callable(getattr(self, "load_config")):
            actions.append(
                CommandAction(
                    id="file.load_config", title="Load configuration…", subtitle="Restore settings from a JSON file",
                    shortcut="Ctrl+O", category="File", icon_name="folder-open", keywords=("import", "json", "open"),
            ...
            if recents:
                actions.append(
                    CommandAction(
                        id="file.open_recent", title="Open recent configuration…", subtitle="Pick from the most recently used config files",
                        category="File", icon_name="file", keywords=("mru", "recent", "history"),
        ...
        if hasattr(self, "_copy_app_logs_to_clipboard"):
            actions.append(
                CommandAction(
                    id="view.copy_logs", title="Copy application logs to clipboard", category="View",
                    icon_name="copy", keywords=("debug", "clipboard", "logs"), callback=lambda: self._copy_app_logs_to_clipboard(),
                )
            )
        actions.append(
            CommandAction(
                id="view.toggle_theme", title="Toggle light / dark theme", category="View",
                icon_name="moon", keywords=("dark", "light", "appearance"), callback=lambda: self._toggle_theme(),
            )
        )
        if hasattr(self, "zoom_in_ui"):
            actions.append(
                CommandAction(
                    id="view.zoom_in", title="Zoom in", subtitle="Increase the interface scale for readability",
                    shortcut="Ctrl+Plus", category="View", icon_name="search-plus",
                    keywords=("zoom", "scale", "larger", "readability"), callback=lambda: self.zoom_in_ui(),
                )
            )
            actions.append(
                CommandAction(
                    id="view.zoom_out", title="Zoom out", subtitle="Reduce the interface scale for denser layouts",
                    shortcut="Ctrl+Minus", category="View", icon_name="search-minus",
                    keywords=("zoom", "scale", "smaller", "density"), callback=lambda: self.zoom_out_ui(),
                )
            )
            actions.append(
                CommandAction(
                    id="view.zoom_reset", title="Reset zoom", subtitle="Return the interface to the default size",
                    shortcut="Ctrl+0", category="View", icon_name="search",
                    keywords=("zoom", "reset", "default", "scale"), callback=lambda: self.reset_ui_zoom(),
                )
            )
        if hasattr(self, "open_shortcuts_overlay"):
            actions.append(
                CommandAction(
                    id="help.shortcuts", title="Show keyboard shortcuts", subtitle="List every registered shortcut in this window",
                    shortcut="F1", category="Help", icon_name="keyboard",
                    keywords=("help", "kbd", "hotkey", "cheatsheet"), callback=lambda: self.open_shortcuts_overlay(),
                )
            )
        actions.append(
            CommandAction(
                id="app.quit", title="Close this window", category="Application", icon_name="x",
                shortcut="Ctrl+W", callback=lambda: self.close(),
            )
        )
    ...
    def _auto_discovered_commands(self) -> list[Any]:
        ...
        catalogue = [
            ("run.optimize", "Run optimization", "Start the main optimisation workflow", ("run_optimization", "run_optim", "optimize", "start_optimization"), "Run", "play", "Ctrl+R", ("run", "optimize", "solve", "fit", "start")),
            ("run.stop", "Stop optimization", "Gracefully interrupt the running solver", ("stop_optimization", "stop", "cancel_run"), "Run", "square", "Esc", ("stop", "cancel", "abort", "halt")),
            ("run.analyze", "Run analysis", "Start the spectral / beam analysis", ("run_analysis", "start_analysis", "analyze", "run_beam"), "Run", "activity", None, ("analyze", "beam", "measure", "spectrum")),
            ("run.smart_init", "Smart init", "Launch the Smart-Init preparation dialog", ("smart_init", "auto_init", "open_smart_init"), "Run", "sparkles", None, ("init", "bootstrap", "smart")),
            ("edit.add_layer", "Add layer", "Append a new layer to the stack", ("add_layer",), "Edit", "plus", None, ("add", "layer", "insert", "stack")),
            ("edit.add_target", "Add target", "Append a new spectral target", ("add_target",), "Edit", "plus", None, ("add", "target", "spec")),
            ("edit.smart_cleanup", "Smart cleanup", "Remove low-impact layers and re-optimise", ("smart_cleanup",), "Edit", "trash-2", None, ("clean", "prune", "optimize", "simplify")),
            ("edit.reset_all", "Reset", "Reset the current session (destructive)", ("reset_all", "clear_stack", "reset"), "Edit", "refresh-ccw", None, ("reset", "clear", "start over")),
            ("file.export_excel", "Export to Excel…", "Save current data to a styled .xlsx workbook", ("export_excel",), "File", "table", None, ("excel", "xlsx", "export", "report")),
            ("file.export_csv", "Export to CSV…", "Save current data to a CSV file", ("export_csv",), "File", "file-text", None, ("csv", "export", "data")),
            ("file.export_pdf", "Export to PDF…", "Save a premium PDF report", ("export_report_pdf", "export_pdf", "export_report"), "File", "file", None, ("pdf", "report", "premium", "export")),
            ("file.export_report_excel", "Export premium Excel report…", "Save a fully-branded .xlsx report (cover + tables + charts)", ("export_report_excel",), "File", "table", None, ("excel", "premium", "report", "branded", "xlsx")),
            ("help.documentation", "Open documentation", "Show the in-app HTML documentation", ("open_help", "open_documentation"), "Help", "book-open", None, ("docs", "manual", "guide", "help")),
        ]
        ...
        for cmd_id, title, subtitle, method_names, category, icon, shortcut, keywords in catalogue:
            ...
            out.append(
                CommandAction(
                    id=cmd_id, title=title, subtitle=subtitle, shortcut=shortcut,
                    category=category, icon_name=icon, keywords=tuple(keywords), callback=(lambda fn=bound: fn()),
                )
            )
        return out
...
class CertusPremiumExportMixin:
    ...
    def export_premium_excel(self, sections: list, output_path: Optional[str] = None, *, ctx: Any = None) -> Optional[str]:
        try:
    ...
    def export_premium_pdf(self, sections: list, output_path: Optional[str] = None, *, ctx: Any = None) -> Optional[str]:
        try:
...
class CertusEmptyStateMixin:
    ...
    _EMPTY_STATE_HINTS: dict[str, tuple[str, str, str, Optional[str]]] = {
        "front_table": ("layers", "No layers yet", "Add a layer from the toolbar above, or load a configuration.", "Add layer"),
        "back_table": ("layers", "No back-side layers", "Enable back-side coating to edit the stack on this side.", None),
        "target_table": ("target", "No spectral targets", "Click 'Add target' to define the first wavelength window.", "Add target"),
        "spectra_table": ("line-chart", "No spectra loaded", "Drag a CSV file here or use File → Load spectra.", None),
        "measurement_table": ("activity", "No measurements yet", "Import measured data or switch to the Sample demos.", None),
        "results_table": ("check-circle", "No results yet", "Run the optimisation from the command palette or toolbar.", None),
    }
        ...
        for attr, (icon, title, desc, cta_label) in self._EMPTY_STATE_HINTS.items():
            ...
            try:
                attach_empty_state_to(
                    widget, icon_name=icon, title=title, description=desc,
                    action_label=cta_label, on_action=on_action,
                )
            except (RuntimeError, AttributeError, TypeError, ValueError):
                pass
...
class CertusDialogMixin:
    ...
    def confirm_destructive(
        self, title: str, message: str, *, detail: Optional[str] = None,
        confirm_label: str = "Continue", cancel_label: str = "Cancel", default_cancel: bool = True,
    ) -> bool:
        try:
            from PyQt6.QtWidgets import QMessageBox
