"""PILOTE DU LOT DE 10 H — grille resolution x epaisseur optique, puis exploration elargie.
...
#: (composant, mode, resolution_nm, elargi, minutes estimees, pourquoi cette cellule)
#: L'ORDRE EST L'ORDRE D'EXECUTION, et il est par valeur d'information decroissante.
CELLULES = [
    ("r75x2", "fast", 1.0, False, 45, "LA question -- 1 nm n'a jamais ete evalue sur x2"),
    ("r75x2", "fast", 0.5, False, 45, "second point de la courbe sur le cas dur"),
    ("r75x0.5", "fast", 5.0, False, 45, "le bonus de bruit /1,5 sur le cas mince"),
    ("r75x0.5", "fast", 2.0, False, 45, "reference propre du mince, meme sonde"),
    ("r75x1.5", "fast", 1.0, False, 45, "le point limite -- 1 deposable sur 704 a 2 nm"),
    ("75c", "fast", 1.0, False, 45, "le composant qui PASSE -- 1 nm le degrade-t-il ?"),
    ("75c", "fast", 5.0, False, 45, "et une fente large l'ameliore-t-elle ?"),
    ("75c", "fast", 0.5, False, 45, "colonne x1 complete"),
    ("r75x1.5", "fast", 0.5, False, 45, "colonne x1,5 complete"),
    # PHASE 2 -- exploration elargie, a resolution FIXEE. En dernier : sa duree est la moins
    # previsible, donc si elle deborde elle est perdue seule.
    ("r75x2", "deep", 1.0, True, 150, "PHASE 2 -- recherche x5 plus large sur la cellule de tete"),
]

# 🔑 x0,5 A 1 nm ET 0,5 nm NE SONT PAS DANS LA GRILLE, et ce n'est pas un oubli. Le run a
# recherche de fente du 2026-08-17 a deja produit des strategies aux trois largeurs sur ce
# composant, avec une tendance forte et sans ambiguite :
#
...
#
# Affiner AGGRAVE, monotonement. ⚠️ Ce sont des echantillons BIAISES -- seules les strategies
# dont les lambda toleraient la largeur ont ete retenues -- donc ils ne valent pas une cellule
# propre. Mais la tendance est assez nette pour ne pas depenser 90 min a la confirmer, alors
# que la phase 2 demandee par 👤 en a besoin. x0,5 garde donc sa reference propre a 2 nm et
# son seul point vraiment inedit, 5 nm.


def _sortie(nom: str, mode: str, res: float, elargi: bool) -> Path:
    suf = ("" if res == 2.0 else f"_res{res:g}") + ("_large" if elargi else "")
    return ROOT / "reports" / f"blocs_vs_plantage_{nom}_{mode}_s{SEED:03d}{suf}.json"
...
def etat() -> int:
    ...
    print("=" * 92)
    reste = 0
    for nom, mode, res, elargi, mn, pourquoi in CELLULES:
        f = _sortie(nom, mode, res, elargi)
        if f.exists():
            marque, note = "[FAIT]", f.name
        else:
            marque, note = "[    ]", f"~{mn} min -- {pourquoi}"
            reste += mn
        print(f"  {marque} {nom:<9} {mode:<7} {res:>4g} nm  {'elargi' if elargi else '      '}  {note}")
...
def main() -> int:
    ...
    for nom, mode, res, elargi, mn, pourquoi in CELLULES:
        f = _sortie(nom, mode, res, elargi)
        if f.exists():
            print(f"  [SAUTE] {nom} {mode} {res:g} nm -- deja mesure ({f.name})", flush=True)
            sautes += 1
            continue
        ...
        if restant < mn * 60:
            print(f"  [REFUSE] {nom} {mode} {res:g} nm -- il reste {restant / 60:.0f} min pour "
        ...
        print(f"\n{'=' * 92}")
        print(f"  {nom} | {mode} | {res:g} nm | {'ELARGI' if elargi else 'standard'} | "
        ...
        r = subprocess.run(
            [PY, SONDE, nom, mode, "0", "0", str(res), "1" if elargi else "0"],
