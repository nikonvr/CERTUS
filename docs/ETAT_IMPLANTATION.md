...
| **§24-18** | `consensus_num_runs` surchargeable | 🟠 Il **existe** (défaut 150, `certus_strat_ui_state.py`, `collect_params` — clé de dictionnaire, donc pas de ligne : voir la note `min_resolution` ci-dessous) mais **aucune surcharge ne l'atteint**. Toute mesure avec consensus actif tourne donc à 150 tirages quoi qu'on demande. |
...
| `min_resolution` comme **critère** | 🟠 calculée, sert de **départage dans le tri** (`certus_strat_ranking.py`, `_apply_strategy_ranking` / `_advanced_key`) — ⚠️ **pas de numéro de ligne ici, et c'est délibéré** : `min_resolution` n'est qu'une **clé de dictionnaire**, donc le contrôle F de `scripts/coherence_md.py` ne peut pas la localiser et le renvoi se périmerait à chaque édition du fichier sans que rien ne le rattrape. 📏 Sept vagues de recalage en une seule session le 2026-08-21 : un renvoi qu'aucun outil ne vérifie ne doit pas porter de ligne, mais **elle ne rejette rien** — §12-contrôle 4 : compter les rejets avant de la câbler. |
