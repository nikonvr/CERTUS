...
def re_progress_pct_p2a(pl: dict[str, Any], ki: int, intra: float) -> float:
    ...
    return re_interpolate_progress_segment(

        pl, float(ki), float(pl["tk"]), "p2a_lo", "p2a_hi", intra
...
def re_progress_pct_p2b(pl: dict[str, Any], ki: int, intra: float) -> float:
    ...
    return re_interpolate_progress_segment(
        ...
        pl, float(ki), float(pl["tk"]), "p2b_lo", "p2b_hi", intra
...
def re_progress_pct_p3(pl: dict[str, Any], si: int, intra: float, re_n_sh_cfg: int) -> float:
    ...
    if int(re_n_sh_cfg) > 0:
        ...
        return re_interpolate_progress_segment(
            ...
            pl, float(si), float(re_n_sh_cfg), "p3_lo", "p3_hi", intra
...
def re_trf_bounds_scipy_tuples(

    lb_ep: np.ndarray, ub_ep: np.ndarray
...
def re_enrich_results_ranking_fields(
    ...
    *,
    ...
    for _r in results:
        ...
        _r["re_ranking_score"] = float(

            re_ranking_combined_rmse(_sp_r, _qw_raw, alpha_rank_ref)
    ...
    *,
    ...
    if stopped_by_user:
        ...
        return (
            ...
            f"RE stopped  best RMSE_sp={best_sp:.5f} | "

            f"RMSE_OT={best_ot:.5f} | RMSE={best_combined:.5f}"

        )

    return (

        f"RE finished in {elapsed_s:.1f}s  "

        f"RMSE_sp={best_sp:.5f} | RMSE_OT={best_ot:.5f} | RMSE={best_combined:.5f}"
