...
def sellmeier_2poles_eval_nj(params, wl_um):
...
def _deduce_knots_from_k8p(wl_um, p_k8, num_knots=8, min_knot_dist_um=0.05):
    ...
    curv[1:-1] = np.abs(
        ...
        (dlogk_dlam[2:] - dlogk_dlam[:-2]) / (wl_um[2:] - wl_um[:-2] + 1e-20)
    ...
    for j in range(1, num_knots - 1):

        t = j / (num_knots - 1)

        target = t * total

        idx = np.searchsorted(cum[1:], target)

        idx = min(max(idx, 0), n_pts - 1)

        knot_lam[j] = wl_um[idx]
...
def _ensure_strictly_increasing(knot_lam: np.ndarray, min_gap: float = 1e-10) -> np.ndarray:
    ...
    """Guarantees knot_lam is strictly increasing (CubicSpline requires strict growth)."""
...
def _merge_closest_knot_pair(knot_lam_um: np.ndarray, log_k_values: np.ndarray):
...
class IRGlobalObjective:
    ...
    def __init__(self, wls, target_T, target_R, n_sub, T_sub, R_sub, thickness, n_tlu_ref, config):
    ...
    def _get_cached(self, p: np.ndarray):
    ...
    def _set_cached(self, p: np.ndarray, cost=None, grad=None, n=None, k=None):
    ...
    def _compute_cost(self, n, k):
        ...
        if self.is_frosted:
            ...
            R_c = calculate_reflection_infinite_substrate_array(self.wls, n, k, self.thickness, self.n_sub)
    ...
    def gradient(self, p):
        ...
        grad = _compute_ir_global_cost_gradient_kernel(
            ...
            self.wls, n, k, self.thickness, self.n_sub, 

            tgt_T, tgt_R, self.spec_w, use_T, use_R, 

            dn_dp, dk_dp, self.T_sub, self.use_norm, wT_actual, wR_actual,

            self.is_frosted, self._abs_sub, k_sub_v, D_sub_v
...
class Phase23SplineObjective:
    ...
    def __init__(self, base_obj: IRGlobalObjective, knot_lambda_um: np.ndarray, k_max: float):
    ...
    def _set_cached(self, x: np.ndarray, cost=None, grad=None):
    ...
    def gradient(self, x: np.ndarray) -> np.ndarray:
        ...
        grad = _compute_ir_global_cost_gradient_kernel(
            ...
            self.obj.wls, n_arr, k, self.obj.thickness, self.obj.n_sub,

            tgt_T, tgt_R, self.obj.spec_w, use_T, use_R,

            dn_dp, dk_dp, self.obj.T_sub, self.obj.use_norm, wT, wR,

            self.obj.is_frosted, self.obj._abs_sub, k_sub_v, D_sub_v
...
class Phase23Pass2SplineObjective:
    ...
    def __init__(self, base_obj: IRGlobalObjective, num_knots: int, wl_min_um: float, wl_max_um: float, k_max: float, min_knot_dist_um: float):
...
def sellmeier_2poles_eval(params, wl_um):
...
def _sellmeier_residuals(params, wl_um, n_exp):

    return (sellmeier_2poles_eval(params, wl_um) - n_exp) * 1000


def fit_sellmeier_global(wls_nm: np.ndarray, n_exp: np.ndarray, material: str = "other", valid_mask: np.ndarray | None = None) -> tuple[np.ndarray, np.ndarray]:
    ...
    try:
        ...
        ls_bounds = (
            ...
            [b[0] for b in b_bounds],

            [b[1] for b in b_bounds]
...
def fit_k_global_8p(wls_nm: np.ndarray, k_exp: np.ndarray, valid_mask: np.ndarray | None = None) -> tuple[np.ndarray, np.ndarray|None]:
    ...
    bounds = (
        ...
        [-20.0, -40.0, -20.0, -40.0,  0.0, 1.0, 0.01, 1.0],

        [ 20.0,   5.0,  20.0,   5.0,  1.0, 20.0, 5.0,  6.0]
...
def analyze_loaded_data(df: pd.DataFrame) -> tuple[DataType, dict[str, np.ndarray]]:
    ...
    if n_cols == 2:
        ...
        col2_type = (
            ...
            col2_header_type if col2_header_type != "unknown" else detect_data_type(col2_data)

        )

        col3_type = (

            col3_header_type if col3_header_type != "unknown" else detect_data_type(col3_data)
...
def _optimize_point_kernel(
    ...
    Local Coordinate Descent for a single wavelength point (n, k).

    Constraints: +/- 2% of start values (relative).

    Fixed thickness d.

    """

    # Constraints: 2% relative to starting values

    delta_n = n_start * 0.02

    delta_k = max(k_start * 0.02, 0.0002)  # minimum absolute floor for k
    ...
    # Manually inlined for Numba simplicity or defined as closure
    ...
    if is_frosted_glass:
        ...
        # Simple casting for substrate index (assume real part dominant for interface)

        n_sub_c = n_sub + 0j

        Rc = calculate_reflection_infinite_substrate_single(

            wl, current_n, current_k, d, n_sub_c
        ...
        Rc, Tc = calculate_RT_single_layer_single(wl, current_n, current_k, d, n_sub_c)
    ...
    if weight_sum < SMALL_EPSILON:

        best_cost = 1e12

    else:

        best_cost = cost_val / weight_sum

    # -------------------------------------------------------------------------

    # Steps: coarse (half the bound) then fixed fine steps for convergence

    coarse_step = delta_n * 0.5  # explores the full +/-2% in ~4 moves
        ...
        while improved:
            ...
            if next_n <= n_max:
                ...
                if is_frosted_glass:
                    ...
                    Rc_ = calculate_reflection_infinite_substrate_single(

                        wl, next_n, current_k, d, n_sub + 0j

                    )

                    if use_R:

                        vR = Rc_  # Frosted: never R/Tnu

                        if not np.isnan(vR) and not np.isnan(target_R):

                            c_val += (vR - target_R) ** 2 * weight_R

                            w_sum += weight_R

                else:

                    Rc_, Tc_ = calculate_RT_single_layer_single(
                    ...
                    if use_T:
                        ...
                        vT = (
                            ...
                            if (use_normalized and T_sub > T_SUB_MIN_T_NORM)

                            else (np.nan if (use_normalized) else Tc_)

                        )

                        if not np.isnan(vT) and not np.isnan(target_T):

                            c_val += (vT - target_T) ** 2 * weight_T
                    ...
                    if use_R:
                        ...
                        vR = (
                            ...
                            if (use_normalized and T_sub > T_SUB_MIN_R_NORM)

                            else (np.nan if (use_normalized) else Rc_)
                ...
                c = c_val / w_sum if w_sum > SMALL_EPSILON else 1e12
            ...
            if next_k <= k_max:
                ...
                if is_frosted_glass:
                    ...
                    Rc_ = calculate_reflection_infinite_substrate_single(
                        ...
                        wl, current_n, next_k, d, n_sub + 0j
...
def estimate_initial_params(
    ...
    # Fenêtre pure IR : E_max_scan peut être ~1 eV ; sans plancher, Eg tombe ~1–2 eV et le TLU
    # part dans une dispersion non physique (n/k absurdes). Oxides types SiO2/Al2O3 : Eg >> hν(IR).
    ...
    if target_T is not None and len(target_T) > 10:
        ...
        T_sub_mean = np.mean(calculate_T_substrate_array(wavelengths, n_substrate))
    ...
    # Spectre type lame claire (T normalisée élevée) : queue d'Urbach resserrée + Eg bien au-dessus de hν max → ε2 et k init minuscules.
...
except (ValueError, TypeError, RuntimeError, AttributeError, KeyError, IndexError, FileNotFoundError) as _e_sap:
...
def _get_sapphire_n_on_grid(wavelengths_nm: np.ndarray) -> np.ndarray | None:

    """Interpolate sapphire n on any wavelength grid. Returns None if data unavailable."""

    if _SAPPHIRE_WLS is None or _SAPPHIRE_N is None:

        return None

    return np.interp(

        wavelengths_nm, _SAPPHIRE_WLS, _SAPPHIRE_N,

        left=_SAPPHIRE_N[0], right=_SAPPHIRE_N[-1],
...
except (ValueError, TypeError, RuntimeError, AttributeError, KeyError, IndexError, FileNotFoundError) as _e_si:
...
def _get_silicon_n_on_grid(wavelengths_nm: np.ndarray) -> np.ndarray | None:
    ...
    return np.interp(
        ...
        wavelengths_nm, _SILICON_WLS, _SILICON_N,

        left=_SILICON_N[0], right=_SILICON_N[-1],
...
class OptimizationConfig:
    ...
    def __init__(
        ...
        self.phase2_pglobal_overrides = phase2_pglobal_overrides if phase2_pglobal_overrides is not None else dict(PHASE2_IR_PGLOBAL_OVERRIDES_FAST)  # Optional: override Phase 2 IR PGlobal; default = compromis rapide HPO

        self.k_sub_data = k_sub_data  # Optional: k_sub per wavelength (same grid as target_data); None = transparent substrate

        self.substrate_thickness_nm = substrate_thickness_nm  # Physical substrate thickness in nm; required when k_sub_data is set

        self.n_sub_data = n_sub_data  # Optional: n_sub from file (e.g. example/sapphire fresnel.xlsx); when set, overrides Sellmeier
...
# Marge intérieure par rapport aux limites utilisées dans epsilon_to_nk (certus_core) :
# évite les trajectoires coincées exactement sur la coupe dure tout en gardant tout l'espace physique
# (diélectriques à fortement absorbants indice élevé ou métalloïde léger dans la boîte du modèle).
TLU_SOFT_EDGE_MARGIN = 0.05


# Prior « lame claire » (T normalisée, BOTH) : repousse les solutions où ε′ est clampée à 1.0 dans le
# modèle TL → n≈1 « air » avec RMSE artificiellement bas. Aligné ~sur la plage diélectrique courante (≥1.5).
...
class TLUObjective:
    ...
    def __init__(
        ...
        if is_frosted_glass:
            ...
            self.R_substrate = calculate_R_frosted_glass_reference(

                self.wavelengths, self.n_substrate

            )

        else:

            self.R_substrate = calculate_R_substrate_array(
        ...
        # Eg (eV) : si Eg << hν sur une partie de la fenêtre, le TLU devient incohérent (n/k folles).
        # Borne inf = max photon de la grille + petite marge (modèle sous-gap utilisable sur tout le fit).
        ...
        # Si les données T montrent une lame très transparente (T/Tsub ou T norm.), le scan ne doit pas
        # démarrer avec des k géants : plafond souple sur max(k) bien plus bas que K_MAX_LIMIT.
        ...
        # Budget de logs WARNING dans __call__ (analyse « pourquoi k explose ») sans saturer 20k evals.

        self._tlu_explode_logs_left = 20

    def format_diag_line(self, params: np.ndarray) -> str:

        """Une ligne lisible : n, k_max @λ, plafonds k, TLU (Eg, Eu, …), eps2, fraction E>Eg."""
    ...
    def format_k_line(self, params: np.ndarray) -> str:
        ...
        """Chaîne courte centrée sur k(λ) pour les logs Phase 1 PGLOBAL."""
    ...
    def get_bounds(self) -> np.ndarray:
        ...
        thickness_bound = np.array(

            [[self.thickness_bounds[0], self.thickness_bounds[1]]], dtype=np.float64

        )

        return np.vstack([thickness_bound, self.param_bounds])

    def _get_cached(self, params: np.ndarray):
    ...
    def _set_cached(self, params: np.ndarray, cost=None, grad=None):
    ...
    def gradient(self, params: np.ndarray) -> np.ndarray:
        ...
        n_calc, k_calc, dn_dp, dk_dp = _compute_tlu_derivatives_kernel(

            self.E_array, Eg, A, E0, C, Eu, eps_inf
        ...
        target_T_arr = (
            ...
            self.target_T if self.target_T is not None else np.zeros_like(self.wavelengths)

        )

        target_R_arr = (

            self.target_R if self.target_R is not None else np.zeros_like(self.wavelengths)
    ...
    def __call__(self, params: np.ndarray) -> float:
        ...
        try:
            ...
            # Correspond aux mêmes bornes globales que epsilon_to_nk, avec une légère marge intérieure :

            # compatible tous types de matériaux dans [N_MIN,N_MAX], k jusqu'à K_MAX_LIMIT.
            ...
            if self.is_frosted_glass:
                ...
                if (
                    ...
                    self.data_type in (DataType.REFLECTION, DataType.BOTH)
                ...
                if self.has_absorbing_substrate:
                    ...
                    R_calc, T_calc = calculate_RT_single_layer_absorbing_substrate_array(
                        ...
                        self.wavelengths, n_calc, k_calc, thickness, self.n_substrate, self.k_sub_data, self.substrate_thickness_nm
...
class GradientSearcher:
    ...
    def search(self, x0: np.ndarray, max_feval: int = 1500) -> tuple:
        ...
        try:
            ...
            # Fallback si gradient échoue (ex. singularité numérique, ZeroDivisionError dans le noyau TLU).

            # Aligné sur CERTUS_INDEX OLD.py : ne pas laisser remonter l'exception → Phase 1 PGLOBAL continue.
    ...
    def optimize(self, max_iter: int = 30, callback=None, x0: np.ndarray | None = None) -> Sample | None:
        ...
        if self.n_workers > 1:
            ...
            numba.set_num_threads(
                ...
                _numba_set_threads_clamped(max(1, nb_cores // self.n_workers))
        ...
        try:
            ...
            for iteration in range(max_iter):
                ...
                cand_x, cand_y = self.clusterer.process_batch(
                    ...
                    x_batch, y_batch, self._n_total_samples
                ...
                if self.n_workers <= 1 and n_dispatch > 0:
                    ...
                    for idx in idx_sorted:
                        ...
                        if callback:
                            ...
                            monitor = lambda xk: callback(Sample(xk, self.objective(xk)))
                        ...
                        try:
                            ...
                            # Aligné OLD : une recherche locale ratée ne doit pas arrêter PGLOBAL.
                ...
                # --- PARALLEL MODE ---
                    ...
                    for future in as_completed(futures):
                        ...
                        try:
                            ...
                            # Aligné OLD : futures L-BFGS-B (gradient) peuvent lever ZeroDivisionError, etc.
...
class IRPGlobalCallback:
    ...
    def __init__(self, worker, opt_instance, obj, c, l_full, thickness, n_sub_full, title="IR Global Search", emit_plot=True):
    ...
    def __call__(self, sample):
        ...
        if rmse < self.state[3] or (now - self.state[4]) >= 10.0:
            ...
            if rmse < self.state[3]: self.state[3] = rmse
...
class IRStage2Callback:
    ...
    def __init__(self, worker, obj, c, l_full, thickness, n_sub_full):
...
class IRSplineCallback:
    ...
    def __init__(self, worker, obj, c, l_full, thickness, n_sub_full, title, phase23_obj=None, knot_lam=None, n_k=None, lk_lo=None, lk_hi=None, t0=None):
...
class IRGlobalModelWorker(QObject):
    ...
    def __init__(self, config: OptimizationConfig, tlu_results: OptimizationResults, logger=None):
    ...
    def _prepare_ir_phase2_inputs(self):
        ...
        n_sub_full = c.n_sub_data if c.n_sub_data is not None else get_n_substrate_array_by_id(sub_id, l_full)
        ...
        if c.is_frosted_glass:
            ...
            R_sub_full = calculate_R_frosted_glass_reference(l_full, n_sub_full)

        elif c.has_absorbing_substrate:

            T_sub_full = calculate_T_substrate_absorbing_array(l_full, n_sub_full, _k_sub_full, _D_sub)

            R_sub_full = calculate_R_substrate_absorbing_array(l_full, n_sub_full, _k_sub_full, _D_sub)

        else:

            T_sub_full = calculate_T_substrate_array(l_full, n_sub_full)

            R_sub_full = calculate_R_substrate_array(l_full, n_sub_full)
    ...
    def _run_ir_stage0_to_stage2(
        ...
        flat_bounds = np.array([
            ...
            [-40.0, 40.0], [-100.0, 80.0],

            [-40.0, 40.0], [-100.0, 80.0],

            [1e-10, 0.1], [0.5, 12.0], [0.005, 5.0],
        ...
        except (ValueError, TypeError, RuntimeError, AttributeError, KeyError, IndexError, FileNotFoundError) as _e_ws:
        ...
        optimizer = PGlobalOptimizerINDEX(obj, pg_bounds, config=pg_conf, log_clues=log_clues, stop_event=self._stop_event)
        ...
        pg_callback = IRPGlobalCallback(self, optimizer, obj, c, l_full, thickness, n_sub_full, "IR Global Search", True)
        ...
        except (ValueError, TypeError, RuntimeError, AttributeError, KeyError, IndexError, FileNotFoundError) as _e_s2:
    ...
    def _run_phase21_refinement(
        ...
        try:
            ...
            _cb_21 = IRSplineCallback(
                ...
                self, obj, c, l_full, thickness, n_sub_full, "Phase 2.1 (90% T)",

                knot_lam=k_spline_knots_lambda_um, n_k=n_k, lk_lo=log_k_lo, lk_hi=log_k_hi
        ...
        except (ValueError, TypeError, RuntimeError, AttributeError, KeyError, IndexError, FileNotFoundError) as _e21:
        ...
        while best_n > MIN_KNOTS:
            ...
            while True:
                ...
                _cb_red_p1 = IRSplineCallback(
                    ...
                    self, obj, c, l_full, thickness, n_sub_full, f"Knot reduction {best_n}->{n_k_try}",

                    knot_lam=knot_lam_try, n_k=n_k_try, lk_lo=log_k_lo, lk_hi=log_k_hi, t0=t0_red

                )

                r1_try = scipy.optimize.minimize(

                    phase23_try, best_x_red, method="L-BFGS-B", jac=phase23_try.gradient,

                    bounds=bounds_p1_try, options={"maxiter": 3000, "ftol": 1e-14, "gtol": 1e-9},
            ...
            bounds_lam_try = [
                ...
                (lam_min + min_knot_dist_um, lam_max - min_knot_dist_um)

                for _ in range(n_k_try - 2)
            ...
            _cb_red_p2 = IRSplineCallback(
                ...
                phase23_obj=phase23_p2_try, n_k=n_k_try, lk_lo=log_k_lo, lk_hi=log_k_hi
            ...
            try:
                ...
                r2_try = scipy.optimize.minimize(
                    ...
                    phase23_p2_try, x0_p2_try, method="L-BFGS-B", jac=phase23_p2_try.gradient,

                    bounds=bounds_p2_try, options={"maxiter": 2000, "ftol": 1e-14, "gtol": 1e-9},
            ...
            except (ValueError, TypeError, RuntimeError, AttributeError, KeyError, IndexError, FileNotFoundError) as _ered:
    ...
    def _run_phase23_spline_refinement(
        ...
        try:
            ...
            while True:
                ...
                _cb_p1 = IRSplineCallback(
                    ...
                    self, obj, c, l_full, thickness, n_sub_full, "Phase 2.3 spline k",

                    knot_lam=knot_lam_um, n_k=num_knots, lk_lo=log_k_lo, lk_hi=log_k_hi, t0=t0_p1
            ...
            if np.isfinite(r23.fun) and r23.fun < 1e10:
                ...
                _cb_p2 = IRSplineCallback(
                    ...
                    self, obj, c, l_full, thickness, n_sub_full, "Phase 2.3 Pass 2",

                    phase23_obj=phase23_pass2, n_k=num_knots, lk_lo=log_k_lo, lk_hi=log_k_hi
                ...
                except (ValueError, TypeError, RuntimeError, AttributeError, KeyError, IndexError, FileNotFoundError) as _ep2:
        ...
        except (ValueError, TypeError, RuntimeError, AttributeError, KeyError, IndexError, FileNotFoundError) as _e23:
    ...
    def run(self):
        ...
        try:
            ...
            n_final, k_final, p_opt_final, k_spline_knots_lambda_um, k_spline_knots_values = self._run_phase23_spline_refinement(
                c,
                obj,
                res_pg,
                l_full,
            ...
            results = self._package_results(
                ...
                n_final, k_final, thickness, l_full, p_opt_final,
    ...
    def _package_results(self, n, k, thickness, l_full, p_opt,

                         n_T=None, k_T=None, p_T=None,

                         k_spline_knots_lambda_um=None, k_spline_knots_values=None):

        c = self.config

        sub_id = SUBSTRATES[c.substrate]["id"]

        n_sub = c.n_sub_data if c.n_sub_data is not None else get_n_substrate_array_by_id(sub_id, l_full)
        ...
        if "T" in c.target_data.columns: df["T_target"] = c.target_data["T"].to_numpy()

        if "R" in c.target_data.columns: df["R_target"] = c.target_data["R"].to_numpy()
        ...
        results = OptimizationResults(
            ...
            config=c, optimal_thickness=thickness, final_mse=self.best_mse,

            df_results=df, tlu_params=None,

            optimization_stats={

                "method": "PGLOBAL Global Refinement (Sellmeier+k8p)",

                "final_cost": self.best_mse
...
def _compute_RT_from_config(c, l_full, n, k, thickness, n_sub):
    ...
    if c.is_frosted_glass:
        ...
        Rc = calculate_reflection_infinite_substrate_array(l_full, n, k, thickness, n_sub)

        return Rc, nan_arr, nan_arr

    elif getattr(c, "has_absorbing_substrate", False):

        Rc, Tc = calculate_RT_single_layer_absorbing_substrate_array(

            l_full, n, k, thickness, n_sub, _k_sub, _D_sub

        )

        Ts = calculate_T_substrate_absorbing_array(l_full, n_sub, _k_sub, _D_sub)
        ...
        Ts = calculate_T_substrate_array(l_full, n_sub)
...
class Phase1Callback:
    ...
    def __init__(self, worker, max_evals):
...
class SubsetOptimTask:
    ...
    def __init__(self, worker, wls, n_sub, target_T, target_R, exclude_range):
...
class OptimizationWorker(QObject):
    ...
    def _try_active_update(self, params, mse, force=False):
    ...
    def _run_subset_optim(self, clues_slice, wls, n_sub, target_T, target_R, exclude_range):
    ...
    def _prepare_run_inputs(
        ...
        if _subn == "Sapphire (Al2O3)":
            ...
            self.logger.info(
                ...
                "[FILE] Substrate Al2O3 (n,k): %s | k column in xlsx: %s",
        ...
        mask = (c.target_data["lambda"] >= c.lambda_min) & (

            c.target_data["lambda"] <= effective_lambda_max
        ...
        if len(wls) == 0:
            ...
            raise ValueError(
                ...
                "Wavelength window empty for optimization (check min/max lambda or exclusion range)."
        ...
        if c.n_sub_data is not None:
            ...
            n_sub = np.interp(wls, l_full, c.n_sub_data, left=c.n_sub_data[0], right=c.n_sub_data[-1]).astype(np.float64)
    ...
    def _run_phase5_final_optimization(self, obj: TLUObjective, params_list: list[np.ndarray]) -> None:
        ...
        try:
            ...
            # Phase 5b: Nelder-Mead (sans bornes SciPy) -> projection obligatoire dans la boîte.
            ...
            if final_mse < self.best_mse:
                ...
                self.logger.info(
                    ...
                    f" Final optimization improved RMSE: {np.sqrt(final_mse):.6f}"
                ...
                self.logger.info(
                    ...
                    f" Final optimization complete (RMSE unchanged: {np.sqrt(self.best_mse):.6f})"
        ...
        try:
            ...
            if res.fun < self.best_mse:
                ...
                self.logger.info(
                    ...
                    f" Polish complete | RMSE: {rmse_polish:.6f} | Evals: {obj.n_evals}"
            ...
            if not self.is_stopped:
                ...
                try:
                    ...
                    exclude_range_p3 = (
                        ...
                        (c.exclude_min, c.exclude_max) if c.exclude_min is not None else None
                    ...
                    # Bounds +/-5% around current solution, intersectées avec les bornes globales TLU
            ...
            if not self.is_stopped:
                ...
                try:
                    ...
                    self.progress.emit(100, "Uncertainty calculationation done.", None)
            ...
            if thickness_initial is None:
                ...
                if hasattr(self, "tlu_results"):
                    ...
                    self.logger.warning(
                        ...
                        "OptimizationWorker: thickness_initial not set  variation will be 0."
            ...
            thickness_variation = (
                ...
                (thickness_optimized - thickness_initial) / thickness_initial
            ...
            results = self._package_results(
                ...
                thickness_optimized, tlu_params, params_list=params_list
            ...
            try:
                ...
                warnings_list = (
                    ["Input data normalized before optimization."]
                    if bool(getattr(c, "use_normalized", False))
                    else []
                )
    ...
    def _package_results(
        ...
        R_calc, T_calc, T_sub = _compute_RT_from_config(

            c, l_full, n_calc, k_calc, thickness, n_sub
    ...
    def run(self):
        ...
        try:
            ...
            mask = (c.target_data["lambda"] >= c.lambda_min) & (
                ...
                c.target_data["lambda"] <= c.lambda_max
...
class ResultRecapWidget(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        lbl_head.setStyleSheet(f"color: {CertusTheme.TEXT_SUB}; font-weight: 800; letter-spacing: 2px; font-size: 13px;")
    ...
    def update_results(self, thickness, rmse, eg, eps_inf):
        # Thickness
...
class CertusIndexApp(CertusBaseApp):
    ...
    def _warmup_numba(self):
        ...
        try:
            ...
            calculate_transmission_array(wls, n_test, k_test, 100.0, n_test)

            calculate_reflection_array(wls, n_test, k_test, 100.0, n_test)

            calculate_RT_single_layer_backside_array(wls, n_test, k_test, 100.0, n_test)

            calculate_reflection_infinite_substrate_array(wls, n_test, k_test, 100.0, n_test)

            calculate_R_substrate_array(wls, n_test)

            calculate_R_frosted_glass_reference(wls, n_test)

            # Absorbing substrate kernels (sapphire / user k_sub)

            k_sub_test = np.array([1e-4, 1e-4])

            calculate_R_substrate_absorbing_array(wls, n_test, k_sub_test, 1.0e6)

            calculate_T_substrate_absorbing_array(wls, n_test, k_sub_test, 1.0e6)
    ...
    def _apply_theme(self):
        ...
        try:
            ...
            if hasattr(self, "_live_curve_T"):
                ...
                self._live_curve_T.setPen(

                    color=CertusTheme.SECONDARY, width=2, style=Qt.PenStyle.DashLine

                )

            if hasattr(self, "_live_curve_R"):

                self._live_curve_R.setPen(

                    color=CertusTheme.ACCENT, width=2, style=Qt.PenStyle.DashLine

                )

            if hasattr(self, "_live_curve_n"):

                self._live_curve_n.setPen(

                    color=CertusTheme.PRIMARY, width=2, style=Qt.PenStyle.DashLine

                )

            if hasattr(self, "_live_curve_k"):

                self._live_curve_k.setPen(

                    color=CertusTheme.DANGER, width=2, style=Qt.PenStyle.DashLine
    ...
    def _build_ui(self):
        ...
        self.plot_convergence = CertusScientificPlot(
            self, "Convergence — RMSE vs Iteration", "RMSE", "Iteration"
        )
        ...
        # (Index 2 = "Final Equations" was hiding the updated graphs.)
    ...
    def _create_input_group(self):
        ...
        self.lbl_file.setStyleSheet(
            ...
            f"font-style: italic; color: {CertusTheme.TEXT_SUB}; font-size: 11px;"
        ...
        self.lbl_data_type.setStyleSheet(

            f"color: {CertusTheme.PRIMARY}; font-weight: bold; font-size: 11px;"
    ...
    def _create_substrate_group(self):
        ...
        self.cb_sub.setToolTip(
            ...
            "Al2O3: example/sapphire fresnel.xlsx. Absorbing mode only if the file contains a k(\u03bb) column.\n"
        ...
        self.lbl_frosted_info.setStyleSheet(

            f"color: {CertusTheme.TEXT_SUB}; font-size: 10px; font-style: italic;"
        ...
        self.chk_absorbing_sub.setToolTip(
            ...
            "Al2O3: available only if sapphire fresnel.xlsx contains a k column; otherwise transparent substrate."
    ...
    def _on_substrate_mode_changed(self):
    ...
    def _on_substrate_changed(self, index: int):
        ...
        if is_sapphire:
            ...
            if _SAPPHIRE_WLS is not None:
                ...
                if _SAPPHIRE_FILE_HAS_K_COLUMN:
                    ...
                    self.lbl_ksub_file.setText(

                        "example/sapphire fresnel.xlsx  no k column: transparent only"
    ...
    def _on_absorbing_sub_toggled(self, checked: bool):

        self._absorbing_sub_widget.setVisible(checked)

    def _on_import_ksub(self):

        path = certus_get_open_file_name(

            self, "Import k_sub substrate", "CSV (*.csv);;All (*)"
    ...
    def _auto_detect_from_file(self, filepath: str):
        ...
        try:
            ...
            except (ValueError, TypeError, RuntimeError, AttributeError, KeyError, IndexError, FileNotFoundError) as e_fft:
            ...
            except (ValueError, TypeError, RuntimeError, AttributeError, KeyError, IndexError, FileNotFoundError) as e_pk:
    ...
    def _update_plot_exclusion(self):
    ...
    def _toggle_oh_band(self, checked):
    ...
    def load_file(self, filepath=None):
        ...
        try:
            ...
            # Utilisation du standard unifié pour charger et nettoyer (sort, normalise %, nm, etc)

            res = load_spectrum_columns(filepath, column_roles={})

            df = res.dataframe

            # Automatic data type analysis (utilise les en-têtes d'origine préservés par column_roles={})
            ...
            self.lbl_file.setStyleSheet(f"color: {CertusTheme.SUCCESS}; font-weight: bold;")
            ...
            self.plot_spectrum.plotItem.setTitle(f"Spectrum      {src_name}")

            # Log file loading details (absolute path for traceability)

            _fp_abs = Path(filepath).resolve()

            self.logger.info("-" * 60)

            self.logger.info("[FILE] Spectrum loaded: %s", _fp_abs)

            self.logger.info("[FILE] Name: %s", Path(filepath).name)

            self.logger.info("[FILE] Dimensions: %d rows × %d columns", df.shape[0], df.shape[1])
            ...
            type_label = type_labels[self.data_type]

            self.lbl_data_type.setText(type_label)

            self.logger.info(f"Data analysis: {type_label}")

            # Update lambda bounds

            lmin, lmax = (

                self.target_data["lambda"].min(),

                self.target_data["lambda"].max(),

            )

            self.sb_lmin.setValue(lmin)

            self.sb_lmax.setValue(lmax)

            self.logger.info(f"Spectral range: {lmin:.1f} - {lmax:.1f} nm")
            ...
            if os.environ.get("QT_QPA_PLATFORM", "").lower() != "offscreen":
                ...
                summary = build_summary_plain_text(
                    ...
                    [
                        ...
                        (f"Transmission column present: {'yes' if has_t else 'no'}", not has_t and self.data_type != DataType.REFLECTION),

                        (f"Reflection column present: {'yes' if has_r else 'no'}", not has_r and self.data_type != DataType.TRANSMISSION),
            ...
            # This section specifically addresses high-frequency noise typically introduced

            # by atmospheric absorption (CO2, H2O) in the IR band (4000nm - 5200nm+).

            # Instead of a single harsh filter that might destroy interference fringes in the VIS/NIR,

            # we use a "Progressive Blended Savitzky-Golay" approach.
            ...
            try:
                ...
                if "T" in self.target_data.columns and _preview_t:

                    _cols_ir_smooth.append("T")

                if "R" in self.target_data.columns and _preview_r:

                    _cols_ir_smooth.append("R")

                for col in _cols_ir_smooth:

                    if col in self.target_data.columns:

                        y = self.target_data[col].values

                        # 1. Apply two levels of Savitzky-Golay filtering:

                        # - y_smooth1: Light filtering (window=11) for the onset of noise (around 4000nm)

                        # - y_smooth2: Aggressive filtering (window=51) for heavy noise (> 5200nm)

                        # polyorder=2 ensures that peaks and valleys (extrema) are not flattened out too much.
                        ...
                        # 2. Define operational zones for the smoothing

                        # Noise typically starts at 4000nm and becomes severe at 5200nm.
                        ...
                        if np.any(mask_transition) or np.any(mask_heavy):
                            ...
                            # 3. Apply Linear Blending (Cross-fading) for a perfectly smooth C0/C1 curve:

                            # At 4000nm: Weight is 0% (uses only light filtering `y_smooth1`)

                            # At 5200nm: Weight is 100% (uses only heavy filtering `y_smooth2`)

                            # This prevents any sudden jumps or discontinuities in the spectral data.

                            if np.any(mask_transition):

                                weights = (wls[mask_transition] - 4000) / (5200 - 4000)

                                y_final[mask_transition] = (1 - weights) * y_smooth1[mask_transition] + weights * y_smooth2[mask_transition]

                            # For anything past 5200nm, use the heavy filter exclusively
                ...
                if smoothed_any:
                    ...
                    # 4. Interactive Visual Confirmation

                    # We briefly clear the plot to overlay both the raw and the smoothed data.
                    ...
                    # 1. Plot Raw data (semi-transparent/lighter)

                    if _preview_t and "T" in raw_data.columns:

                        self.plot_spectrum.plot(

                            wls, raw_data["T"].values * 100, 

                            pen=pg.mkPen(color=(40, 167, 69, 100), width=1), 

                            symbol="o", symbolSize=2, symbolBrush=pg.mkBrush(color=(40, 167, 69, 100)),

                            name="T data (Raw)"

                        )

                    if _preview_r and "R" in raw_data.columns:

                        self.plot_spectrum.plot(

                            wls, raw_data["R"].values * 100, 

                            pen=pg.mkPen(color=(220, 53, 69, 100), width=1), 

                            symbol="s", symbolSize=2, symbolBrush=pg.mkBrush(color=(220, 53, 69, 100)),

                            name="R data (Raw)"

                        )

                    # 2. Plot Smoothed data (thick and prominent)

                    if _preview_t and "T" in smoothed_data.columns:

                        c_t_sm = self.plot_spectrum.plot(

                            wls, smoothed_data["T"].values * 100, 
                        ...
                        try: self.plot_spectrum.add_tracked_curve(c_t_sm, "T", "%")

                        except (ValueError, TypeError, RuntimeError, AttributeError, KeyError, IndexError, FileNotFoundError): pass

                    if _preview_r and "R" in smoothed_data.columns:

                        c_r_sm = self.plot_spectrum.plot(

                            wls, smoothed_data["R"].values * 100, 
                        ...
                        try: self.plot_spectrum.add_tracked_curve(c_r_sm, "R", "%")

                        except (ValueError, TypeError, RuntimeError, AttributeError, KeyError, IndexError, FileNotFoundError): pass

                    # 5. Timer-Driven Dialog

                    # Ask the user if they want to keep the smoothed or the raw data.

                    # Defaults to "Smoothed" after 5 seconds to not block an automated workflow.

                    msg_box = QMessageBox(self)

                    msg_box.setWindowTitle("Noise Filtering")

                    msg_box.setText("IR Noise detected in 4000-5200nm band.\\nDo you want to keep the smoothed response or the raw response?\\n\\nIf no selection is made, the smoothed response will be kept after 5 seconds.")

                    btn_smooth = msg_box.addButton("Keep Smoothed", QMessageBox.ButtonRole.AcceptRole)

                    btn_raw = msg_box.addButton("Keep Raw", QMessageBox.ButtonRole.RejectRole)

                    msg_box.setDefaultButton(btn_smooth)

                    # Start a 5-second timer connected to the 'accept' slot of the MessageBox
                    ...
                    # 6. Apply Selection and Cleanup Plot

                    if msg_box.clickedButton() == btn_raw:

                        self.target_data = raw_data

                        # The user chose Raw: Clear the dual-plot and redraw just the raw data normally
                        ...
                        if _preview_t and "T" in self.target_data.columns:
                            ...
                            c_t = self.plot_spectrum.plot(wls, self.target_data["T"].values * 100, pen=None, symbol="o", symbolSize=3, symbolBrush=CertusTheme.SUCCESS, name="T data")

                            try: self.plot_spectrum.add_tracked_curve(c_t, "T", "%")

                            except (ValueError, TypeError, RuntimeError, AttributeError, KeyError, IndexError, FileNotFoundError): pass

                        if _preview_r and "R" in self.target_data.columns:

                            c_r = self.plot_spectrum.plot(wls, self.target_data["R"].values * 100, pen=None, symbol="s", symbolSize=3, symbolBrush=CertusTheme.DANGER, name="R data")

                            try: self.plot_spectrum.add_tracked_curve(c_r, "R", "%")

                            except (ValueError, TypeError, RuntimeError, AttributeError, KeyError, IndexError, FileNotFoundError): pass

                    else:

                        self.target_data = smoothed_data

                        # The user chose Smoothed (or timer expired): Clear the dual-plot and draw just the smoothed data
                        ...
                        if _preview_t and "T" in self.target_data.columns:
                            ...
                            c_t_final = self.plot_spectrum.plot(wls, self.target_data["T"].values * 100, pen=None, symbol="o", symbolSize=3, symbolBrush=CertusTheme.SUCCESS, name="T data")

                            try: self.plot_spectrum.add_tracked_curve(c_t_final, "T", "%")

                            except (ValueError, TypeError, RuntimeError, AttributeError, KeyError, IndexError, FileNotFoundError): pass

                        if _preview_r and "R" in self.target_data.columns:

                            c_r_final = self.plot_spectrum.plot(wls, self.target_data["R"].values * 100, pen=None, symbol="s", symbolSize=3, symbolBrush=CertusTheme.DANGER, name="R data")

                            try: self.plot_spectrum.add_tracked_curve(c_r_final, "R", "%")
        ...
        def _resolve_substrate_absorption_inputs(
            ...
            *,
            ...
            if is_sapphire:
                if _SAPPHIRE_WLS is None:
                    self._abort_run_optimization(
                        "Sapphire reference missing",
                        "example/sapphire fresnel.xlsx not found.\n"
                        "Calculation blocked for Al2O3.",
                        critical=True,
                    )
                    self.logger.error("[SAPPHIRE] example/sapphire fresnel.xlsx missing -> run blocked.")
                    return None

                n_sub_data = _get_sapphire_n_on_grid(wls_target)
                if n_sub_data is None or not np.all(np.isfinite(n_sub_data)):
                    self._abort_run_optimization(
                        "Sapphire data invalid",
                        "example/sapphire fresnel.xlsx is present but invalid for n(lambda).\n"
                        "Calculation interrupted to avoid drift on Al2O3.",
                        critical=True,
                    )
                    return None
                ...
                if not _SAPPHIRE_FILE_HAS_K_COLUMN:
                    if thickness_mm > 0.0 or self.chk_absorbing_sub.isChecked():
                        ...
                        return None
                    self.logger.info("[SAPPHIRE] No k column in the file -> transparent substrate (k=0).")
                    return None, None, n_sub_data
                ...
                if thickness_mm > 0:
                    ...
                    self.logger.info("[SAPPHIRE] Thickness = 0 -> transparent substrate (k=0 everywhere).")
            ...
            if self.chk_absorbing_sub.isChecked():
                ...
                if thickness_mm <= 0:
                    self.logger.info(
                        "Absorbent substrate: thickness = 0 -> transparent substrate (k=0 everywhere)."
                    )
                else:
    ...
    def run_optimization(self):
        ...
        substrate_mode = (
            ...
            substrateMode.FROSTED_GLASS if is_frosted_glass else substrateMode.STANDARD
        ...
        if substrate_name == "Sapphire (Al2O3)":
            ...
            self.logger.info(
                ...
                "[FILE] Substrate Al2O3: %s | k column: %s",
    ...
    def _on_evals_update(self, n_evals: int):
    ...
    def _make_index_tlu_live_ctx(self, c: OptimizationConfig) -> dict | None:
        ...
        try:
            ...
            effective_lambda_max = (
                ...
                else min(float(_lmf), float(c.lambda_max))
    ...
    def _index_tlu_live_payload_from_params(self, params: np.ndarray) -> dict | None:
        ...
        R_calc, T_calc, T_sub_ref = _compute_RT_from_config(

            c, wls, n_calc, k_calc, thickness, n_sub
    ...
    def _apply_index_live_plot_payload(self, extra_info: dict) -> None:
        ...
        c_n = self.plot_nk.plot(

            wls, n_c, pen=pg.mkPen(color="#3b82f6", width=3), name="n (Live)"
        ...
        if not hasattr(self, "_vb_k") or self._vb_k is None:
            ...
            pi.vb.sigResized.connect(
                ...
                lambda: self._vb_k.setGeometry(pi.vb.sceneBoundingRect())
    ...
    def _on_progress(self, step: int, phase: str, extra_info=None):
        ...
        if isinstance(extra_info, dict) and "n" in extra_info and "k" in extra_info and "wls" in extra_info:
            ...
            try:
                ...
                if getattr(self, "latest_results", None) is not None:
                    ...
                    d_val = int(round(self.latest_results.thickness)) if hasattr(self.latest_results, 'thickness') and self.latest_results.thickness else 0
            ...
            self.lbl_status.setText(
                ...
                f"{phase} (Best RMSE: {self._best_rmse_display:.4f})"
    ...
    def _on_curve_update(self, params):
    ...
    def _update_spectrum_plot(self, wls, sub_df, res: OptimizationResults):
        ...
        if (
            ...
            res.config.data_type in (DataType.REFLECTION, DataType.BOTH)

            or res.config.is_frosted_glass
    ...
    def _update_nk_plot(self, wls, sub_df, res: OptimizationResults):
        ...
        # Verify n_calc/k_calc exist (not all NaN)

        if "n_calc" not in sub_df.columns or "k_calc" not in sub_df.columns:

            self.logger.error(

                f"Missing n_calc or k_calc in sub_df. Columns: {list(sub_df.columns)}"
        ...
        # Visual Mask for Phase 1 (TLU): Don't plot the unphysical IR extrapolation

        method_str = res.optimization_stats.get("method", "")

        lambda_max_fit = getattr(res.config, "lambda_max_fit", None)

        if lambda_max_fit is not None and "Spline" not in method_str and res.tlu_params is not None:

            # It's the end of Phase 1. Mask wavelengths beyond the fit boundary.
        ...
        try:
            ...
            if "n_calc_005" in sub_df.columns:
                ...
                c_n = self.plot_nk.plot(
                    ...
                    wls, n_values, pen=pg.mkPen(CertusTheme.PRIMARY, width=3), name="n (R+T)"
                ...
                if "n_center" in sub_df.columns and "n_hi" in sub_df.columns and "n_lo" in sub_df.columns:
                    ...
                    c_nc = self.plot_nk.plot(
                        ...
                        wls, n_cen, pen=pg.mkPen(color=CertusTheme.SUCCESS, width=2, style=Qt.PenStyle.DashLine), name="n (center)"
                    ...
                    if "n_hi_2" in sub_df.columns and "n_lo_2" in sub_df.columns:
                        ...
                        err_n2 = pg.ErrorBarItem(x=wls, y=n_err_center, top=top_n2, bottom=bot_n2, beam=0.5, pen=pg.mkPen(color=(30, 136, 229, 80), width=3))

                        self.plot_nk.plotItem.addItem(err_n2)

                    err_n = pg.ErrorBarItem(x=wls, y=n_err_center, top=top_n, bottom=bot_n, beam=0.5, pen=pg.mkPen(CertusTheme.PRIMARY, width=1))
                    ...
                    if "n_raw" in sub_df.columns:
                        ...
                        c_raw = self.plot_nk.plot(
                            ...
                            wls, n_err_center, pen=pg.mkPen(color=(30, 136, 229, 120), width=1, style=Qt.PenStyle.DotLine), name="n (Raw point-by-point)"
                ...
                if "n_fit_T_only" in sub_df.columns:
                    ...
                    c_nt = self.plot_nk.plot(wls, sub_df["n_fit_T_only"].values,
                ...
                if "n_fit_R_only" in sub_df.columns:
                    ...
                    c_nr = self.plot_nk.plot(wls, sub_df["n_fit_R_only"].values,
            ...
            if "k_calc_005" in sub_df.columns:
                ...
                k_plot = np.where(np.isfinite(k_values) & (k_values >= 1e-8), np.log10(np.maximum(k_values, 1e-7)), -7.0)

                c_k = pg.PlotCurveItem(

                    wls, k_plot, pen=pg.mkPen(CertusTheme.WARNING, width=3), name="k (R+T)"
                ...
                if "k_center" in sub_df.columns and "k_hi" in sub_df.columns and "k_lo" in sub_df.columns:
                    ...
                    c_kc = pg.PlotCurveItem(
                        ...
                        wls, kc_plot, pen=pg.mkPen(color=CertusTheme.WARNING, width=2, style=Qt.PenStyle.DashLine), name="k (center)"
                    ...
                    k_err_center_plot = np.where(np.isfinite(k_err_center) & (k_err_center >= 0), np.maximum(k_err_center, 1e-7), np.nan)
                    ...
                    if "k_hi_2" in sub_df.columns and "k_lo_2" in sub_df.columns:
                        ...
                        err_k2 = pg.ErrorBarItem(x=wls, y=k_err_center_plot, top=top_k2, bottom=bot_k2, beam=0.5, pen=pg.mkPen(color=(255, 179, 0, 80), width=3))

                        self._vb_k.addItem(err_k2)

                    err_k = pg.ErrorBarItem(x=wls, y=k_err_center_plot, top=top_k, bottom=bot_k, beam=0.5, pen=pg.mkPen(CertusTheme.WARNING, width=1))
            ...
            if "k_fit_T_only" in sub_df.columns:
                ...
                c_kt = pg.PlotCurveItem(wls, kt_p,
            ...
            if "k_fit_R_only" in sub_df.columns:
                ...
                c_kr = pg.PlotCurveItem(wls, kr_p,
            ...
            if "k_raw" in sub_df.columns:
                ...
                c_k_raw = pg.PlotCurveItem(
                    ...
                    wls, k_err_center_plot, pen=pg.mkPen(color=(255, 179, 0, 120), width=1, style=Qt.PenStyle.DotLine), name="k (Raw point-by-point)"
    ...
    def _update_model_text(self, res: OptimizationResults):
        ...
        try:
            ...
            if hasattr(self, "lbl_final_eq"):
                ...
                if hasattr(res, "sellmeier_params") and res.sellmeier_params is not None:
                    ...
                    eq_html += "<i>n2 = A + (B1lambda2) / (lambda2 - C1) + (B2lambda2) / (lambda2 - C2)</i> (lambda in m)<br>"
            ...
            try:
                ...
                if hasattr(res, "sellmeier_params") and res.sellmeier_params is not None:
                    ...
                    params_rows.extend([
                        ...
                        ("B1", f"{sp[1]:.6f}"), ("C1 (m2)", f"{sp[2]**2:.6f}"),

                        ("B2", f"{sp[3]:.6f}"), ("C2 (m2)", f"{sp[4]**2:.6f}")
                ...
                if hasattr(res, "k_8p_params") and res.k_8p_params is not None:
                    ...
                    names = ["P1 (Slope1)", "P2 (Pos1)", "P3 (Slope2)", "P4 (Pos2)", "P5 (Amp)", "P6 (Center)", "P7 (Width)", "P8 (Beta Exponent)"]
    ...
    def _update_data_table(self, sub_df, res: OptimizationResults):
                ...
                if res.config.is_frosted_glass:
                    ...
                    for i in range(len(sub_df)):
                        ...
                        self.table_res.setItem(
                            ...
                            i, col_idx, QTableWidgetItem(f"{row_data['lambda']:.1f}")
                        ...
                        self.table_res.setItem(
                            ...
                            i, col_idx, QTableWidgetItem(f"{row_data['n_calc']:.4f}")
                        ...
                        self.table_res.setItem(
                            ...
                            i, col_idx, QTableWidgetItem(f"{row_data['k_calc']:.6f}")
    ...
    def _display_results(self, res: OptimizationResults):
        ...
        try:
            ...
            if sub_df.empty:
                ...
                self.logger.error(f"sub_df is empty after filtering! lambda_min={res.config.lambda_min}, lambda_max={res.config.lambda_max}")
    ...
    def _copy_nk_to_clipboard(self):
    ...
    def _copy_eq_to_clipboard(self):
    ...
    def _on_finished(self, res: OptimizationResults):
                ...
                if "lambda" in df.columns and "n_calc" in df.columns and "k_calc" in df.columns:
                    ...
                    if res.tlu_params is not None:
                        ...
                        try:
                            ...
                            from certus_physics import epsilon2_TLU_array, epsilon1_TL_analytic, epsilon_to_nk, HC_EV_NM
                    ...
                    n_fit_global, params_n = fit_sellmeier_global(wls, n_exp, material=res.config.substrate, valid_mask=valid_mask)
    ...
    def _on_tlu_constrained_finished(self, tlu_res: OptimizationResults):
        ...
        self._worker2 = IRGlobalModelWorker(

            tlu_res.config, tlu_res, logger=self.logger
    ...
    def _build_uncertainty_content(self, df: pd.DataFrame) -> dict[str, str]:
        ...
        if "delta_n_res" in df.columns:
            ...
            uncertainty_content["Deltan Min (3-way)"] = (

                f"{np.min(dn[dn > 0]):.4f}" if np.any(dn > 0) else "0.0000"
        ...
        try:
            ...
            if res.tlu_params:
                ...
                disp_data = [
                    ...
                    {"Parameter": "Mode", "Value": "IR Global Model (Phase 2/2)" if is_phase2_ir else "Spline Refinement", "Unit": "-"},
            ...
            if hasattr(res, "k_8p_params") and res.k_8p_params is not None:
                ...
                kp_names = ["k_P1_Slope1", "k_P2_Pos1", "k_P3_Slope2", "k_P4_Pos2", "k_P5_Amp", "k_P6_Center", "k_P7_Width", "k_P8_Beta"]
            ...
            sections = [
...

                {
                    ...
                    "content": {
                        ...
                        "Model": "IR Global Model (Sellmeier + k 8p)" if (res.tlu_params is None and ("PGLOBAL" in (res.optimization_stats or {}).get("method", "") or "Sellmeier+k" in (res.optimization_stats or {}).get("method", ""))) else "Tauc-Lorentz-Urbach",
    ...
    def export_results(self):
        ...
        try:
            ...
            if res.config.substrate == "Sapphire (Al2O3)":
                ...
                summary_data["Sapphire index file (n, path)"] = [str(Path(_SAPPHIRE_DATA_FILE).resolve())]
            ...
            if hasattr(res, "k_8p_params") and res.k_8p_params is not None:
                ...
                kp_names = ["k_P1_Slope1", "k_P2_Pos1", "k_P3_Slope2", "k_P4_Pos2",

                            "k_P5_Amp", "k_P6_Center", "k_P7_Width", "k_P8_Beta"]
            ...
            try:
                if bool(getattr(res.config, "use_normalized", False)):
                    ...
                    self.add_validation_warning(
                        "Input data normalized before optimization/export."
                    )
                else:
    ...
    def detach_current_plot(self):
        ...
        # 2. OPTION : TAB N_K (Index 1) -> Split into two windows!
            ...
            if "nk_k" not in self.detached_plot_windows or not self.detached_plot_windows["nk_k"].isVisible():
                ...
                k_clone = CertusScientificPlot(None, title="Extinction Coefficient k", 

                                             y_label="k", x_label="lambda (nm)",
    ...
    def _collect_config(self) -> dict:
        ...
        return {
            ...
            "model_type": (
                ...
                self.model_combo.currentText() if hasattr(self, "model_combo") else "TLU"
...
if __name__ == "__main__":
    ...
    setup_logging(log_file="certus_index.log")
