...
class TestStratCorridorSensitivity:
    ...
    def test_corridor_min_k_half_width_is_enforced_and_counts_changes(self):
        ...
        assert np.all(hi >= lo)
        assert hi[0] - lo[0] >= 1e-4 - 1e-6 - 1e-12
        assert hi[1] - lo[1] >= 1e-4 - 1e-12
    ...
    def test_corridor_package_detects_n_k_crossing_and_keeps_bounds(self, monkeypatch):
        from certus.spline.spline_profile_corridors import CorridorProfileContext, _package_corridor_results
        import certus.spline.spline_profile_corridors as mod
        warns = []
        monkeypatch.setattr(mod.log, "warning", lambda msg, *args: warns.append(msg))
        ...
        assert any("n/k CROSSING DETECTED" in w for w in warns)
    ...
    def test_corridor_d_interval_clips_to_bounds_when_needed(self):
        ...
        ctx = CorridorProfileContext(
            ...
            n_curves=[np.array([1.0]), np.array([1.0]), np.array([1.0])],
            nom_pack=None,
            pconf=type("PConf", (), {"parabola_half_window_pts": 3, "symmetric_interval_center_mode": "nominal", "force_symmetric_interval": True, "symmetric_interval_min_half_width_rel": 0.5, "mode": "alpha", "rmse_alpha": 1.0, "lr_conf_level": 0.95})(),
            rmse_opt=0.1,
            rmse_ref_tag="best_polished",
            rmse_thr_sub="alpha",
            rmse_thresh=0.2,
        ...
        d_raw, d_sym, src, _fit = _derive_symmetric_d_interval(ctx, np.array([4.9, 5.0, 5.1]), np.array([0.3, 0.1, 0.2]))
        assert d_raw == (4.9, 5.1)
