...
import numpy as np
import pandas as pd
from certus.core.certus_core import NUMERICAL_FAULT_EXCEPTIONS, HC_EV_NM, N_MIN_LIMIT, PI, SMALL_EPSILON, T_SUB_MIN_R_NORM, T_SUB_MIN_T_NORM, __version__, get_resource_path, get_safe_worker_count
...
from certus_physics import PGlobalConfig, TLUParameters, calculate_single_interface_R, calculate_reflection_array, calculate_bare_substrate_R, calculate_bare_substrate_R_absorbing, calculate_bare_substrate_RT, calculate_bare_substrate_T_absorbing, calculate_RT_single_layer_absorbing_substrate_array, calculate_RT_single_layer_backside_array, clip_to_bounds, compute_mse_vectorized, epsilon1_TL_analytic, epsilon2_TLU_array, epsilon_to_nk, get_n_frosted_glass_array, SplineBasisCache
from certus.utils.certus_index_utils import sellmeier_2poles_eval_nj, k_law_8p_eval, _deduce_knots_from_k8p, _ensure_strictly_increasing, _merge_closest_knot_pair, fit_sellmeier_global, fit_k_global_8p, DataType, _get_substrate_n_array_index, calculate_index_rmse
from certus.core.certus_metrology import ValidationStatus
from certus.utils.certus_services import IndexFitService, IndexFitRequest
from certus.core.certus_index_core import IRGlobalObjective, Phase23SplineObjective, Phase23Pass2SplineObjective, TLUObjective, OptimizationConfig, OptimizationResults, PGlobalOptimizerINDEX, SubsetOptimTask, calculate_relative_R_normalization, _SAPPHIRE_DATA_FILE, _SAPPHIRE_FILE_HAS_K_COLUMN, estimate_initial_params

class IRPGlobalCallback:

    def __init__(self, worker, opt_instance, obj, c, l_full, thickness, n_sub_full, title='IR Global Search', emit_plot=True) -> None:
        self.worker = worker
    ...
    def __call__(self, sample) -> None:
        ...
        if not self.emit_plot:
            self.worker.emit_progress_snapshot(build_progress_snapshot(message=self.title, sub_message='IR global search', progress_ratio=0.90, display_ratio=0.90, eta_seconds=None, confidence=0.25, module='INDEX', phase='IRPGLOBAL', metadata={'best_mse': float(self.state[0])}))
            self.worker.emit_progress_snapshot(build_progress_snapshot(message=self.title, sub_message='IR global search', progress_ratio=0.90, display_ratio=0.90, eta_seconds=None, confidence=0.25, module='INDEX', phase='IRPGLOBAL', metadata={'best_mse': float(self.state[0])}))
            return
        ...
        plot_data = {'n': n, 'k': k, 'wls': self.l_full, 'R_calc': R_v, 'T_calc': T_v, 'is_frosted_glass': self.c.is_frosted_glass, 'live_show_T': _st, 'live_show_R': _sr, 'mse': self.state[0]}
        ...
        self.worker.emit_progress_snapshot(build_progress_snapshot(message=self.title, sub_message='IR global search', progress_ratio=0.90, display_ratio=0.90, eta_seconds=None, confidence=0.25, module='INDEX', phase='IRPGLOBAL', metadata={'n': n, 'k': k, 'mse': float(self.state[0]), 'plot_data': plot_data}))
    ...
    def __call__(self, xk) -> None:
        ...
        try:
            ...
            if self.c.is_frosted_glass:
                T_v = np.full_like(self.l_full, np.nan)
            _st, _sr = _index_live_spectrum_visibility(self.c)
            self.worker.emit_progress_snapshot(build_progress_snapshot(message='Stage 2 polish', sub_message='IR stage 2', progress_ratio=0.91, display_ratio=0.91, eta_seconds=None, confidence=0.25, module='INDEX', phase='IRSTAGE2', metadata={'n': n, 'k': k, 'mse': None}))
            self.worker.emit_progress_snapshot(build_progress_snapshot(message='Stage 2 polish', sub_message='IR stage 2', progress_ratio=0.91, display_ratio=0.91, eta_seconds=None, confidence=0.25, module='INDEX', phase='IRSTAGE2', metadata={'n': n, 'k': k, 'mse': None, 'R_calc': R_v, 'T_calc': T_v, 'live_show_T': _st, 'live_show_R': _sr}))
        except NUMERICAL_FAULT_EXCEPTIONS:
            logging.getLogger('CERTUS').debug('Silenced exception in %s', __name__, exc_info=True)

class IRSplineCallback:

    def __init__(self, worker, obj, c, l_full, thickness, n_sub_full, title, phase23_obj=None, knot_lam=None, n_k=None, lk_lo=None, lk_hi=None, t0=None) -> None:
        self.worker = worker
        self.obj = obj
        self.c = c
        self.l_full = l_full
    ...
    def get_plot_data(self, xk) -> dict | None:
        try:
            ...
            if self.c.is_frosted_glass:
                T_v = np.full_like(self.l_full, np.nan)
            _st, _sr = _index_live_spectrum_visibility(self.c)
            return {'n': n, 'k': k, 'wls': self.l_full, 'R_calc': R_v, 'T_calc': T_v, 'is_frosted_glass': self.c.is_frosted_glass, 'live_show_T': _st, 'live_show_R': _sr, 'mse': None}
        except NUMERICAL_FAULT_EXCEPTIONS:
            return None
    ...
    def __call__(self, xk) -> bool:
        if time.time() - self._last_plot >= 1.5:
            ...
            if pd is not None:
                self.worker.emit_progress_snapshot(build_progress_snapshot(message=self.title, sub_message='IR spline', progress_ratio=0.99, display_ratio=0.99, eta_seconds=None, confidence=0.2, module='INDEX', phase='IRSPLINE', metadata={'mse': None}))
                self.worker.emit_progress_snapshot(build_progress_snapshot(message=self.title, sub_message='IR spline', progress_ratio=0.99, display_ratio=0.99, eta_seconds=None, confidence=0.2, module='INDEX', phase='IRSPLINE', metadata={'mse': None, 'plot_data': pd}))
...
class IRGlobalModelWorker(QObject):
    ...
    def _run_ir_stage0_to_stage2(self, c: OptimizationConfig, l_full: np.ndarray, n_sub_full: np.ndarray, df_tlu: pd.DataFrame, n_tlu_ref: np.ndarray, obj: IRGlobalObjective, thickness: float) -> tuple | None:
        from certus.workers.certus_index_workers_ir_strat import IRGlobalModelStrategy
        return IRGlobalModelStrategy._run_ir_stage0_to_stage2(self, c, l_full, n_sub_full, df_tlu, n_tlu_ref, obj, thickness)

    def _run_phase21_refinement(self, c: OptimizationConfig, obj: IRGlobalObjective, target_T: np.ndarray | None, k_spline_knots_lambda_um: np.ndarray | None, k_spline_knots_values: np.ndarray | None, p_opt_final: np.ndarray, res_pg, l_full: np.ndarray, thickness: float, n_sub_full: np.ndarray) -> tuple[np.ndarray | None, np.ndarray | None, np.ndarray | None]:
        from certus.workers.certus_index_workers_ir_strat import IRGlobalModelStrategy
        return IRGlobalModelStrategy._run_phase21_refinement(self, c, obj, target_T, k_spline_knots_lambda_um, k_spline_knots_values, p_opt_final, res_pg, l_full, thickness, n_sub_full)

    def _run_phase23_knot_reduction(self, *, c: OptimizationConfig, obj: IRGlobalObjective, res_pg, l_full: np.ndarray, thickness: float, n_sub_full: np.ndarray, log_k_lo: float, log_k_hi: float, lam_min: float, lam_max: float, min_knot_dist_um: float, initial_best_n: int, initial_best_mse: float, initial_best_knot_lam: np.ndarray, initial_best_log_k: np.ndarray, initial_best_p_sell: np.ndarray, n_final: np.ndarray, k_final: np.ndarray, p_opt_final: np.ndarray) -> tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray, float]:
        from certus.workers.certus_index_workers_ir_strat import IRGlobalModelStrategy
        return IRGlobalModelStrategy._run_phase23_knot_reduction(self, c=c, obj=obj, res_pg=res_pg, l_full=l_full, thickness=thickness, n_sub_full=n_sub_full, log_k_lo=log_k_lo, log_k_hi=log_k_hi, lam_min=lam_min, lam_max=lam_max, min_knot_dist_um=min_knot_dist_um, initial_best_n=initial_best_n, initial_best_mse=initial_best_mse, initial_best_knot_lam=initial_best_knot_lam, initial_best_log_k=initial_best_log_k, initial_best_p_sell=initial_best_p_sell, n_final=n_final, k_final=k_final, p_opt_final=p_opt_final)

    def _run_phase23_spline_refinement(self, c: OptimizationConfig, obj: IRGlobalObjective, res_pg, l_full: np.ndarray, thickness: float, n_sub_full: np.ndarray) -> tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray | None, np.ndarray | None]:
        from certus.workers.certus_index_workers_ir_strat import IRGlobalModelStrategy
    ...
    def run(self) -> None:
        try:
            ...
            self.logger.info('-' * 65)
            n_final, k_final, p_opt_final, k_spline_knots_lambda_um, k_spline_knots_values = self._run_phase23_spline_refinement(c, obj, res_pg, l_full, thickness, n_sub_full)
            n_T, k_T, p_opt_T = self._run_phase21_refinement(c, obj, target_T, k_spline_knots_lambda_um, k_spline_knots_values, p_opt_final, res_pg, l_full, thickness, n_sub_full)
            results = self._package_results(n_final, k_final, thickness, l_full, p_opt_final, n_T, k_T, p_opt_T, k_spline_knots_lambda_um=k_spline_knots_lambda_um, k_spline_knots_values=k_spline_knots_values)
            results.execution_time = duration
            try:
                status_val = ValidationStatus.WARNING_DATA_NORMALIZED if bool(getattr(c, 'use_normalized', False)) else ValidationStatus.OK
                warnings_list = ['Input data normalized before optimization.'] if bool(getattr(c, 'use_normalized', False)) else []
                svc = IndexFitService(runner=lambda _cfg: results)
                svc_resp = svc.fit(IndexFitRequest(config=c, source_paths=[str(getattr(c, 'source_file', '') or '')], seed=getattr(c, 'random_seed', None), app_id='CERTUS_INDEX', app_version=__version__, warnings=warnings_list, status=status_val))
            ...
            self.emit_progress_snapshot(build_progress_snapshot(message='Done (IR Refined)', sub_message='IR pipeline complete', progress_ratio=1.0, display_ratio=1.0, eta_seconds=0.0, confidence=1.0, module='INDEX', phase='IRDONE', metadata={'result_type': type(results).__name__}))
            self.emit_progress_snapshot(build_progress_snapshot(message='Done (IR Refined)', sub_message='IR pipeline complete', progress_ratio=1.0, display_ratio=1.0, eta_seconds=0.0, confidence=1.0, module='INDEX', phase='IRDONE', metadata={'result_type': type(results).__name__}))
    ...
    def _package_results(self, n, k, thickness, l_full, p_opt, n_T=None, k_T=None, p_T=None, k_spline_knots_lambda_um=None, k_spline_knots_values=None) -> Any:
        from certus.workers.certus_index_workers_ir_strat import IRGlobalModelStrategy
        return IRGlobalModelStrategy._package_results(self, n, k, thickness, l_full, p_opt, n_T, k_T, p_T, k_spline_knots_lambda_um, k_spline_knots_values)
...
class Phase1Callback:
    ...
    def __call__(self, s) -> None:
        ...
        if improved or now - self._last_status_emit >= 2.0:
            ...
            self.worker.emit_progress_snapshot(build_progress_snapshot(message='Global Search...', sub_message='Phase 1', progress_ratio=progress / 100.0, display_ratio=progress / 100.0, eta_seconds=None, confidence=0.3, module='INDEX', phase='PHASE1', metadata={'best_mse': float(self.worker.best_mse)}))
            self.worker.emit_progress_snapshot(build_progress_snapshot(message='Global Search...', sub_message='Phase 1', progress_ratio=progress / 100.0, display_ratio=progress / 100.0, eta_seconds=None, confidence=0.3, module='INDEX', phase='PHASE1', metadata={'best_mse': float(self.worker.best_mse)}))
...
class Phase2PolishCallback:
    ...
    def __call__(self, xk) -> None:
        ...
        self.worker.emit_progress_snapshot(build_progress_snapshot(message=f'Polish {self.polish_iters}', sub_message='Phase 2', progress_ratio=prog_polish / 100.0, display_ratio=prog_polish / 100.0, eta_seconds=None, confidence=0.3, module='INDEX', phase='PHASE2', metadata={'best_mse': float(self.worker.best_mse)}))
        self.worker.emit_progress_snapshot(build_progress_snapshot(message=f'Polish {self.polish_iters}', sub_message='Phase 2', progress_ratio=prog_polish / 100.0, display_ratio=prog_polish / 100.0, eta_seconds=None, confidence=0.3, module='INDEX', phase='PHASE2', metadata={'best_mse': float(self.worker.best_mse)}))
...
class OptimizationWorker(QObject):
    ...
    def _update_phase1_best(self, sample) -> None:
        ...
        if significant_improvement or time_elapsed:
            ...
            self.logger.info(f'  Best RMSE: {rmse:.6f} | Evaluations: {n_ev} | Thickness: {sample.x[0]:.2f} nm{_tlu_ex}{_k_inline}')
        if _o1 is None and (not getattr(self, '_warned_phase1_missing_obj', False)):
            self.logger.warning('Phase1 TLU diag/k unavailable (neither _phase1_obj nor _optimizer.objective) - run an up-to-date CERTUS_INDEX.py (e.g. folder 1904).')
    ...
    def _try_active_update(self, params, mse, force=False) -> None:
        ...
        if mse <= self.best_mse:
            ...
            if self.best_params is not None:
                ...
                self.logger.info(f"[LIVE VIEW] Emitted 5s live refresh signal | Index RMSE={np.sqrt(self.best_mse):.6f}")
    ...
    def _run_subset_optim(self, clues_slice, wls, n_sub, target_T, target_R, exclude_range) -> Any:
        ...
        return IndexOptimizationStrategy()._run_subset_optim(self, clues_slice, wls, n_sub, target_T, target_R, exclude_range)
    ...
    def _prepare_run_inputs(self, c: OptimizationConfig) -> tuple[np.ndarray, np.ndarray, np.ndarray | None, np.ndarray | None, tuple[float, float] | None, TLUObjective]:
        from certus.workers.certus_index_workers_opt_strat import IndexOptimizationStrategy
    ...
    def _run_phase1_global_search(self, c: OptimizationConfig, obj: TLUObjective, wls: np.ndarray, target_T: np.ndarray | None, n_sub: np.ndarray) -> bool:
        from certus.workers.certus_index_workers_opt_strat import IndexOptimizationStrategy
    ...
    def run(self) -> None:
        try:
            ...
            self.logger.info('TLU setup Phase1 | Eg_min(bounds)=%.4f eV | k_soft_ceiling=%.5g | prior_lame_claire(T)=%s | budget_logs_k_pen=%d | hÎ½_max=%.4f eV | n_lo_soft=%.3f', float(obj.param_bounds[0, 0]), float(getattr(obj, '_k_soft_ceiling', float('nan'))), getattr(obj, '_prior_transparent_low_k', False), int(getattr(obj, '_tlu_explode_logs_left', 0)), float(np.max(HC_EV_NM / np.maximum(wls, 1.0))), _setup_n_lo)
            if self._run_phase1_global_search(c, obj, wls, target_T, n_sub):
                return
            self.logger.info('\n--- PHASE 2: L-BFGS-B Local Polish ---')
            self.emit_progress_snapshot(build_progress_snapshot(message='Polish...', sub_message='Phase 3', progress_ratio=0.60, display_ratio=0.60, eta_seconds=None, confidence=0.35, module='INDEX', phase='PHASE3'))
            phase2_cb = Phase2PolishCallback(self)
            res = scipy.optimize.minimize(obj, self.best_params, method='L-BFGS-B', jac=obj.gradient, bounds=[(lb, ub) for lb, ub in obj.get_bounds()], callback=phase2_cb, options={'ftol': SMALL_EPSILON, 'gtol': SMALL_EPSILON, 'maxiter': 2000})
            ...
            if not self.is_stopped:
                ...
                self.emit_progress_snapshot(build_progress_snapshot(message='Fine tuning...', sub_message='Phase 3', progress_ratio=0.75, display_ratio=0.75, eta_seconds=None, confidence=0.35, module='INDEX', phase='PHASE3'))
                try:
                    exclude_range_p3 = (c.exclude_min, c.exclude_max) if c.exclude_min is not None else None
                    obj_p3 = TLUObjective(wls, target_T, target_R, n_sub, c.data_type, (c.thickness_min, c.thickness_max), use_normalized=c.use_normalized, weight_T=c.weight_T, weight_R=c.weight_R, exclude_range=exclude_range_p3, is_frosted_glass=c.is_frosted_glass)
                    ...
                    res_p3 = scipy.optimize.minimize(obj_p3, x0_p3, method='L-BFGS-B', jac=obj_p3.gradient, bounds=bounds_p3, options={'ftol': 1e-14, 'gtol': 1e-10, 'maxiter': 500})
            ...
            if self._finalize_if_stopped():
                return
            self.emit_progress_snapshot(build_progress_snapshot(message='Uncertainty (3-way split)...', sub_message='Phase 4', progress_ratio=0.90, display_ratio=0.90, eta_seconds=None, confidence=0.35, module='INDEX', phase='PHASE4'))
            params_list = []
            if not self.is_stopped:
                try:
                    ...
                    self.emit_progress_snapshot(build_progress_snapshot(message='Uncertainty calculation done.', sub_message='Phase 4', progress_ratio=1.0, display_ratio=1.0, eta_seconds=0.0, confidence=1.0, module='INDEX', phase='PHASE4', state=StepState.DONE))
            ...
            if self._finalize_if_stopped(params_list=params_list):
                return
            self.emit_progress_snapshot(build_progress_snapshot(message='Packaging results...', sub_message='Finalization', progress_ratio=0.95, display_ratio=0.95, eta_seconds=None, confidence=0.5, module='INDEX', phase='FINALIZE'))
            if self.best_params is not None:
                ...
                if float(np.max(np.abs(_xp - np.asarray(self.best_params, dtype=np.float64).ravel()))) > 1e-08:
                    self.logger.warning('TLU: best_params projected within bounds before synthesis (Nelder or sub-spectra average).')
            ...
            self.logger.info(f'TLU Parameters: Eg={tlu_params.Eg:.3f} eV, A={tlu_params.A:.3f}, E0={tlu_params.E0:.3f} eV')
            self.logger.info('=' * 80)
            results = self._package_results(thickness_optimized, tlu_params, params_list=params_list)
            self.emit_progress_snapshot(build_progress_snapshot(message='Done', sub_message='Complete', progress_ratio=1.0, display_ratio=1.0, eta_seconds=0.0, confidence=1.0, module='INDEX', phase='DONE', state=StepState.DONE))
            self._cleanup()
    ...
    def _package_results(self, thickness: float, tlu_params: TLUParameters, params_list: list | None=None) -> list[OptimizationResults]:
        from certus.workers.certus_index_workers_opt_strat import IndexOptimizationStrategy
...
class IndexBeamAnalysisWorker(QObject):
    ...
    def __init__(self, start_params: np.ndarray, config: OptimizationConfig, scan_range_nm: float=1.0, step_nm: float=0.1) -> None:
        super().__init__()
