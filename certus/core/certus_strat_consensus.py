            ...
            with executor_cls(max_workers=worker_count) as executor:
                futures_stage = [
                    executor.submit(
                        ...
                        # This halving step only reads _extract_rmse_p95_for_noise(res, ...)
                        # and re-stacks `strat`, the INPUT strategy. The theoretical
                        # profile is not needed here so it is not computed.
                        # The complete ELITE evaluation below retains it as required
                        # by full_res["strategy"].
