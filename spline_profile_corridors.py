...
def _fit_local_quadratic_rmse_profile(
    ...
    half = max(1, int(half_window_pts))
    ...
    # 1. Filtre basique des outliers pour robustifier l'ajustement
    if r_loc.size >= 4:
    ...
    # 2. Relais sur le centrage (excursion autorisée de 25% hors fenêtre)
    ...
    # 3 & 4. Borne d'extrapolation et excursion minimale de +/-0.2% d
    ...
    max_excursion = float(win_span * 2.5)  # Sécurité contre explosion
...
def _estimate_adaptive_rmse_abs_tolerance(
    ...
    *,
    ...
    h_ref = float(max(getattr(pconf, "adaptive_rmse_abs_ref_half_width_nm", 1.0) or 0.0, 1e-6))

    probe_steps = int(max(2, getattr(pconf, "adaptive_rmse_abs_probe_steps_each_side", 2) or 2))

    noise_factor = float(max(getattr(pconf, "adaptive_rmse_abs_noise_factor", 2.5) or 0.0, 0.0))
    ...
    fit_q = _fit_local_quadratic_rmse_profile(
        ...
        min(max(2, int(getattr(pconf, "parabola_half_window_pts", 3) or 3)), max(2, int(d_arr.size // 2))),
    ...
    delta_geom = float(max(float(fit_q.get("curvature", 0.0)), 0.0) * (float(h_ref) ** 2))

    delta_noise = float(noise_factor * sigma_prof) if np.isfinite(sigma_prof) else 0.0

    # P0.2 FIX: delta_geom is diagnostic only (curvature), NOT an uncertainty.
    # Using it in tolerance creates a self-justifying corridor equal to probe window.
    ...
    out.update(

        {
            ...
            "delta_rmse_geom": float(delta_geom),
...
class ProfileCorridorConfig:
    ...
    adaptive_rmse_abs_ref_half_width_nm: float = 1.5

    adaptive_rmse_abs_probe_steps_each_side: int = 3

    adaptive_rmse_abs_noise_factor: float = 3.0
    ...
    # P2.3 FIX: Archive full profiling state for reproducibility and debugging.
    # If True, saves all intermediate fit states, seeds, and gradients (memory-intensive but fully reproducible).
    archive_full_profiling_state: bool = False
...
def log_coaching_uncertainty_parameter_guide() -> None:
    from spline_corridor_log_coaching import log_coaching_uncertainty_parameter_guide as _impl
    _impl()


def log_coaching_corridor_pipeline_skip_empty() -> None:
    from spline_corridor_log_coaching import log_coaching_corridor_pipeline_skip_empty as _impl
    _impl()
...
def _log_coaching_corridor_outcome(
    ...
    *,
    ...
    from spline_corridor_log_coaching import _log_coaching_corridor_outcome as _impl
    _impl(
        pconf=pconf,
        use_lr=use_lr,
...
def _log_coaching_corridor_failure(
    ...
    *,
    ...
    from spline_corridor_log_coaching import _log_coaching_corridor_failure as _impl
    _impl(
        reason=reason,
        pconf=pconf,
        use_lr=use_lr,
...
def _log_coaching_bootstrap_outcome(
    ...
    *,
    ...
    from spline_corridor_log_coaching import _log_coaching_bootstrap_outcome as _impl
...
def _log_coaching_reg_sensitivity_outcome(
    ...
    *,
    ...
    from spline_corridor_log_coaching import _log_coaching_reg_sensitivity_outcome as _impl
...
def _fit_nodes_at_fixed_d(
    ...
    *,
    ...
    def _chi2_nodes(x_nodes: np.ndarray) -> float:
        ...
        x_full = _pack(x_nodes)
        n_lam, k_lam = nk_from_x_pwlnk(
            x_full, lam_full, sk, cfg.k_clip_lo, cfg.k_clip_hi,
            sig_pre=None, n_mono_band_nm=cfg.n_mono_band_nm,
            profile_interp=str(cfg.nk_profile_interp or "smooth"),
        )
        ...
        from spline_objective import DataType
        ...
        if cfg.data_type in (DataType.TRANSMISSION, DataType.BOTH) and t_exp_f is not None and float(cfg.weight_t) > 0:
            if cfg.t_is_ratio:
                t_th = _ratio_theoretical_from_nk(lam_f, n_f, k_f, n_sub_f, d_nm)
            else:
                t_th = calculate_transmission_array(lam_f, n_f, k_f, d_nm, n_sub_f)
        ...
        if cfg.data_type in (DataType.REFLECTION, DataType.BOTH) and r_exp_f is not None and float(cfg.weight_r) > 0:
            if cfg.t_is_ratio:
                r_th = _reflectance_ratio_theoretical_from_nk(lam_f, n_f, k_f, n_sub_f, d_nm)
            else:
    ...
    res = minimize(
        ...
        _obj_nodes,  # P0.1 FIX: Use chi² for LR mode, MSE for alpha mode
        ...
        if fit is not None and str(fit.get("message", "")).upper().find("EXCEEDS LIMIT") >= 0:
            ...
            if scale > 1.0:
                ...
                fit_retry = _fit_nodes_at_fixed_d(
                    ...
                    seed_keep_tol_abs=float(getattr(pconf, "seed_gate_tol_abs", 1e-5)),
                    pure_spectral=pure_spectral_refit,
                    # P0.1 FIX: Pass LR mode parameters for chi² objective coherence
                    use_lr=bool(use_lr),
                    sig_t=float(sig_t),
                    sig_r=float(sig_r),
                    sigma_t_f=sigma_t_f,
...
def _corridor_profile_walk_side(
    ...
    # P2.3 FIX: Archive full profiling state for reproducibility/debugging
    _archive_state: list[dict[str, Any]] = [] if bool(getattr(pconf, "archive_full_profiling_state", False)) else []

    # --- Pour le suivi de la parabole locale ---
    ...
    while nsteps < int(pconf.max_steps_each_side) and span < effective_max_span:
        ...
        if seed_gate_eval_count >= 3 and nsteps <= 5:
            ...
            if _kept_rate > 0.5 and not _auto_escalated:
                ...
                _old_nstarts = int(pconf.n_starts)
                ...
                log.warning(
                    ...
                    _LOG_PREFIX, dir_lbl, 100.0 * _kept_rate, _old_maxfun, maxfun_prof
                )
        ...
        # --- Détection de pic (Spike) vs Parabole locale ---
        # "Si elle colle avec la parabole on l'insert sinon on la met pas"
        is_spike = False
        rm_pred = float("nan")
        # Tolérance adaptative : on prend max(plancher, facteur * sigma_local)
        parab_tol_abs = float(getattr(pconf, "parabola_spike_tolerance_abs", 2e-5))
        
        if len(clean_d_vals) >= 5:
            try:
                # On utilise les derniers points "propres" (ceux acceptés par la parabole)
                _ds = np.asarray(clean_d_vals[-12:], dtype=np.float64)
                _rs = np.asarray(clean_rmse_vals[-12:], dtype=np.float64)
                # Fit parabolique local (relatif à d0 pour la stabilité)
                _coeffs = np.polyfit(_ds - d0, _rs, 2)
                rm_pred = float(np.polyval(_coeffs, d_try - d0))
                
                # Calcul du bruit local autour de la parabole pour une tolérance robuste
                _pred_hist = np.polyval(_coeffs, _ds - d0)
                _sigma = float(np.std(_rs - _pred_hist))
                _tol_eff = max(parab_tol_abs, 4.0 * _sigma) # 4-sigma pour être conservateur
        ...
        if is_spike:
            log.info(
                ...
                _LOG_PREFIX, dir_lbl, d_try, rm, rm_pred
            )
        ...
        if is_spike:
            ...
            # "sinon on la met pas" -> On avance l'épaisseur pour ne pas boucler, 
            # mais on n'ajoute pas le point aux résultats et on ne valide pas l'étape.
            d_prev = float(d_try)
            nsteps += 1
            # Note: x_prev reste le dernier point stable, favorisant un bon saut au prochain step.
            continue

        force_points = nsteps < max(3, int(getattr(pconf, "min_valid_each_side", 1)))
        ...
        if ok or force_points:
            ...
            if ok:
                ...
                # On met à jour l'historique "propre" pour la parabole
...
def compute_regular_grid_rmse_profile(
    ...
    *,
    ...
    """Recalcule RMSE(d) sur une grille d'épaisseurs imposées (refit n,L à d fixé).

    Même convention que le profiling corridor (base « scientific nominal » si disponible,

    sinon ``base_result`` tel quel). Retourne des clés ``profile_d_*`` à fusionner dans le dict résultat GUI.
    ...
    x_nodes0 = clip_to_bounds(

        np.asarray(x_nodes0, dtype=np.float64).ravel(), bounds_nodes[:, 0], bounds_nodes[:, 1]
    ...
    # Centre de parcours : grille demandée (visit_anchor_nm ou médiane des cibles), pas le d nominal du pack.
    # Sinon l'ordre « first/last » reste ancré sur l'ancienne épaisseur après recentrage GUI.
    ...
    if visit_indices:
        ...
        log.info(
            ...
            int(order_dbg.size),
            (
                ", ".join(f"{v:.4f}" for v in order_dbg[: min(5, int(order_dbg.size))])
                if order_dbg.size > 0
                else "-"
            ),
            (
    ...
    # Baseline nominal RMSE (avant toute mise à jour par global-opt) pour comparer au minimum discret grille.
    ...
    def _touch_absolute_best(d_nm: float, rmse: float, *, tag: str) -> None:
        """À chaque amélioration du meilleur RMSE vu sur cette grille / global_opt / P0 → log d et RMSE (précision float)."""
    ...
    def _append_fit_record(
        ...
        *,
        ...
        if i_same >= 0:
            ...
            keep_new = bool(
                np.isfinite(rm_new) and ((not np.isfinite(rm_old)) or (rm_new < rm_old - 1e-12))
            )
            if not keep_new:
                log.info(
            ...
            point_status_code_list[i_same] = int(
                min(int(point_status_code_list[i_same]), int(point_status_code))
            )
    ...
    def _fit_point_with_extra_polish(d_nm: float, x_seed_in: np.ndarray) -> dict[str, Any] | None:
        ...
        if np.isfinite(rm1) and (not np.isfinite(rm0) or rm1 <= rm0):
            ...
            fit1["nfev"] = int((fit0.get("nfev", 0) or 0) + (fit1.get("nfev", 0) or 0))

            fit1["nit"] = int((fit0.get("nit", 0) or 0) + (fit1.get("nit", 0) or 0))

            return fit1

        return fit0

    def _build_emergency_fit_record(d_nm: float, x_seed_in: np.ndarray) -> dict[str, Any] | None:
    ...
    def _detect_breakpoint(
        side: int, d_now: float, rmse_now: float
        ...
        # "Cassure" = amélioration trop brutale en un pas (heuristique conservative).
            ...
            if np.all(np.isfinite(d_prev)) and np.all(np.isfinite(r_prev)) and np.ptp(d_prev) > 1e-12:
                ...
                try:
                    ...
                    trigger_parabola = bool(
                        np.isfinite(parab_gain)
                        and (parab_gain > parab_thr)
                        and (parab_gain >= float(min_gain_abs_parab))
                    )
        ...
        *,
        ...
        for sgn in (+1, -1):
            ...
            if not (d_lo_b - 1e-12 <= d_try <= d_hi_b + 1e-12):
                continue
            fit_p = _fit_point_with_extra_polish(float(d_try), np.asarray(x_seed_start, dtype=np.float64).ravel().copy())
            if fit_p is None:
                continue
    ...
    def _run_global_opt_from_breakpoint(*, d_break: float, fit_break: dict[str, Any], side_origin: int) -> dict[str, Any] | None:
    ...
    for step_i, ji in enumerate(visit_indices):
        ...
        if side != 0:
            ...
            is_break, gain_abs, gain_rel, abs_thr, rel_thr, trigger_prev5, best_prev5, gain_vs_prev5, trigger_parabola, parab_pred, parab_gain, parab_thr = _detect_breakpoint(side, float(d_t), rm_now)

            if bool(is_break):
                if int(abs(int(ji) - int(side_last_break_index.get(side, -10_000_000)))) < int(min_breakpoint_spacing_points):
                    is_break = False
            ...
            if is_break and n_branch_trigger < int(max_branch_events):
                ...
                rmse_global = float(cand_global.get("rmse", float("nan"))) if isinstance(cand_global, dict) else float("nan")
                ...
                beat_abs = bool(
                    ...
                    and (
                        (not np.isfinite(rmse_abs_ref) and (not np.isfinite(best_global_rmse) or rmse_global < best_global_rmse))
    ...
    # ── P0 re-pass : rescue points piégés dans un minimum local (n,k) ────────────
    # Problème : le warmstart en chaîne (chaque point seedé depuis son voisin)
    # peut propager un mauvais minimum local sur tout un bras de l'exploration.
    # Solution : après la boucle principale, on re-fit une fois chaque point
    # « suspect » avec le seed du point optimal global.
    #
    # Détection des suspects : un point est « piégé » si son RMSE est
    # significativement au-dessus de CELUI DE SES VOISINS (pas du min global),
    # ce qui signale une bosse anormale plutôt qu'une montée naturelle de la
    # parabole.  Les points aux extrémités (pas de voisin des deux côtés)
    # ne sont suspects que si leur RMSE est très élevé par rapport au global.
    if d_list and r_list and (len(x_nodes_best_list) == len(d_list)):
        # We work on sorted-by-d order for neighbour detection.
        ...
        if np.any(finite_mask_p0):
            rmse_min_p0 = float(np.min(_p0_r[finite_mask_p0]))
            # Find the absolute best point for the seed.
            ...
            if np.isfinite(rmse_min_p0) and int(x_seed_gb.size) == 2 * int(k):
                # Neighbour-relative threshold: suspect if RMSE > max(left, right) + margin.
                p0_margin = float(max(5e-5, 0.01 * rmse_min_p0))
                # Fallback global threshold for edge points (no two-sided neighbours).
                p0_global_thr = float(max(5e-4, 0.15 * rmse_min_p0))
                suspect_sorted: list[int] = []
                for qi in range(_p0_n):
                    rv = float(_p0_r[qi])
                    if not np.isfinite(rv):
                        continue
                    has_left = qi > 0 and np.isfinite(float(_p0_r[qi - 1]))
                    has_right = qi < _p0_n - 1 and np.isfinite(float(_p0_r[qi + 1]))
                    if has_left and has_right:
                        # Interior point: suspect if RMSE above both neighbours.
                        ref_neigh = float(max(_p0_r[qi - 1], _p0_r[qi + 1]))
                        if rv > ref_neigh + p0_margin:
                            suspect_sorted.append(qi)
                    elif has_left or has_right:
                        # Edge point: use global fallback (only extreme outliers).
                        if rv > rmse_min_p0 + p0_global_thr:
                            suspect_sorted.append(qi)
                ...
                log.info(
                    ...
                    _LOG_PREFIX, rmse_min_p0, p0_margin, p0_global_thr, len(suspect), len(d_list),
                )
                n_p0_improved = 0
                for ii in suspect:
                    if stop_check is not None and bool(stop_check()):
                        break
                    fit_rp = _fit_point_with_extra_polish(float(d_list[ii]), x_seed_gb.copy())
                    if fit_rp is None:
                        continue
                    ...
                    if np.isfinite(rmse_rp) and rmse_rp < float(r_list[ii]) - 1e-12:
                        log.info(
                            ...
                            _LOG_PREFIX, float(d_list[ii]), float(r_list[ii]), rmse_rp,
                        ...
                        if int(k_rp.size) == int(lam_full.size):
                            k_list[ii] = k_rp
                        nfev_list[ii] += float(fit_rp.get("nfev", 0) or 0)
                        x_nodes_best_list[ii] = np.asarray(
                            fit_rp.get("x_nodes_best", x_seed_gb), dtype=np.float64
                        ).ravel().copy()
                        n_p0_improved += 1
                        _touch_absolute_best(float(d_list[ii]), float(rmse_rp), tag="P0_repass")
                log.info(
                    "%s P0 re-pass done | improved=%d/%d suspect pts",
                    _LOG_PREFIX, n_p0_improved, len(suspect),
                )
    # ─────────────────────────────────────────────────────────────────────────────
    ...
    missing_base_targets: list[float] = []
    for d_req in np.asarray(d_arr, dtype=np.float64).ravel():
        has_req = any(np.isclose(float(d0), float(d_req), rtol=0.0, atol=1e-9) for d0 in d_list)
        if not has_req:
            missing_base_targets.append(float(d_req))
    ...
    if missing_base_targets:
        ...
        for d_miss in missing_base_targets:
            fit_e = _build_emergency_fit_record(float(d_miss), x_seed_emergency)
            if fit_e is None:
                log.warning(
            ...
            x_seed_emergency = np.asarray(
                fit_e.get("x_nodes_best", x_seed_emergency),
                dtype=np.float64,
            ).ravel().copy()
            _emit_live(float(d_miss), 1.0)

    # Paranoid coverage audit: requested base-grid vs returned base-grid.
    ...
    if np.any(fg_min):
        ...
        if (
            np.isfinite(nom_b)
            and np.isfinite(r_cb)
            and (r_cb + rel_gate < nom_b)
            and (abs(d_cb - float(d0)) > 1e-3)
        ):
    ...
    if (
        ...
        log.info(
            "%s manual RMSE(d) grid | Synthèse: minimum discret courbe RMSE=%.8f @ d=%.6f nm < RMSE adoption "
    ...
    return {
        ...
        "profile_d_manual_grid_best_global_rmse": float(best_global_rmse) if np.isfinite(best_global_rmse) else float("nan"),

        "profile_d_manual_grid_best_global_result": dict(best_global_result) if isinstance(best_global_result, dict) else None,
...
def compute_profiled_corridors_by_d(
    ...
    *,
    ...
    use_abs_delta = (not use_lr) and rmse_thr_sub in ("abs_delta", "alpha_plus_delta", "abs_delta_adaptive", "alpha_plus_adaptive_delta")
    ...
    if _use_intelligent_alpha:
        ...
        if _alpha_user > alpha_intelligent * 1.1 and not use_explicit_auto_flag:
            ...
            log.info("%s Corridor intelligent threshold | N_eff=%d (N_obs=%d, N_params=%d) => alpha=%.5f", 
        ...
        log.info("%s Corridor user alpha threshold | alpha=%.5f (intelligent would be %.5f)", 
    ...
    _use_hetero = bool(
        getattr(pconf, "use_heteroscedastic_sigma", False)
        or getattr(pconf, "sigma_hetero_residual", False)
    )
    if _use_hetero:
        # Use new unified params, fallback to legacy for compatibility
        _heto_scale = float(
            getattr(pconf, "heteroscedastic_sigma_scale", None)
            or getattr(pconf, "sigma_hetero_scale", 1.0)
            or 1.0
        )
        _heto_floor = float(
    ...
    if _user_mask is not None and len(np.asarray(_user_mask)) > 0:
        log.info("%s User RMSE mask applied: %d wavelengths excluded", _LOG_PREFIX, int(np.sum(~np.asarray(_user_mask, dtype=bool))))
    ...
    if use_abs_delta:
        ...
        log.info(
            ...
            "%s Start | K_sigma=%d | d_opt=%.6f nm | RMSE_ref=%.8f (%s) | mode_seuil=%s | Delta_mode=%s | seuil absolu RMSE <= %.8f + Delta=%.6f -> %.8f | "
    ...
    if use_adaptive_abs_delta and np.isfinite(rmse_opt) and fit0 is not None and np.isfinite(rm_c):
        ...
        if bool(adaptive_abs_meta.get("ok", False)) and np.isfinite(float(adaptive_abs_meta.get("delta_rmse_tol", float("nan")))):
            ...
            # --- Insertion des points de sondage (sauf d0 déjà géré plus bas) ---
            ...
            for i_smp in range(len(_s_d)):
                if abs(float(_s_d[i_smp]) - float(d0)) < 1e-8:
                    continue # On ignore le centre ici, il sera ajouté proprement par la suite
    ...
    if parallel_walks:
        ...
        try:
            ...
            p_plus = pconf.replace(rng_seed=int(pconf.rng_seed) + 1_000_003)

            p_minus = pconf.replace(rng_seed=int(pconf.rng_seed) + 1_000_019)
    ...
    _sg_global_rate = (
        ...
        if int(seed_gate_eval_count) > 0 else float("nan")
    )
    seed_gate_saturated_global = (
        np.isfinite(_sg_global_rate) and _sg_global_rate > 0.5 and int(seed_gate_eval_count) >= 5
    )
    if seed_gate_saturated_global:
        log.warning(
            ...
            _LOG_PREFIX, 100.0 * _sg_global_rate
        )
    seed_gate_auto_escalated_global = bool(
        r_plus.get("seed_gate_auto_escalated", False)
        or r_minus.get("seed_gate_auto_escalated", False)
    )
    ...
    if len(d_vals) < min_req:
        ...
        if len(d_vals) >= 1 and auto_relaxed_alpha:
            ...
            _is_plateau = False
            if len(d_vals) >= 2 and len(rmse_vals) >= 2:
                _rmse_arr = np.asarray(rmse_vals, dtype=np.float64)
                ...
                _all_near_thresh = np.all(np.abs(_rmse_arr - rmse_thresh_active) < 0.01 * rmse_thresh_active)
            ...
            if _is_plateau:
                log.warning(
                    ...
                    _LOG_PREFIX, int(len(d_vals)), min_req, float(_rmse_mean), float(rmse_thresh_active)
                )
            else:
                log.warning(
    ...
    if n_stack.ndim == 2 and k_stack.ndim == 2 and n_stack.shape == k_stack.shape:
        ...
        if np.any(_crossing_mask):
            ...
            log.warning(
                ...
                _LOG_PREFIX, _n_bad, 100.0 * _frac_bad
            )
    ...
    parab_half_window_pts = int(
        max(
            1,
            int(getattr(pconf, "parabola_half_window_pts", 3) or 3),
            int(max(1, d_arr.size)),
        )
    )
    ...
    center_mode = str(getattr(pconf, "symmetric_interval_center_mode", "parabola") or "parabola").strip().lower()
    if bool(getattr(pconf, "force_symmetric_interval", True)):
        if center_mode == "parabola" and np.isfinite(parab_center_nm):
            d_center_sym = float(parab_center_nm)
            center_source = "parabola"
        else:
            d_center_sym = float(d0)
            center_source = "nominal"
    else:
        d_center_sym = float(parab_center_nm) if np.isfinite(parab_center_nm) else float(d0)
    ...
    min_half_sym = float(max(0.0, float(getattr(pconf, "symmetric_interval_min_half_width_rel", 0.002) or 0.0))) * max(abs(float(d0)), 1e-12)
    half_bounds = float(max(0.0, min(float(d_center_sym - float(cfg.d_lo)), float(cfg.d_hi - float(d_center_sym)))))
    if half_bounds > 0.0:
    ...
    if half_sym + 1e-12 < min_half_sym:
        ...
        log.info(
            ...
            float(d_center_sym),
            float(half_sym),
            100.0 * float(half_sym) / max(abs(float(d0)), 1e-12),
        )
    ...
    pos_valid = int(np.count_nonzero(d_arr > d0 + 1e-12))
    neg_valid = int(np.count_nonzero(d_arr < d0 - 1e-12))
    d_interval_sym = (float(d_sym_lo), float(d_sym_hi))

    log.info(
        "%s Intervalle auto elargi au maillage physique garanti (min_excursion incluse) | "
    ...
    # ── Source 2 : incertitude intrinsèque méthode ±1e-4 sur k (en k linéaire) ─────────────────
    # Appliquée AVANT les stats pour que logk_span reflète le corridor final (union des deux sources).
    ...
    if k_ref_for_min.size != np.asarray(k_lo, dtype=np.float64).ravel().size:
        k_ref_for_min = 0.5 * (
            np.asarray(k_lo, dtype=np.float64).ravel()
            + np.asarray(k_hi, dtype=np.float64).ravel()
        )
    ...
    # ── Stats de diagnostic : calculées APRÈS enforce, donc sur le corridor final ─────────────────
    ...
    m_logk_span = np.isfinite(k_lo) & np.isfinite(k_hi) & (np.asarray(k_lo, dtype=np.float64) > 0.0) & (np.asarray(k_hi, dtype=np.float64) > 0.0)
    ...
    k_ref_diag = (
        np.asarray(corridor_ref_k_lam, dtype=np.float64).ravel()
        if corridor_ref_k_lam is not None
        else np.asarray(base_result.get("k_lam", []), dtype=np.float64).ravel()
    )
    ...
    if k_ref_diag.size >= k_span.size and k_span.size > 0:
        ...
        n_ref_inside = int(np.count_nonzero(m_ref_inside & (k_ref_diag >= np.asarray(k_lo, dtype=np.float64)) & (k_ref_diag <= np.asarray(k_hi, dtype=np.float64))))
    ...
    if k_stack.ndim == 2 and k_stack.shape[0] >= 2 and k_stack.shape[1] > 0:
        ...
        if k_ref_curve is not None:
            ...
            for _i in range(int(k_stack.shape[0])):
                ...
                if np.any(m_row):
                    delta_logk[_i, m_row] = np.log10(np.maximum(kr[m_row], 1e-30)) - np.log10(np.maximum(k_ref_curve[m_row], 1e-30))
            ...
            log.info(
                ...
                _LOG_PREFIX,
                int(k_stack.shape[0]),
                f"{float(np.nanmedian(row_max_abs[np.isfinite(row_max_abs)])):.6e}" if np.any(np.isfinite(row_max_abs)) else "n/a",
                f"{float(np.nanmax(row_max_abs[np.isfinite(row_max_abs)])):.6e}" if np.any(np.isfinite(row_max_abs)) else "n/a",
                f"{float(np.nanmedian(row_med_abs[np.isfinite(row_med_abs)])):.6e}" if np.any(np.isfinite(row_med_abs)) else "n/a",
                f"{float(np.nanmax(row_med_abs[np.isfinite(row_med_abs)])):.6e}" if np.any(np.isfinite(row_med_abs)) else "n/a",
            )
    ...
    out_prof: dict[str, Any] = {
        ...
        "profile_d_sigma_hetero": bool(
            _use_hetero and (sigma_t_f_hetero is not None or sigma_r_f_hetero is not None)
        ),
        "profile_d_sigma_hetero_scale": float(
            getattr(pconf, "heteroscedastic_sigma_scale", None)
            or getattr(pconf, "sigma_hetero_scale", 1.0)
            or 1.0
        ) if _use_hetero else None,
        "profile_d_use_heteroscedastic_sigma": bool(getattr(pconf, "use_heteroscedastic_sigma", False)),
        "profile_d_heteroscedastic_sigma_floor": float(
            getattr(pconf, "heteroscedastic_sigma_floor", 1e-6)
        ) if _use_hetero else None,
        ...
        "profile_delta_rmse_abs": float(tol_abs_effective) if use_abs_delta else None,
        ...
        "profile_d_rmse_abs_tolerance": float(tol_abs_effective) if use_abs_delta else None,

        "profile_d_rmse_abs_tolerance_nominal": float(tol_abs) if use_abs_delta else None,

        "profile_d_rmse_abs_tolerance_adaptive": bool(use_adaptive_abs_delta),

        "profile_d_rmse_abs_tolerance_adaptive_ok": bool(adaptive_abs_meta.get("ok", False)) if use_abs_delta else None,

        "profile_d_rmse_abs_tolerance_geom": float(adaptive_abs_meta.get("delta_rmse_geom", float("nan"))) if use_abs_delta else None,

        "profile_d_rmse_abs_tolerance_noise": float(adaptive_abs_meta.get("delta_rmse_noise", float("nan"))) if use_abs_delta else None,

        "profile_d_rmse_abs_tolerance_profile_sigma": float(adaptive_abs_meta.get("profile_sigma", float("nan"))) if use_abs_delta else None,

        "profile_d_rmse_abs_tolerance_curvature": float(adaptive_abs_meta.get("curvature", float("nan"))) if use_abs_delta else None,

        "profile_d_rmse_abs_tolerance_center_nm": float(adaptive_abs_meta.get("center_nm", float("nan"))) if use_abs_delta else None,

        "profile_d_rmse_abs_tolerance_h_ref_nm": float(adaptive_abs_meta.get("h_ref_nm", float("nan"))) if use_abs_delta else None,
        ...
        "profile_d_seed_gate_saturated": bool(seed_gate_saturated_global),
        "profile_d_seed_gate_auto_escalated": bool(seed_gate_auto_escalated_global),

        # P1.1 FIX: User RMSE mask for manual spectral exclusion
        "profile_d_user_rmse_mask": (
            np.asarray(_user_mask, dtype=bool).copy()
            if _user_mask is not None and len(np.asarray(_user_mask)) > 0
            else None
        ),
