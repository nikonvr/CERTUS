...
from certus_core import NUMERICAL_FAULT_EXCEPTIONS, get_safe_worker_count
                ...
                live_point_cb(
                    {
                        "d_nm": float(d_try),
                        "rmse": float(rm),
                        "n_lam": np.asarray(fit["n_lam"], dtype=np.float64).ravel(),
                        "k_lam": np.asarray(fit["k_lam"], dtype=np.float64).ravel(),
                        "chi2": float(chi_v) if np.isfinite(chi_v) else float("nan"),
                    }
                )
...
def _build_emergency_fit_record(
    ...
    try:
        ...
        from spline_profile_corridors import clip_to_bounds
    ...
    def _run_walks_sequential() -> tuple[dict[str, Any], dict[str, Any]]:
        ...
        return (
            _corridor_profile_walk_side(
                ...
                ctx._push_live_point_from_payload,
            ),
            _corridor_profile_walk_side(
                -1.0,
                ctx.pconf,
