...
def find_matching_sheets(target_name: str, sheet_names: list[str]) -> list[str]:
    """Mirror of ``certus.utils.certus_strat_db.find_matching_sheets``.

    This copy exists because ``_certus_physics_impl`` is imported in
    sub-process workers where the full ``certus_strat_db`` module is not
    guaranteed to be available.  **Both copies must be kept identical.**

    When editing the alias table (ALIASES / MATERIALS / PROCESSES) update
    the copy in ``certus/utils/certus_strat_db.py`` as well.

    See the docstring in ``certus_strat_db.find_matching_sheets`` for the
    full specification of the matching algorithm.
    """
...
def merge_two_curves(c1, c2):
    """Mirror of ``certus.utils.certus_strat_db.merge_two_curves``.

    This copy exists so sub-process workers can merge dispersion curves
    without importing the full ``certus_strat_db`` module.
    **Both copies must be kept identical.**

    Algorithm summary
    -----------------
    * **No overlap**: simple concatenation.
    * **Single-point touch** (``overlap_max <= overlap_min``): shift c2 by
      a constant offset so n(λ) is C0-continuous at the boundary.
      DO NOT replace with blending — blending over a zero-width interval
      cannot eliminate the jump.
    * **True overlap**: linear blend from weight=0 (pure c1) to weight=1
      (pure c2) across the overlap band.

    See the docstring in ``certus_strat_db.merge_two_curves`` for the full
    physical contract and warnings.
    """
    wl1, n1 = c1['wl'], c1['n']
    wl2, n2 = c2['wl'], c2['n']
...
def merge_multiple_curves(curves: list[dict]) -> dict:
    """Mirror of ``certus.utils.certus_strat_db.merge_multiple_curves``.

    Sorts curves by minimum wavelength, then folds left-to-right with
    :func:`merge_two_curves`.  Keep both copies in sync.
    """
    if not curves:
        return {}
...
class MergedMaterialDict(dict):
    """Mirror of ``certus.utils.certus_strat_db.MergedMaterialDict``.

    Drop-in ``dict`` replacement that transparently merges multi-sheet
    material data from ``clues.xlsx`` on first access, using
    :func:`find_matching_sheets` and :func:`merge_multiple_curves`.

    Results are cached in ``self._merged_cache`` after the first lookup.

    Keep this class in sync with the canonical implementation in
    ``certus/utils/certus_strat_db.py``.
    """
    def __init__(self, raw_data, sheet_names, logger=None):
        super().__init__(raw_data)
        self.sheet_names = sheet_names
        self.logger = logger or logging.getLogger("MaterialDB")
        self._merged_cache = {}
...
def simulate_growth_kernel(
    ...
    # [FIX 2026] Adaptive sampling to match Phase A resolution (ceil(thickness/1.0))

    n_dyn_steps = max(21, int(np.ceil(nominal_th / 1.0)) + 1)
    ...
    if nominal_th > 1e-4:
        ...
        idx_mono = np.array([0, q // 4, q // 2, 3 * q // 4, q])

        T_mono = T_vals_for_dyn[idx_mono]

        diffs = np.diff(T_mono)
...
def validate_wavelengths_batch(
    ...
    for c_idx in prange(n_cands):
        ...
        p95 = np.percentile(abs_errors, 95.0)

        results[c_idx, 0] = p95

        results[c_idx, 1] = np.std(abs_errors)
