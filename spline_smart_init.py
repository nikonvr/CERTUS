...
from certus_core import NUMERICAL_FAULT_EXCEPTIONS, K_MAX_LIMIT, N_MAX_LIMIT, N_MIN_LIMIT


from certus_index_utils import _ratio_theoretical_from_nk, _transmittance_absolute_from_nk
...
def interp_n_L_pwlnk_to_sigmas(
    ...
    *,
    ...
    cnt_snap = cnt_interp = cnt_exlo = cnt_exhi = 0
    ...
    *,
    ...
    def _eval_d_mse_and_t(dv: float) -> tuple[float, np.ndarray]:
        ...
        if cfg.t_is_ratio and t_sub_cache is not None:
            t_th = _ratio_theoretical_from_nk(lam_f, n_l, k_l, dv, n_sub_f)

        else:
            t_th = _transmittance_absolute_from_nk(lam_f, n_l, k_l, dv, n_sub_f)
    ...
    obj_combined: SplinePWLObjective | None = SplinePWLObjective(cfg_obj, sk) if use_combined_d else None
...
def compute_smart_init_spectral_preview(
    ...
    *,
    ...
    if uniform_sigma_nodes is not None and int(uniform_sigma_nodes) >= 2:
        ...
        sk = build_smart_manual_sigma_knots_from_preview_grid(sig_f, n_uniform_in_sigma2=int(uniform_sigma_nodes))
    ...
    return _finalize_smart_preview_from_nodes(cfg, sk, sn, sL, grids, d_scan_points=d_scan_points)
...
def guess_smart_x0_from_extrema(
    ...
    E = (8.0 * n_swan * n_swan * ns) / np.maximum(r_M * t_nu, eps) + (n_swan * n_swan - 1.0) * (
        n_swan * n_swan - ns * ns
    )
...
def smart_init_sweep_node_thickness_rmse(
    ...
    *,
    ...
    try:
        _, rmse_init = rmse_at_spline_stage_x0_init(cfg, sk_a, n0b, L0b, d_ref, relax_n_mono=relax_n_mono)

    except (TypeError, ValueError, AttributeError, RuntimeError):
        logging.getLogger("CERTUS").debug("rmse_at_spline_stage_x0_init failed for reference RMSE", exc_info=True)

        rmse_init = float("inf")

    rel_tol = max(1e-12, abs(float(rmse_init)) * 1e-9) if np.isfinite(rmse_init) else 1e-12

    def _rmse_of(dv: float, nw: np.ndarray, Lw: np.ndarray) -> float:

        try:
            _, rm = rmse_at_spline_stage_x0_init(cfg, sk_a, nw, Lw, float(dv), relax_n_mono=relax_n_mono)
    ...
    def _obj_2d(p) -> Any:
        ...
        try:
            _, rm = rmse_at_spline_stage_x0_init(cfg, sk_a, n_w, L_w, dv, relax_n_mono=relax_n_mono)
...
def tune_d_and_rmse_for_manual_material_preset(
    ...
    *,
    ...
    _, n_p, L_p, _dref = project_manual_material_preset(str(preset_id), sk_t, d_nm_hint=float(d_nm_hint))

    def obj_d(dv: float) -> float:

        _, rm = rmse_at_spline_stage_x0_init(cfg, sk_t, n_p, L_p, float(dv), relax_n_mono=relax_n_mono)
    ...
    _, rmse = rmse_at_spline_stage_x0_init(cfg, sk_t, n_p, L_p, d_opt, relax_n_mono=relax_n_mono)
