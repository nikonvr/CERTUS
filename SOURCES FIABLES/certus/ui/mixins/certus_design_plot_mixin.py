...
class CertusDesignUIPlotMixin:
    ...
    def _ensure_pareto_ui(self) -> bool:
        ...
        if QApplication.instance() is None:
            self.log("Pareto is not available yet: Qt application is not ready.", "WARNING")
            return False

        if getattr(self, "pareto_table", None) is None:
            self.log("Pareto is not available yet: table is still initializing.", "WARNING")
            return False

        if getattr(self, "pareto_window", None) is None:
            parent = self if isinstance(self, QWidget) else None
            self.pareto_window = QDialog(parent)
